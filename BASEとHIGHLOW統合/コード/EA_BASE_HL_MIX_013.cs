...
private bool PassesEma10Ema20DirectionDecision(TradeType type, int i1)
        ...
        private sealed class ForwardNewsSharedCacheFile
        {
        ...
        private string GetForwardNewsSharedCacheDirectory()
        {
            string localAppData = Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData);
            return Path.Combine(localAppData, "EA_NEWS_CACHE");
        }

        private string GetForwardNewsSharedCacheJsonPath(DateTime utcDate)
        {
            return Path.Combine(GetForwardNewsSharedCacheDirectory(),
                "news_" + utcDate.ToString("yyyyMMdd", CultureInfo.InvariantCulture) + ".json");
        }

        private string GetForwardNewsSharedCacheLockPath(DateTime utcDate)
        {
            return Path.Combine(GetForwardNewsSharedCacheDirectory(),
                "news_" + utcDate.ToString("yyyyMMdd", CultureInfo.InvariantCulture) + ".lock");
        }

        private bool TryLoadForwardNewsSharedCache(DateTime utcDate, out List<NewsEvent> events, out string error)
        {
            events = new List<NewsEvent>();
            error = "CACHE_MISS";

            string jsonPath = GetForwardNewsSharedCacheJsonPath(utcDate);
            ...
            try
            {
                ...
                {
                    error = "CACHE_READ_FAIL";
                    return false;
                }

                ForwardNewsSharedCacheFile cache = System.Text.Json.JsonSerializer.Deserialize<ForwardNewsSharedCacheFile>(text);
                if (cache == null)
                {
                    error = "CACHE_READ_FAIL";
                    return false;
                ...
                string expectedDate = utcDate.ToString("yyyy-MM-dd", CultureInfo.InvariantCulture);
                string actualDate = (cache.dateUtc ?? "").Trim();
                if (!string.Equals(actualDate, expectedDate, StringComparison.Ordinal))
                {
                    error = "CACHE_READ_FAIL";
                    return false;
                }

                List<NewsEvent> src = cache.events ?? new List<NewsEvent>();
                for (int i = 0; i < src.Count; i++)
                {
                    ...
                    DateTime eventUtc = ev.UtcTime;
                    if (eventUtc.Kind != DateTimeKind.Utc)
                        eventUtc = DateTime.SpecifyKind(eventUtc.ToUniversalTime(), DateTimeKind.Utc);

                    if (eventUtc.Date != utcDate.Date)
                        continue;

                    events.Add(new NewsEvent
                    {
                        UtcTime = eventUtc,
                        Currency = (ev.Currency ?? "").Trim().ToUpperInvariant(),
        ...
        private bool TrySaveForwardNewsSharedCache(DateTime utcDate, List<NewsEvent> sourceEvents, out string error)
        {
            error = "";

            string dir = GetForwardNewsSharedCacheDirectory();
            string jsonPath = GetForwardNewsSharedCacheJsonPath(utcDate);
            string tmpPath = Path.Combine(dir,
                "news_" + utcDate.ToString("yyyyMMdd", CultureInfo.InvariantCulture) + ".tmp");

            try
            {
                Directory.CreateDirectory(dir);

                List<NewsEvent> snapshot = new List<NewsEvent>();
                if (sourceEvents != null)
                {
                    for (int i = 0; i < sourceEvents.Count; i++)
                    {
                        NewsEvent ev = sourceEvents[i];
                        if (ev == null)
                            continue;
                ...
                snapshot.Sort((a, b) => a.UtcTime.CompareTo(b.UtcTime));

                ForwardNewsSharedCacheFile cache = new ForwardNewsSharedCacheFile
                {
                    dateUtc = utcDate.ToString("yyyy-MM-dd", CultureInfo.InvariantCulture),
                    version = 1,
                    events = snapshot
                };
            ...
            catch (Exception ex)
            {
                ...
                error = "CACHE_WRITE_" + ex.GetType().Name;
                return false;
            }
        }

        private bool TryAcquireForwardNewsSharedCacheLock(DateTime utcDate, out FileStream lockHandle)
        {
            lockHandle = null;

            try
            {
                Directory.CreateDirectory(GetForwardNewsSharedCacheDirectory());
                lockHandle = new FileStream(
                    GetForwardNewsSharedCacheLockPath(utcDate),
        ...
        private void ReleaseForwardNewsSharedCacheLock(DateTime utcDate, FileStream lockHandle)
        {
            try
            {
                if (lockHandle != null)
            ...
            try
            {
                string lockPath = GetForwardNewsSharedCacheLockPath(utcDate);
        ...
        private void SetForwardNewsState(DateTime utcDate, List<NewsEvent> events)
        {
            _newsEventsUtc.Clear();
            _newsForwardEvents = events ?? new List<NewsEvent>();
            _newsLoadedUtcDate = utcDate;
        }

        private void LogForwardNewsSource(DateTime utcNow, DateTime todayUtc, string mode)
        {
            HashSet<string> targetCurrencies = GetTargetCurrenciesForSymbol(SymbolName);
            int minImpact = GetEffectiveMinImpactLevel();
            ...
            Print("NEWS_SOURCE | CodeName={0} | Symbol={1} | Mode={2} | UtcDate={3:yyyy-MM-dd} | Events={4} | MinImpact={5} | TargetCcy={6} | NextEventUtc={7} | NextUnblockUtc={8}{9}",
                CODE_NAME, SymbolName, mode, todayUtc, _newsForwardEvents.Count, minImpact, ccy, nextEventUtc, nextUnblockUtc, BuildTimeTag(utcNow));
        }

        private void LogForwardNewsFallback(DateTime utcNow, DateTime todayUtc, string mode, string reason)
        {
            HashSet<string> targetCurrencies = GetTargetCurrenciesForSymbol(SymbolName);
            int minImpact = GetEffectiveMinImpactLevel();
            NewsEvent next = FindNextRelevantForwardEventUtc(utcNow);
            ...
            string nextEventUtc = (next == null) ? "NA" : next.UtcTime.ToString("yyyy-MM-dd HH:mm", CultureInfo.InvariantCulture);
            string nextUnblockUtc = (next == null) ? "NA" : next.UtcTime.AddMinutes(Math.Max(0, MinutesAfterNews)).ToString("yyyy-MM-dd HH:mm", CultureInfo.InvariantCulture);
            string resolvedReason = string.IsNullOrWhiteSpace(reason) ? "NA" : reason;

            Print("NEWS_FALLBACK | CodeName={0} | Symbol={1} | Mode={2} | UtcDate={3:yyyy-MM-dd} | Reason={4} | Events={5} | MinImpact={6} | TargetCcy={7} | NextEventUtc={8} | NextUnblockUtc={9}{10}",
                CODE_NAME, SymbolName, mode, todayUtc, resolvedReason, _newsForwardEvents.Count, minImpact, ccy, nextEventUtc, nextUnblockUtc, BuildTimeTag(utcNow));
        }
...
Stop();
            ...
            if (TryLoadForwardNewsSharedCache(todayUtc, out List<NewsEvent> cachedEvents, out string cacheReadErr))
            {
                SetForwardNewsState(todayUtc, cachedEvents);
                LogForwardNewsSource(utcNow, todayUtc, "FORWARD_CACHE");
                return;
            }

            bool sawCacheReadFail = string.Equals(cacheReadErr, "CACHE_READ_FAIL", StringComparison.OrdinalIgnoreCase);

            FileStream newsLockHandle;
            bool isMaster = TryAcquireForwardNewsSharedCacheLock(todayUtc, out newsLockHandle);
            if (isMaster)
            {
                try
                {
                    if (TryLoadCalendarEventsForDate(todayUtc, out List<NewsEvent> eventsUtc, out string apiErr))
                    {
                        if (TrySaveForwardNewsSharedCache(todayUtc, eventsUtc, out string cacheWriteErr))
                        {
                            SetForwardNewsState(todayUtc, eventsUtc);
                            LogForwardNewsSource(utcNow, todayUtc, "FORWARD_API_MASTER");
                            return;
                        }

                        SetForwardNewsState(todayUtc, BuildFallbackTemplateEventsUtc(todayUtc));
                        LogForwardNewsFallback(utcNow, todayUtc, "FORWARD_API_MASTER", cacheWriteErr);
                        return;
                    }

                    SetForwardNewsState(todayUtc, BuildFallbackTemplateEventsUtc(todayUtc));
                    LogForwardNewsFallback(utcNow, todayUtc, "FORWARD_API_MASTER", apiErr);
                    return;
                }
                finally
                {
                    ReleaseForwardNewsSharedCacheLock(todayUtc, newsLockHandle);
                }
            }

            for (int retry = 0; retry < 5; retry++)
            {
                int delayMs = 100 + (int)(((uint)(Environment.TickCount + ((retry + 1) * 97))) % 201u);
                System.Threading.Thread.Sleep(delayMs);

                if (TryLoadForwardNewsSharedCache(todayUtc, out List<NewsEvent> slaveCachedEvents, out string slaveCacheErr))
                {
                    SetForwardNewsState(todayUtc, slaveCachedEvents);
                    LogForwardNewsSource(utcNow, todayUtc, "FORWARD_CACHE");
                    return;
                }

                if (string.Equals(slaveCacheErr, "CACHE_READ_FAIL", StringComparison.OrdinalIgnoreCase))
                    sawCacheReadFail = true;
            }

            SetForwardNewsState(todayUtc, BuildFallbackTemplateEventsUtc(todayUtc));
            LogForwardNewsFallback(utcNow, todayUtc, "FORWARD_CACHE", sawCacheReadFail ? "CACHE_READ_FAIL" : "CACHE_MISS");
        }
...
private bool IsForwardNewsEventMatch(NewsEvent ev, HashSet<string> targetCurrencies, int minImpact)
        ...
        private bool TryLoadCalendarEventsForDate(DateTime utcDate, out List<NewsEvent> events, out string error)
        {
            ...
            try
            {
                ...
                // countryCode / volatility は固定しない（取得後に currencyCode / MinImpact で絞る）
                string url = "https://" + host + "/calendar/history/next-month";
                                ...
                                if (root.ValueKind == System.Text.Json.JsonValueKind.Object && root.TryGetProperty("events", out eventsEl) && eventsEl.ValueKind == System.Text.Json.JsonValueKind.Array)
                                {
                                    ...
                                    for (int i = 0; i < eventsEl.GetArrayLength(); i++)
                                    {
                                        ...
                                        if (string.IsNullOrWhiteSpace(currency))
                                            continue;
                                        if (currency != "USD" && currency != "JPY" && currency != "EUR" && currency != "GBP")
                                            continue;
                                    ...
                                    Print("NEWS_API_FILTER | CodeName={0} | Symbol={1} | UtcDate={2:yyyy-MM-dd} | RAW={3} | CCY4={4} | IMP={5}",
                                        CODE_NAME, SymbolName, day, rawCount, ccy4Count, impCount);
                                }
                                else
                                {
