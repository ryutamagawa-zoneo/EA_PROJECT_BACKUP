...
private void MarkEntryAttempt(int decisionBarIndex, DateTime nowUtc)
{
    ...
    _entryGuardUntilUtc = nowUtc.AddMilliseconds(800);
        ...
        private string MaskSensitiveParameterValue(string propertyName, string valueText)
        {
            ...
            if (string.Equals(propertyName, "FredApiKey", StringComparison.Ordinal))
                return "REDACTED";
...
private void EmitExecuteSendJsonl(TradeType plannedType, int barIndex, long volumeUnits, double slPipsToSend, double tpPipsToSend)
        ...
        // ============================================================
        // [ADD] HighLow（HL）トレンドフィルター統合パラメータ
        // ============================================================
        #region HighLowトレンドフィルター

        [Parameter("HLフィルターを使用", Group = "HighLowトレンドフィルター", DefaultValue = false)]
        public bool UseHLFilter { get; set; }

        [Parameter("HL_計算対象バー", Group = "HighLowトレンドフィルター", DefaultValue = 500, MinValue = 200)]
        public int HL_計算対象バー { get; set; }

        [Parameter("HL_ログ：毎バー出す", Group = "HighLowトレンドフィルター", DefaultValue = false)]
        public bool HL_ログ毎バー出す { get; set; }

        [Parameter("HL_EA内部Pivot描画", Group = "HighLowトレンドフィルター", DefaultValue = false)]
        public bool HL_EA内部Pivot描画 { get; set; }

        [Parameter("HL_強制クリーン頻度(0=しない/1=毎回)", Group = "HighLowトレンドフィルター", DefaultValue = 50, MinValue = 0)]
        public int HL_強制クリーン頻度 { get; set; }

        [Parameter("HL_描画検証ログ", Group = "HighLowトレンドフィルター", DefaultValue = false)]
        public bool HL_描画検証ログ { get; set; }

        [Parameter("HL_ログ出力（はい / いいえ）", Group = "HighLowトレンドフィルター", DefaultValue = true)]
        public bool HL_LogOutput { get; set; }

        [Parameter("HL_同値とみなす許容幅（ピップス）", Group = "HighLowトレンドフィルター", DefaultValue = 0.0, MinValue = 0.0)]
...
";
            ...
            out int loIndex, out int hiIndex, out bool hlFormed, out bool lhFormed, out string reason)
        {
            ...
            if (state == TrendState.DownTrend)
            {
                ...
                refHigh = trendLessRefHigh; refLow = defenseLow; return state;
            }
