...
private void EmitExecuteSendJsonl(TradeType plannedType, int barIndex, long volumeUnits, double slPipsToSend, double tpPipsToSend)
        ...
        [Parameter("停止対象重要度（中以上／高のみ）", Group = "経済指標フィルター（UTC）", DefaultValue = 2)]
        public int MinImpactLevel { get; set; }
...
private bool PassesEma10Ema20DirectionDecision(TradeType type, int i1)
        ...
        private void News_InitOrRefresh(DateTime utcNow, bool force = false)
        {
            ...
            if (TryLoadCalendarEventsForDate(todayUtc, out List<NewsEvent> eventsUtc, out string err))
            {
                ...
                Print("NEWS_SOURCE | CodeName={0} | Symbol={1} | Mode=FORWARD_API | Events={2}{3}",
                    CODE_NAME, SymbolName, _newsForwardEvents.Count, BuildTimeTag(utcNow));
            }
            else
            {
                _newsEventsUtc.Clear();
                ...
                _newsLoadedUtcDate = todayUtc;
                Print("NEWS_FALLBACK | CodeName={0} | Symbol={1} | Reason={2} | Events={3}{4}",
                    CODE_NAME, SymbolName, string.IsNullOrWhiteSpace(err) ? "NA" : err, _newsForwardEvents.Count, BuildTimeTag(utcNow));
            }
        }
        ...
        private bool TryLoadCalendarEventsForDate(DateTime utcDate, out List<NewsEvent> events, out string error)
        {
            ...
            try
            {
                ...
                string url =
                    ...
                    "?d1=" + Uri.EscapeDataString(dateStr) +
                    "&d2=" + Uri.EscapeDataString(dateStr) +
                    "&c=" + Uri.EscapeDataString(CalendarApiKey ?? "");
                ...
                List<string> objects = ExtractTopLevelJsonObjects(body);
                for (int i = 0; i < objects.Count; i++)
                {
                    string obj = objects[i];

                    if (!TryGetJsonStringField(obj, "Date", out string dateText))
                        continue;
                    if (!TryGetJsonStringField(obj, "Currency", out string currency))
                        continue;
                    if (!TryGetJsonRawField(obj, "Importance", out string importanceRaw))
                        continue;
                    if (!TryParseCalendarImportance(importanceRaw, out int importance))
                        continue;
                    if (!TryParseCalendarApiUtcDateTime(dateText, out DateTime eventUtc))
                        continue;

                    string title = "";
                    TryGetJsonStringField(obj, "Event", out title);
