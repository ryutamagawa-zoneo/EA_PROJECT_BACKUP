...
namespace cAlgo
{
    ...
    public class EA_HL_010_007 : Robot
    {
        // ============================================================
        // CODE NAME (Project Constitution compliant)
        // ============================================================
        // BASE: EA_HL_010_006
        // THIS: EA_HL_010_007
        private const string CODE_NAME = "EA_HL_010_007";
...
private void EmitExecuteSendJsonl(TradeType plannedType, int barIndex, long volumeUnits, double slPipsToSend, double tpPipsToSend)
        ...
        [Parameter("判定モード", Group = "レンジ・トレンド判定", DefaultValue = 判定モード種別.Auto)]
        public 判定モード種別 RegimeJudgeMode { get; set; }

        [Parameter("判定モード（ON/OFF）", Group = "レンジ・トレンド判定", DefaultValue = true)]
        public bool RegimeJudgeEnabled { get; set; }

        [Parameter("判定状態をチャート表示する", Group = "レンジ・トレンド判定", DefaultValue = true)]
        public bool RegimeShowStateOnChart { get; set; }

        [Parameter("判定ログを出力する", Group = "レンジ・トレンド判定", DefaultValue = false)]
        public bool RegimeEnableLog { get; set; }

        [Parameter("判定に使用する時間足", Group = "レンジ・トレンド判定", DefaultValue = 判定時間足.H1)]
        public 判定時間足 RegimeJudgeTimeFrame { get; set; }

        [Parameter("判定に使用する最大バー数", Group = "レンジ・トレンド判定", DefaultValue = 3000, MinValue = 200)]
        public int RegimeMaxBars { get; set; }

        [Parameter("トレンド確定に必要な連続構造回数", Group = "レンジ・トレンド判定", DefaultValue = 2, MinValue = 1)]
        public int RegimeTrendStructureNeedCount { get; set; }

        [Parameter("レンジ判定スイング幅上限（pips）", Group = "レンジ・トレンド判定", DefaultValue = 120, MinValue = 1)]
        public int RegimeRangeSwingUpperPips { get; set; }

        [Parameter("有効スイングとみなす最小幅（pips）", Group = "レンジ・トレンド判定", DefaultValue = 40, MinValue = 1)]
        public int RegimeMinValidSwingPips { get; set; }

        [Parameter("同値とみなす許容幅（pips）", Group = "レンジ・トレンド判定", DefaultValue = 5, MinValue = 0)]
        public int RegimeEqualTolerancePips { get; set; }

        [Parameter("ATR判定を使用する", Group = "レンジ・トレンド判定", DefaultValue = true)]
        public bool RegimeUseAtrJudge { get; set; }

        [Parameter("ATR期間", Group = "レンジ・トレンド判定", DefaultValue = 14, MinValue = 2)]
        public int RegimeAtrPeriod { get; set; }

        [Parameter("レンジとみなすATR上限（pips）", Group = "レンジ・トレンド判定", DefaultValue = 80, MinValue = 1)]
        public int RegimeRangeAtrUpperPips { get; set; }

        [Parameter("トレンドとみなすATR下限（pips）", Group = "レンジ・トレンド判定", DefaultValue = 110, MinValue = 1)]
        public int RegimeTrendAtrLowerPips { get; set; }

        [Parameter("EMA判定を使用する", Group = "レンジ・トレンド判定", DefaultValue = true)]
        public bool RegimeUseEmaJudge { get; set; }

        [Parameter("EMA期間", Group = "レンジ・トレンド判定", DefaultValue = 200, MinValue = 2)]
        public int RegimeEmaPeriod { get; set; }

        [Parameter("EMA傾きの最小変化幅（pips）", Group = "レンジ・トレンド判定", DefaultValue = 10, MinValue = 0)]
        public int RegimeEmaSlopeMinChangePips { get; set; }

        [Parameter("価格とEMAの最小乖離幅（pips）", Group = "レンジ・トレンド判定", DefaultValue = 25, MinValue = 0)]
        public int RegimePriceEmaMinGapPips { get; set; }

        [Parameter("EMA評価に使用するバー数", Group = "レンジ・トレンド判定", DefaultValue = 20, MinValue = 2)]
        public int RegimeEmaEvalBars { get; set; }

        [Parameter("トレンド確定スコア閾値", Group = "レンジ・トレンド判定", DefaultValue = 3)]
        public int RegimeTrendScoreThreshold { get; set; }

        [Parameter("レンジ確定スコア閾値", Group = "レンジ・トレンド判定", DefaultValue = -3)]
        public int RegimeRangeScoreThreshold { get; set; }

        [Parameter("判定確定に必要な連続回数", Group = "レンジ・トレンド判定", DefaultValue = 3, MinValue = 1)]
        public int RegimeConfirmConsecutiveCount { get; set; }

        [Parameter("判定後の最低維持バー数", Group = "レンジ・トレンド判定", DefaultValue = 5, MinValue = 0)]
        public int RegimeMinHoldBarsAfterSwitch { get; set; }

        [Parameter("判定表示フォントサイズ", Group = "レンジ・トレンド判定", DefaultValue = 28, MinValue = 10, MaxValue = 80)]
        public int RegimeDisplayFontSize { get; set; }

        [Parameter("判定表示横オフセット", Group = "レンジ・トレンド判定", DefaultValue = 15, MinValue = 0, MaxValue = 500)]
        public int RegimeDisplayOffsetX { get; set; }

        [Parameter("判定表示縦オフセット", Group = "レンジ・トレンド判定", DefaultValue = 15, MinValue = 0, MaxValue = 500)]
        public int RegimeDisplayOffsetY { get; set; }
        ...
        private sealed class RegimeJudgeDetail
        {
            public int Score;
            public int StructureScore;
            public int AtrScore;
            public int EmaScore;
            public double LatestSwingInternalPips = double.NaN;
            public double AtrInternalPips = double.NaN;
            public double EmaSlopeInternalPips = double.NaN;
            public double PriceEmaGapInternalPips = double.NaN;
            public double RangeSwingUpperInternalPips;
            public double MinValidSwingInternalPips;
            public double EqualToleranceInternalPips;
            public double AtrRangeUpperInternalPips;
            public double AtrTrendLowerInternalPips;
            public double EmaSlopeMinInternalPips;
            public double PriceEmaGapMinInternalPips;
            public int StructureTrendStreak;
            public int UsedBars;
            public string Note = "";
        }

        private const string REGIME_STATUS_NAME_MAIN = "REGIME_STATUS";
        private const string REGIME_STATUS_NAME_BOLD_1 = "REGIME_STATUS_B1";
        private const string REGIME_STATUS_NAME_BOLD_2 = "REGIME_STATUS_B2";

        private Bars _regimeBars;
        private TimeFrame _regimeBarsTimeFrame = TimeFrame.Hour;
        private RegimeState _regimeState = RegimeState.Standby;
        private RegimeState _regimeRawState = RegimeState.Standby;
        private RegimeState _regimeCandidateState = RegimeState.Standby;
        private int _regimeCandidateCount = 0;
        private int _regimeHoldCount = 0;
        private string _regimeLastDrawnText = "";
        private int _regimeLastDrawnFontSize = -1;
        private int _regimeLastDrawnOffsetX = -1;
        private int _regimeLastDrawnOffsetY = -1;
...
private DateTime? ComputeNewsUnblockUtc(DateTime utcNow)
{
    ...
    return null;
        ...
        private const int HL_RightBars_M5 = 1;
        private const int HL_RightBars_H1 = 2;
        ...
        private bool _regimeAndHlOffCleanupDone = false;
        ...
        protected override void OnStart()
        {
            ...
            _regimeBarsTimeFrame = ResolveRegimeJudgeTimeFrame();
            _regimeBars = MarketData.GetBars(_regimeBarsTimeFrame, SymbolName);
            ...
            if (RegimeJudgeEnabled)
                Print("HL_FORCE_CLEAN_STARTUP | Removed={0} Failed={1}", startupForceCleanRemoved, startupForceCleanFailed);

            if (!RegimeJudgeEnabled)
                ApplyRegimeAndHlOffModeCleanupIfNeeded();
            else
                EvaluateRegimeAndUpdateUi(utcNow, true);
        }
        ...
        protected override void OnBar()
        {
            ...
            {
                _regimeAndHlOffCleanupDone = false;
            ...
            DateTime utcNow = UtcNow();
            if (RegimeJudgeEnabled)
                EvaluateRegimeAndUpdateUi(utcNow, false);
...
";
        ...
        private TimeFrame ResolveRegimeJudgeTimeFrame()
        {
            switch (RegimeJudgeTimeFrame)
            {
                case 判定時間足.M30:
                    return TimeFrame.Minute30;
                case 判定時間足.H4:
        ...
        private Bars EnsureRegimeBars()
        {
            TimeFrame tf = ResolveRegimeJudgeTimeFrame();
            if (_regimeBars == null || _regimeBarsTimeFrame != tf)
            {
                _regimeBarsTimeFrame = tf;
                _regimeBars = MarketData.GetBars(tf, SymbolName);
            }

            return _regimeBars;
        }

        private void ResetRegimeStateToStandby()
        {
            _regimeRawState = RegimeState.Standby;
            _regimeState = RegimeState.Standby;
            _regimeCandidateState = RegimeState.Standby;
            _regimeCandidateCount = 0;
            _regimeHoldCount = 0;
        }

        private void ApplyRegimeAndHlOffModeCleanupIfNeeded()
        {
            if (_regimeAndHlOffCleanupDone)
                return;

            ResetRegimeStateToStandby();
            RemoveRegimeStatusDisplay();
            HL_ClearDrawings(true);

            _m5Pivots = new List<HL_Pivot>(1024);
            _h1Pivots = new List<HL_Pivot>(1024);
            _h1Projected = new List<HL_Pivot>(1024);
            _hlRedrawPending = false;
            _regimeAndHlOffCleanupDone = true;
        }

        private void EvaluateRegimeAndUpdateUi(DateTime utcNow, bool initializeOnStart)
        {
            if (!RegimeJudgeEnabled)
            {
                ApplyRegimeAndHlOffModeCleanupIfNeeded();
            ...
            RegimeJudgeDetail detail;
            RegimeState rawState = DetermineRegime(out detail);
            _regimeRawState = rawState;
            string transitionReason = "NA";

            if (initializeOnStart)
            {
                _regimeState = rawState;
                _regimeCandidateState = rawState;
                _regimeCandidateCount = 1;
                _regimeHoldCount = 0;
                transitionReason = "INIT_SYNC_TO_RAW";
            }
            else
            {
                if (_regimeHoldCount > 0)
                {
                    _regimeHoldCount--;
                    transitionReason = string.Format(CultureInfo.InvariantCulture, "HOLD_ACTIVE({0})", _regimeHoldCount);
                }
                else if (rawState != _regimeState)
                {
                    if (rawState == _regimeCandidateState)
                    {
                        _regimeCandidateCount++;
                    }
                    else
                    {
                        _regimeCandidateState = rawState;
                    ...
                    int needCount = Math.Max(1, RegimeConfirmConsecutiveCount);
                    if (_regimeCandidateCount >= needCount)
                    {
                        _regimeState = rawState;
                        _regimeCandidateState = rawState;
                        _regimeCandidateCount = 0;
                        _regimeHoldCount = Math.Max(0, RegimeMinHoldBarsAfterSwitch);
                        transitionReason = string.Format(
                            CultureInfo.InvariantCulture,
                            "SWITCH_CONFIRMED(raw={0},need={1},hold={2})",
                            rawState,
                            needCount,
                            _regimeHoldCount);
                    }
                ...
                else
                {
                    _regimeCandidateState = rawState;
                    _regimeCandidateCount = 0;
                    transitionReason = "RAW_EQUALS_STATE";
                }
            }

            UpdateRegimeStatusDisplay(initializeOnStart);

            if (RegimeEnableLog)
            {
                Print(
                    "REGIME_LOG | State={0} | Raw={1} | Score={2} | Structure={3} | ATR={4} | EMA={5} | UsedBars={6} | Scale={7} | ThrTrend={8} ThrRange={9} | " +
                    "ThrInt(SwingUpper={10},MinSwing={11},EqTol={12},AtrUpper={13},AtrLower={14},EmaSlope={15},Gap={16}) | " +
                    "ValInt(Swing={17},Atr={18},EmaSlope={19},Gap={20}) | StructureStreak={21} | Candidate={22}:{23} | Hold={24} | RawNote={25} | Transition={26}{27}",
                    _regimeState,
                    _regimeRawState,
                    detail.Score,
                    ...
                    detail.UsedBars,
                    _pipsScale,
                    RegimeTrendScoreThreshold,
                    RegimeRangeScoreThreshold,
                    detail.RangeSwingUpperInternalPips.ToString("F1", CultureInfo.InvariantCulture),
                    detail.MinValidSwingInternalPips.ToString("F1", CultureInfo.InvariantCulture),
                    detail.EqualToleranceInternalPips.ToString("F1", CultureInfo.InvariantCulture),
                    detail.AtrRangeUpperInternalPips.ToString("F1", CultureInfo.InvariantCulture),
                    detail.AtrTrendLowerInternalPips.ToString("F1", CultureInfo.InvariantCulture),
                    detail.EmaSlopeMinInternalPips.ToString("F1", CultureInfo.InvariantCulture),
                    detail.PriceEmaGapMinInternalPips.ToString("F1", CultureInfo.InvariantCulture),
                    double.IsNaN(detail.LatestSwingInternalPips) ? "NA" : detail.LatestSwingInternalPips.ToString("F1", CultureInfo.InvariantCulture),
                    double.IsNaN(detail.AtrInternalPips) ? "NA" : detail.AtrInternalPips.ToString("F1", CultureInfo.InvariantCulture),
                    double.IsNaN(detail.EmaSlopeInternalPips) ? "NA" : detail.EmaSlopeInternalPips.ToString("F1", CultureInfo.InvariantCulture),
                    double.IsNaN(detail.PriceEmaGapInternalPips) ? "NA" : detail.PriceEmaGapInternalPips.ToString("F1", CultureInfo.InvariantCulture),
                    detail.StructureTrendStreak,
                    _regimeCandidateState,
                    _regimeCandidateCount,
                    _regimeHoldCount,
                    string.IsNullOrWhiteSpace(detail.Note) ? "NA" : detail.Note,
                    transitionReason,
                    BuildTimeTag(utcNow));
            }
        }

        private RegimeState DetermineRegime(out RegimeJudgeDetail detail)
        {
            detail = new RegimeJudgeDetail();

            detail.RangeSwingUpperInternalPips = InputPipsToInternalPips(Math.Max(1, RegimeRangeSwingUpperPips));
            detail.MinValidSwingInternalPips = InputPipsToInternalPips(Math.Max(1, RegimeMinValidSwingPips));
            detail.EqualToleranceInternalPips = InputPipsToInternalPips(Math.Max(0, RegimeEqualTolerancePips));
            detail.AtrRangeUpperInternalPips = InputPipsToInternalPips(Math.Max(1, RegimeRangeAtrUpperPips));
            detail.AtrTrendLowerInternalPips = InputPipsToInternalPips(Math.Max(1, RegimeTrendAtrLowerPips));
            detail.EmaSlopeMinInternalPips = InputPipsToInternalPips(Math.Max(0, RegimeEmaSlopeMinChangePips));
            detail.PriceEmaGapMinInternalPips = InputPipsToInternalPips(Math.Max(0, RegimePriceEmaMinGapPips));

            switch (RegimeJudgeMode)
            {
                case 判定モード種別.レンジ固定:
                    detail.Note = "MODE_FIXED_RANGE";
                    return RegimeState.Range;
                case 判定モード種別.トレンド固定:
                    detail.Note = "MODE_FIXED_TREND";
                    return RegimeState.Trend;
                case 判定モード種別.様子見固定:
                    detail.Note = "MODE_FIXED_STANDBY";
                    return RegimeState.Standby;
            }

            Bars bars = EnsureRegimeBars();
            if (bars == null || bars.Count < 20)
            {
                detail.Note = "BARS_NOT_READY";
                return RegimeState.Standby;
            }

            int lastClosedIndex = bars.Count - 2;
            if (lastClosedIndex < 2)
            {
                detail.Note = "NOT_ENOUGH_CLOSED_BARS";
                return RegimeState.Standby;
            }

            int maxBars = Math.Max(200, RegimeMaxBars);
            int startIndex = Math.Max(1, lastClosedIndex - maxBars + 1);
            detail.UsedBars = lastClosedIndex - startIndex + 1;
            ...
            // (1) HighLow構造判定
            var pivots = HL_BuildPivots(bars, 2, maxBars);
            var filteredPivots = FilterRegimePivotsByMinSwing(pivots, detail.MinValidSwingInternalPips);
            detail.StructureTrendStreak = ComputeRegimeStructureTrendStreak(filteredPivots, detail.EqualToleranceInternalPips);

            if (detail.StructureTrendStreak >= Math.Max(1, RegimeTrendStructureNeedCount))
                structureScore += 2;

            if (filteredPivots.Count >= 2)
            {
                int last = filteredPivots.Count - 1;
                detail.LatestSwingInternalPips = PriceToInternalPips(Math.Abs(filteredPivots[last].Price - filteredPivots[last - 1].Price));
                if (detail.LatestSwingInternalPips < detail.RangeSwingUpperInternalPips)
                    structureScore -= 2;
            }
            ...
            // (2) ATR判定
            if (RegimeUseAtrJudge)
            {
                detail.AtrInternalPips = CalculateAtrInternalPips(bars, lastClosedIndex, Math.Max(2, RegimeAtrPeriod));
                if (!double.IsNaN(detail.AtrInternalPips))
                {
                    if (detail.AtrInternalPips <= detail.AtrRangeUpperInternalPips)
                        atrScore -= 2;
                    if (detail.AtrInternalPips >= detail.AtrTrendLowerInternalPips)
                        atrScore += 2;
                }
            }
            ...
            // (3) EMA判定
            if (RegimeUseEmaJudge)
            {
                int emaPeriod = Math.Max(2, RegimeEmaPeriod);
                int evalBars = Math.Max(2, RegimeEmaEvalBars);
                int emaPastIndex = lastClosedIndex - evalBars;
                if (emaPastIndex >= 0 && lastClosedIndex >= emaPeriod + 2)
                {
                    var ema = Indicators.ExponentialMovingAverage(bars.ClosePrices, emaPeriod);
                    double emaNow = ema.Result[lastClosedIndex];
                    double emaPast = ema.Result[emaPastIndex];
                    double closeNow = bars.ClosePrices[lastClosedIndex];

                    detail.EmaSlopeInternalPips = PriceToInternalPips(Math.Abs(emaNow - emaPast));
                    detail.PriceEmaGapInternalPips = PriceToInternalPips(Math.Abs(closeNow - emaNow));

                    if (detail.EmaSlopeInternalPips < detail.EmaSlopeMinInternalPips)
                        emaScore -= 1;

                    if (detail.PriceEmaGapInternalPips < detail.PriceEmaGapMinInternalPips)
                        emaScore -= 1;
                    else if (detail.PriceEmaGapMinInternalPips > 0.0)
                        emaScore += 1;
                }
            }
            ...
            detail.Score = score;
            detail.StructureScore = structureScore;
            detail.AtrScore = atrScore;
            detail.EmaScore = emaScore;

            if (score >= RegimeTrendScoreThreshold)
            {
                detail.Note = string.Format(
                    CultureInfo.InvariantCulture,
                    "SCORE_GE_TREND_THRESHOLD({0}>={1})",
                    score,
                    RegimeTrendScoreThreshold);
                return RegimeState.Trend;
            }
            if (score <= RegimeRangeScoreThreshold)
            {
                detail.Note = string.Format(
                    CultureInfo.InvariantCulture,
                    "SCORE_LE_RANGE_THRESHOLD({0}<={1})",
                    score,
                    RegimeRangeScoreThreshold);
                return RegimeState.Range;
            }
            ...
            detail.Note = string.Format(
                CultureInfo.InvariantCulture,
                "SCORE_BETWEEN_THRESHOLDS({0},trend={1},range={2})",
                score,
                RegimeTrendScoreThreshold,
                RegimeRangeScoreThreshold);
            return RegimeState.Standby;
        ...
        private List<HL_Pivot> FilterRegimePivotsByMinSwing(List<HL_Pivot> pivots, double minSwingInternalPips)
        {
            var filtered = new List<HL_Pivot>();
            if (pivots == null || pivots.Count == 0)
                return filtered;

            filtered.Add(pivots[0]);
            for (int i = 1; i < pivots.Count; i++)
            {
                var prev = filtered[filtered.Count - 1];
                var cur = pivots[i];
                double swingInternalPips = PriceToInternalPips(Math.Abs(cur.Price - prev.Price));
                if (double.IsNaN(swingInternalPips) || swingInternalPips < minSwingInternalPips)
                    continue;
        ...
        private int ComputeRegimeStructureTrendStreak(List<HL_Pivot> pivots, double toleranceInternalPips)
        {
            ...
            {
                if (pivots[i].Side > 0) highs.Add(pivots[i].Price);
                else if (pivots[i].Side < 0) lows.Add(pivots[i].Price);
            }

            if (highs.Count < 2 || lows.Count < 2)
                return 0;

            int upHigh = CountRegimeDirectionalStreak(highs, toleranceInternalPips, true);
            int upLow = CountRegimeDirectionalStreak(lows, toleranceInternalPips, true);
            int downHigh = CountRegimeDirectionalStreak(highs, toleranceInternalPips, false);
            int downLow = CountRegimeDirectionalStreak(lows, toleranceInternalPips, false);

            int up = Math.Min(upHigh, upLow);
            int down = Math.Min(downHigh, downLow);
            return Math.Max(up, down);
        }

        private int CountRegimeDirectionalStreak(List<double> values, double toleranceInternalPips, bool up)
        {
            ...
            {
                double diffInternalPips = PriceToInternalPips(values[i] - values[i - 1]);
                if (double.IsNaN(diffInternalPips))
                    break;

                bool ok = up ? (diffInternalPips > toleranceInternalPips) : (diffInternalPips < -toleranceInternalPips);
                if (!ok)
                    break;
        ...
        private double CalculateAtrInternalPips(Bars bars, int lastClosedIndex, int period)
        {
            if (bars == null || period < 2)
                return double.NaN;

            int start = lastClosedIndex - period + 1;
            if (start < 1)
                return double.NaN;
            ...
            double atrPrice = sumTr / period;
            return PriceToInternalPips(atrPrice);
        }

        private void UpdateRegimeStatusDisplay(bool force)
        {
            if (!RegimeShowStateOnChart)
            {
                if (!string.IsNullOrEmpty(_regimeLastDrawnText) || force)
                    RemoveRegimeStatusDisplay();
                return;
            }

            int fontSize = Math.Max(10, Math.Min(80, RegimeDisplayFontSize));
            int offsetX = Math.Max(0, Math.Min(500, RegimeDisplayOffsetX));
            int offsetY = Math.Max(0, Math.Min(500, RegimeDisplayOffsetY));
            string text = _regimeState == RegimeState.Trend ? "トレンド" : (_regimeState == RegimeState.Range ? "レンジ" : "様子見");

            if (!force &&
                string.Equals(_regimeLastDrawnText, text, StringComparison.Ordinal) &&
                _regimeLastDrawnFontSize == fontSize &&
                _regimeLastDrawnOffsetX == offsetX &&
                _regimeLastDrawnOffsetY == offsetY)
            {
                return;
            }

            _regimeLastDrawnText = text;
            _regimeLastDrawnFontSize = fontSize;
            _regimeLastDrawnOffsetX = offsetX;
            _regimeLastDrawnOffsetY = offsetY;
            ...
            BeginInvokeOnMainThread(() =>
            {
                try
                {
                    ...
                    Chart.RemoveObject(REGIME_STATUS_NAME_MAIN);
                    Chart.RemoveObject(REGIME_STATUS_NAME_BOLD_1);
                    Chart.RemoveObject(REGIME_STATUS_NAME_BOLD_2);

                    string mainText = BuildRegimeDisplayText(text, offsetX, offsetY, fontSize);
                    object obj = Chart.DrawStaticText(REGIME_STATUS_NAME_MAIN, mainText, VerticalAlignment.Bottom, HorizontalAlignment.Right, Color.Yellow);
                    TrySetStaticTextStyle(obj, fontSize, true);

                    // 擬似太字（DrawStaticTextの太字反映が弱い環境向け）
                    object objB1 = Chart.DrawStaticText(REGIME_STATUS_NAME_BOLD_1, BuildRegimeDisplayText(text, offsetX + 2, offsetY, fontSize), VerticalAlignment.Bottom, HorizontalAlignment.Right, Color.Yellow);
                    TrySetStaticTextStyle(objB1, fontSize, true);
                    object objB2 = Chart.DrawStaticText(REGIME_STATUS_NAME_BOLD_2, BuildRegimeDisplayText(text, offsetX, offsetY + 2, fontSize), VerticalAlignment.Bottom, HorizontalAlignment.Right, Color.Yellow);
                    TrySetStaticTextStyle(objB2, fontSize, true);
                }
                catch
                {
                    // no-op
        ...
        private string BuildRegimeDisplayText(string text, int offsetX, int offsetY, int fontSize)
        {
            string label = string.IsNullOrWhiteSpace(text) ? "様子見" : text;

            // DrawStaticTextにピクセルオフセット指定がない環境向けの近似（右下基準で左/上に寄せる）
            int spaceStep = Math.Max(1, fontSize / 3);
            int lineStep = Math.Max(1, fontSize / 2);
            int spaceCount = Math.Max(0, offsetX / spaceStep);
            int lineCount = Math.Max(0, offsetY / lineStep);

            return new string('\n', lineCount) + new string(' ', spaceCount) + label;
        }

        private void RemoveRegimeStatusDisplay()
        {
            _regimeLastDrawnText = "";
            _regimeLastDrawnFontSize = -1;
            _regimeLastDrawnOffsetX = -1;
            _regimeLastDrawnOffsetY = -1;
        ...
        private void HL_RecalculateAndProject()
        {
            ...
            int maxBars = Math.Max(200, HL_計算対象バー);
            _m5Pivots = HL_BuildPivots(Bars, HL_RightBars_M5, maxBars);

            if (_barsH1 != null && _barsH1.Count >= 10)
                _h1Pivots = HL_BuildPivots(_barsH1, HL_RightBars_H1, Math.Max(5000, maxBars));
            else
                _h1Pivots = new List<HL_Pivot>();
        ...
        private void HL_LogPerBar()
        {
            ...
            Print("HL_BAR | M5={0} | H1={1} | H1Proj={2}", m5, h1, h1p);
        }
        ...
        private string HL_GetLastPivotTypeText(List<HL_Pivot> pivots)
        {
            ...
            for (int i = 0; i < pivots.Count; i++)
            {
                ...
                if (p.Side == 1)
                {
                    lastType = prevHigh.HasValue ? (p.Price > prevHigh.Value ? "HH" : "LH") : "INIT";
                    prevHigh = p.Price;
                }
                else
                {
                    lastType = prevLow.HasValue ? (p.Price > prevLow.Value ? "HL" : "LL") : "INIT";
                    prevLow = p.Price;
                }
            }
        ...
        private List<HL_Pivot> HL_ProjectToM5(List<HL_Pivot> h1Pivots)
        {
            ...
            if (h1Pivots == null || h1Pivots.Count == 0 || Bars == null || Bars.Count <= 0)
                return projected;
            ...
            for (int i = 0; i < h1Pivots.Count; i++)
            {
                ...
                try
                {
                    m5Index = Bars.OpenTimes.GetIndexByTime(p.Time);
                }
                catch
                {
                    m5Index = -1;
                ...
                if (m5Index < 0 || m5Index >= Bars.Count)
                    continue;
        ...
        {
            if (!RegimeJudgeEnabled || Chart == null)
                return;
            ...
            try
            {
                BeginInvokeOnMainThread(() =>
                {
                    ...
                    try
                    {
                        ...
                        var m5PivotsSnapshot = _m5Pivots != null ? new List<HL_Pivot>(_m5Pivots) : null;
                        var h1ProjectedSnapshot = _h1Projected != null ? new List<HL_Pivot>(_h1Projected) : null;

                        if (m5PivotsSnapshot != null && m5PivotsSnapshot.Count >= 2)
                            drawCount += HL_DrawSeries("M5", m5PivotsSnapshot, Color.Red);

                        if (h1ProjectedSnapshot != null && h1ProjectedSnapshot.Count >= 2)
                            drawCount += HL_DrawSeries("H1", h1ProjectedSnapshot, Color.White);
        ...
        private int HL_DrawSeries(string tf, List<HL_Pivot> pivots, Color lineColor)
        {
            ...
            for (int i = 0; i < pivots.Count; i++)
            {
                ...
                if (p.Side == 1)
                {
                    ...
                    {
                        text = p.Price > prevHigh.Value ? "HH" : "LH";
                        textColor = text == "HH" ? Color.Lime : Color.Orange;
                    }
                ...
                else
                {
                    ...
                    {
                        text = p.Price > prevLow.Value ? "HL" : "LL";
                        textColor = text == "HL" ? Color.Aqua : Color.Red;
                    }
        ...
        private int HL_ForceCleanDrawingsByPrefix(out int failedCount)
        {
            ...
            for (int i = 0; i < names.Count; i++)
            {
                ...
                try
                {
                    ...
                    if (RegimeJudgeEnabled && HL_ログ毎バー出す && loggedRemovedNameCount < HL_ForceCleanLogNameLimit)
                    {
                ...
                {
                    failedCount++;
                    if (RegimeJudgeEnabled && HL_ログ毎バー出す && loggedFailedNameCount < HL_ForceCleanLogNameLimit)
                    {
