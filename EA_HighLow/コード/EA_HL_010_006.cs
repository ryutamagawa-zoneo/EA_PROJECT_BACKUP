...
namespace cAlgo
{
    ...
    public class EA_HL_010_002 : Robot
    {
        // ============================================================
        // CODE NAME (Project Constitution compliant)
        // ============================================================
        // BASE: EA_HL_010_005
        // THIS: EA_HL_010_006
        private const string CODE_NAME = "EA_HL_010_002";
...
private void EmitExecuteSendJsonl(TradeType plannedType, int barIndex, long volumeUnits, double slPipsToSend, double tpPipsToSend)
        ...
        [Parameter("レジーム判定を行う", Group = "レンジ・トレンド判定", DefaultValue = true)]
        public bool レジーム判定を行う { get; set; }
...
private DateTime? ComputeNewsUnblockUtc(DateTime utcNow)
{
    ...
    return null;
        ...
        private const string HL_DrawObjectPrefix = "HL_";
        ...
        protected override void OnStart()
        {
            ...
            _hlInstanceId = Guid.NewGuid().ToString("N").Substring(0, 8);
...
";
        ...
        private void EvaluateRegimeAndUpdateUi(DateTime utcNow, bool initializeOnStart)
        {
            if (!レジーム判定を行う)
            {
                ...
                // 判定停止時は表示も消して完全停止扱いにする
                if (!string.IsNullOrEmpty(_regimeLastDrawnText) || initializeOnStart)
                    RemoveRegimeStatusDisplay();
                return;
            }
        ...
        private void HL_RedrawPivots()
        {
            ...
            if (string.IsNullOrEmpty(_hlInstanceId))
                _hlInstanceId = Guid.NewGuid().ToString("N").Substring(0, 8);
            ...
            try
            {
                BeginInvokeOnMainThread(() =>
                {
                    try
                    {
                        // UIスレッドで最新データをスナップショット化して描画順序を安定化
                        ...
                        if (m5PivotsSnapshot != null && m5PivotsSnapshot.Count >= 2)
                            HL_DrawSeries("M5", m5PivotsSnapshot, Color.Red);

                        if (h1ProjectedSnapshot != null && h1ProjectedSnapshot.Count >= 2)
                            HL_DrawSeries("H1", h1ProjectedSnapshot, Color.White);
                    }
                    finally
                    {
        ...
        private void HL_DrawSeries(string tf, List<HL_Pivot> pivots, Color lineColor)
        {
            for (int i = 0; i < pivots.Count - 1; i++)
            {
                var a = pivots[i];
                var b = pivots[i + 1];
                string lineName = string.Format(
                    CultureInfo.InvariantCulture,
                    "HL_{0}_{1}_L_{2}_{3}_{4}",
                    _hlInstanceId,
                    tf,
                    i,
                    a.Index,
            ...
            for (int i = 0; i < pivots.Count; i++)
            {
                ...
                string textName = string.Format(CultureInfo.InvariantCulture, "HL_{0}_{1}_T_{2}_{3}", _hlInstanceId, tf, i, p.Index);
        ...
        private void HL_ClearDrawings()
        {
            ...
            _hlClearCallCount++;
            int forceCleanFrequency = Math.Max(0, HL_強制クリーン頻度);
            bool doForceClean = forceCleanFrequency > 0 &&
                                (forceCleanFrequency == 1 || (_hlClearCallCount % forceCleanFrequency) == 0);
            ...
            {
                Print(
                    "HL_CLEAR | Removed={0} Failed={1} | ForceClean={2} Freq={3} Call={4} ForceRemoved={5} ForceFailed={6}",
                    removedCount,
                    failedCount,
                    doForceClean ? "Y" : "N",
                    forceCleanFrequency,
        ...
        private int HL_ForceCleanDrawingsByPrefix(out int failedCount)
        {
            ...
            try
            {
                foreach (var obj in Chart.Objects)
                {
                    ...
                    if (!string.IsNullOrEmpty(name) && name.StartsWith(HL_DrawObjectPrefix, StringComparison.Ordinal))
                        names.Add(name);
                }
            }
            catch
