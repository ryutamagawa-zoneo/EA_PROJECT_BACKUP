...
";
        ...
        private void EvaluateRegimeAndUpdateUi(DateTime utcNow, bool initializeOnStart)
        {
            ...
            {
                Print(
                    "REGIME_LOG | State={0} | Raw={1} | Score={2} | Structure={3} | ATR={4} | EMA={5} | UsedBars={6} | ThrTrend={7} ThrRange={8} | " +
                    "ThrInt(SwingUpper={9},MinSwing={10},EqTol={11},AtrUpper={12},AtrLower={13},EmaSlope={14},Gap={15}) | " +
                    "ValInt(Swing={16},Atr={17},EmaSlope={18},Gap={19}) | StructureStreak={20} | Candidate={21}:{22} | Hold={23} | Note={24}{25}",
                    _regimeState,
                    _regimeRawState,
                    detail.Score,
        ...
        private RegimeState DetermineRegime(out RegimeJudgeDetail detail)
        {
            ...
            var filteredPivots = FilterRegimePivotsByMinSwing(pivots, detail.MinValidSwingInternalPips * Symbol.PipSize);
            detail.StructureTrendStreak = ComputeRegimeStructureTrendStreak(filteredPivots, detail.EqualToleranceInternalPips * Symbol.PipSize);
            ...
            if (filteredPivots.Count >= 2)
            {
                ...
                detail.LatestSwingInternalPips = Math.Abs(filteredPivots[last].Price - filteredPivots[last - 1].Price) / Symbol.PipSize;
            ...
            if (RegimeUseEmaJudge)
            {
                ...
                if (emaPastIndex >= 0 && lastClosedIndex >= emaPeriod + 2)
                {
                    ...
                    detail.EmaSlopeInternalPips = Math.Abs(emaNow - emaPast) / Symbol.PipSize;
                    detail.PriceEmaGapInternalPips = Math.Abs(closeNow - emaNow) / Symbol.PipSize;
        ...
        private List<HL_Pivot> FilterRegimePivotsByMinSwing(List<HL_Pivot> pivots, double minSwingPrice)
        {
            ...
            for (int i = 1; i < pivots.Count; i++)
            {
                ...
                var cur = pivots[i];
                if (Math.Abs(cur.Price - prev.Price) < minSwingPrice)
                    continue;
        ...
        private int ComputeRegimeStructureTrendStreak(List<HL_Pivot> pivots, double tolerancePrice)
        {
            ...
            int upHigh = CountRegimeDirectionalStreak(highs, tolerancePrice, true);
            int upLow = CountRegimeDirectionalStreak(lows, tolerancePrice, true);
            int downHigh = CountRegimeDirectionalStreak(highs, tolerancePrice, false);
            int downLow = CountRegimeDirectionalStreak(lows, tolerancePrice, false);
        ...
        private int CountRegimeDirectionalStreak(List<double> values, double tolerancePrice, bool up)
        {
            ...
            for (int i = values.Count - 1; i > 0; i--)
            {
                ...
                bool ok = up ? (diff > tolerancePrice) : (diff < -tolerancePrice);
                if (!ok)
                    break;
