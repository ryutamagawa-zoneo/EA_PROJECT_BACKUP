...
// ============================================================
// BASE: EA_HL_010_006
// THIS: EA_HL_DEV_010_034
// ARCHIVE_ID: 020_053 (file export marker; no runtime effect)
// REASON: Env=TRENDLESS時の方向許可を「縛りなし」から「EntryTfに従う（両方TRENDLESSのみ縛りなし）」へ変更し、逆行エントリー混入を抑止するため
// ============================================================
...
namespace cAlgo
{
    ...
    public class EA_HL_DEV_010_034 : Robot
    {
        // ============================================================
        // CODE NAME (Project Constitution compliant)
        // ============================================================
        ...
        // THIS: EA_HL_DEV_010_034
        // REASON: Env=TRENDLESS時の方向許可を「縛りなし」から「EntryTfに従う（両方TRENDLESSのみ縛りなし）」へ変更し、逆行エントリー混入を抑止するため
        private const string CODE_NAME = "EA_HL_DEV_010_034";
        private const string BOT_LABEL = "EA_HL_DEV";
...
private bool ShouldBlockNewEntry(TradeType type, int decisionBarIndex, string reasonTag)
{
    ...
    catch
    {
        ...
        private static string TrendStateGateText(TrendState state)
        {
            switch (state)
            {
                case TrendState.UpTrend: return "UP";
                case TrendState.DownTrend: return "DOWN";
                default: return "TRENDLESS";
            }
        }
        ...
        private void EmitExecuteSkipJsonl(string reason, TradeType plannedType, int barIndex, double? close1, double? ema1, double? bid, double? ask)
        {
            ...
            // Trend observation (for verification)
            var envTf = EnvironmentTimeFrame;
            var entryTf = EntryTimeFrame;
            var envTrend = GetEnvTrendState(envTf);
            var entryTrend = GetEnvTrendState(entryTf);
            root["env_tf"] = envTf.ToString();
            root["entry_tf"] = entryTf.ToString();
            root["env_trend"] = TrendStateGateText(envTrend);
            root["entry_trend"] = TrendStateGateText(entryTrend);
            root["env_trend_raw"] = envTrend.ToString();
            root["entry_trend_raw"] = entryTrend.ToString();
...
private void EmitExecuteSendJsonl(TradeType plannedType, int barIndex, long volumeUnits, double slPipsToSend, double tpPipsToSend)
        ...
        [Parameter("環境足（M1 / M5 / M15 / H1）", Group = "HLエントリー関連", DefaultValue = HL時間足.H1)]
        public HL時間足 EnvironmentTimeFrame { get; set; }

        [Parameter("エントリー足（M1 / M5 / M15 / H1）", Group = "HLエントリー関連", DefaultValue = HL時間足.M5)]
        public HL時間足 EntryTimeFrame { get; set; }
        ...
        private static int FindBarIndexAtOrAfterTime(Bars bars, DateTime time)
        {
            ...
            // binary search on OpenTimes (ascending)
            while (lo < hi)
            {
                int mid = lo + ((hi - lo) / 2);
        ...
        private static DateTime? FindFirstCrossAboveClose(Bars bars, DateTime startTime, double level)
        {
            if (bars == null || bars.Count < 3)
                return null;

            int endIndex = bars.Count - 2; // last closed
            if (endIndex < 1)
                return null;

            int startIndex = FindBarIndexAtOrAfterTime(bars, startTime);
            if (startIndex < 1)
                startIndex = 1; // need prev
            ...
            for (int i = startIndex; i <= endIndex; i++)
            {
                ...
                double curClose = bars.ClosePrices[i];

                if (prevClose <= level && curClose > level)
                    return bars.OpenTimes[i];
            }

            return null;
        }

        private static DateTime? FindFirstCrossBelowClose(Bars bars, DateTime startTime, double level)
        {
            if (bars == null || bars.Count < 3)
                return null;
        ...
        private static bool TryFindFirstPivotAfterTime(
            List<HL_Pivot> pivots,
            DateTime afterTime,
            Func<HL_Pivot, bool> predicate,
            out HL_Pivot found)
        {
            found = default(HL_Pivot);

            if (pivots == null || pivots.Count == 0 || predicate == null)
                return false;

            // pivots are expected to be time-ordered (display order)
        ...
        {
            _hlDowStateM1 = TrendState.TrendLess;
            _hlDowStateM5 = TrendState.TrendLess;
            _hlDowStateM15 = TrendState.TrendLess;
            _hlDowStateH1 = TrendState.TrendLess;

            _hlDowContextM1 = TrendLessContext.None;
            _hlDowContextM5 = TrendLessContext.None;
            _hlDowContextM15 = TrendLessContext.None;
            _hlDowContextH1 = TrendLessContext.None;

            _hlDowContextStartTimeM1 = DateTime.MinValue;
            _hlDowContextStartTimeM5 = DateTime.MinValue;
            _hlDowContextStartTimeM15 = DateTime.MinValue;
            _hlDowContextStartTimeH1 = DateTime.MinValue;

            _hlDowLastProcessedBarTimeM1 = DateTime.MinValue;
            _hlDowLastProcessedBarTimeM5 = DateTime.MinValue;
            _hlDowLastProcessedBarTimeM15 = DateTime.MinValue;
            _hlDowLastProcessedBarTimeH1 = DateTime.MinValue;

            _hlDowLastLoggedBarTimeM1 = DateTime.MinValue;
            _hlDowLastLoggedBarTimeM5 = DateTime.MinValue;
            _hlDowLastLoggedBarTimeM15 = DateTime.MinValue;
            ...
            out HL_Pivot? triggerLow)
        {
            ...
            if (pivots == null || pivots.Count == 0 || bars == null || bars.Count < 3)
                return (state = TrendState.TrendLess);

            int closedIndex = bars.Count - 2;
            if (closedIndex < 1)
                return (state = TrendState.TrendLess);

            double prevClose = bars.ClosePrices[closedIndex - 1];
            ...
            // latest pivots (display truth)
            for (int i = pivots.Count - 1; i >= 0; i--)
            {
                HL_Pivot p = pivots[i];

                if (lastHHIndex < 0 && IsPivotKind(p, "HH")) { lastHH = p; lastHHIndex = i; }
                if (lastHLIndex < 0 && IsPivotKind(p, "HL")) { lastHL = p; lastHLIndex = i; }
                if (lastLHIndex < 0 && IsPivotKind(p, "LH")) { lastLH = p; lastLHIndex = i; }
                if (lastLLIndex < 0 && IsPivotKind(p, "LL")) { lastLL = p; lastLLIndex = i; }

                if (lastHHIndex >= 0 && lastHLIndex >= 0 && lastLHIndex >= 0 && lastLLIndex >= 0)
                    break;
            }

            // KeyLH = LH just before latest LL
            if (lastLLIndex >= 0)
            {
                for (int i = lastLLIndex - 1; i >= 0; i--)
                {
                    HL_Pivot p = pivots[i];
                    if (IsPivotKind(p, "LH")) { keyLH = p; break; }
                }
            }

            // KeyHL = HL just before latest HH
            if (lastHHIndex >= 0)
            {
                for (int i = lastHHIndex - 1; i >= 0; i--)
                {
                    HL_Pivot p = pivots[i];
                    if (IsPivotKind(p, "HL")) { keyHL = p; break; }
                }
            }

            // Trigger search (Dow Theory)
            // Default (context-less):
            //   TriggerHigh = first High pivot after latest HL (HH/LH)
            //   TriggerLow  = first Low pivot  after latest LH (HL/LL)
            // Context override:
            //   AfterDownEnd: first HL after contextStartTime -> first High pivot after that = TriggerHigh
            //   AfterUpEnd  : first LH after contextStartTime -> first Low  pivot after that = TriggerLow
            HL_Pivot? candTrigHigh = null;
            if (lastHLIndex >= 0)
            {
                for (int i = lastHLIndex + 1; i < pivots.Count; i++)
                {
                    HL_Pivot p = pivots[i];
                    if (IsHighPivotKind(p)) { candTrigHigh = p; break; }
                }
            }

            HL_Pivot? candTrigLow = null;
            if (lastLHIndex >= 0)
            {
                for (int i = lastLHIndex + 1; i < pivots.Count; i++)
                {
                    HL_Pivot p = pivots[i];
                    if (IsLowPivotKind(p)) { candTrigLow = p; break; }
                }
            }

            triggerHigh = candTrigHigh;
            triggerLow = candTrigLow;

            // AfterDownEnd context override
            if (context == TrendLessContext.AfterDownEnd && contextStartTime != DateTime.MinValue)
            {
                HL_Pivot transitionHL;
                if (TryFindFirstPivotAfterTime(pivots, contextStartTime, p => IsPivotKind(p, "HL"), out transitionHL))
                {
                    HL_Pivot trigH;
                    if (TryFindFirstPivotAfterTime(pivots, transitionHL.Time, p => IsHighPivotKind(p), out trigH))
                        triggerHigh = trigH;
                }
            }

            // AfterUpEnd context override
            if (context == TrendLessContext.AfterUpEnd && contextStartTime != DateTime.MinValue)
            {
                HL_Pivot transitionLH;
                if (TryFindFirstPivotAfterTime(pivots, contextStartTime, p => IsPivotKind(p, "LH"), out transitionLH))
                {
                    HL_Pivot trigL;
                    if (TryFindFirstPivotAfterTime(pivots, transitionLH.Time, p => IsLowPivotKind(p), out trigL))
                        triggerLow = trigL;
                }
            }

            // --- Structure sanity (safety) ---
            bool structureBroken = false;

            if (lastHH.HasValue && lastHL.HasValue && lastHL.Value.Price > lastHH.Value.Price)
                structureBroken = true;

            if (lastLL.HasValue && lastLH.HasValue && lastLH.Value.Price < lastLL.Value.Price)
                structureBroken = true;
            ...
            // --- Events on this closed bar (Close-only, strict) ---
            bool downEndEventNow = keyLH.HasValue && prevClose <= keyLH.Value.Price && currentClose > keyLH.Value.Price;
            bool upEndEventNow = keyHL.HasValue && prevClose >= keyHL.Value.Price && currentClose < keyHL.Value.Price;

            // Provisional confirm (TriggerHigh/TriggerLow based)
            bool upConfirmEventNow = triggerHigh.HasValue && prevClose <= triggerHigh.Value.Price && currentClose > triggerHigh.Value.Price;
            bool downConfirmEventNow = triggerLow.HasValue && prevClose >= triggerLow.Value.Price && currentClose < triggerLow.Value.Price;

            // Hard confirmation (volatility case): break of last extreme (optional shortcut)
            bool hardUpConfirmEventNow = lastHH.HasValue && (((prevClose <= lastHH.Value.Price) && (currentClose > lastHH.Value.Price)) || (currentClose > lastHH.Value.Price));
            bool hardDownConfirmEventNow = lastLL.HasValue && (((prevClose >= lastLL.Value.Price) && (currentClose < lastLL.Value.Price)) || (currentClose < lastLL.Value.Price));

            upConfirmEventNow = upConfirmEventNow || hardUpConfirmEventNow;
            downConfirmEventNow = downConfirmEventNow || hardDownConfirmEventNow;

            // Context-confirm override (Dow strict): after HL/LH formation, confirmation is break of the previous extreme (not moving Trigger)
            // AfterDownEnd: HL formed -> Up confirmed when Close breaks above the HH that existed before that HL
            if (context == TrendLessContext.AfterDownEnd && contextStartTime != DateTime.MinValue)
            {
                HL_Pivot transitionHL;
                if (TryFindFirstPivotAfterTime(pivots, contextStartTime, p => IsPivotKind(p, "HL"), out transitionHL))
                {
                    HL_Pivot? baseHH = null;
                    for (int i = pivots.Count - 1; i >= 0; i--)
                    {
                        HL_Pivot p = pivots[i];
                        if (p.Time >= transitionHL.Time)
                            continue;
                        if (IsPivotKind(p, "HH"))
                        {
                            baseHH = p;
                    ...
                    if (baseHH.HasValue)
                    {
                        upConfirmEventNow = ((prevClose <= baseHH.Value.Price) && (currentClose > baseHH.Value.Price)) || (currentClose > baseHH.Value.Price);
                        upConfirmEventNow = upConfirmEventNow || hardUpConfirmEventNow;
                    }
                }
            }

            // AfterUpEnd: LH formed -> Down confirmed when Close breaks below the LL that existed before that LH
            if (context == TrendLessContext.AfterUpEnd && contextStartTime != DateTime.MinValue)
            {
                HL_Pivot transitionLH;
                if (TryFindFirstPivotAfterTime(pivots, contextStartTime, p => IsPivotKind(p, "LH"), out transitionLH))
                {
                    HL_Pivot? baseLL = null;
                    for (int i = pivots.Count - 1; i >= 0; i--)
                    {
                        HL_Pivot p = pivots[i];
                        if (p.Time >= transitionLH.Time)
                            continue;
                        if (IsPivotKind(p, "LL"))
                        {
                            baseLL = p;
                    ...
                    if (baseLL.HasValue)
                    {
                        downConfirmEventNow = ((prevClose >= baseLL.Value.Price) && (currentClose < baseLL.Value.Price)) || (currentClose < baseLL.Value.Price);
                        downConfirmEventNow = downConfirmEventNow || hardDownConfirmEventNow;
                    }
                }
            }
            ...
            // --- State transitions (Dow Theory) ---
            if (state == TrendState.DownTrend)
            {
                if (downEndEventNow)
                {
                    // Forced reversal: Down -> Up (skip TrendLess) when both events occur on the same closed bar
                    if (upConfirmEventNow)
                    {
            ...
            if (state == TrendState.UpTrend)
            {
                if (upEndEventNow)
                {
                    // Forced reversal: Up -> Down (skip TrendLess) when both events occur on the same closed bar
                    if (downConfirmEventNow)
                    {
            ...
            if (context == TrendLessContext.AfterDownEnd)
            {
                ...
                // remain TrendLess until Up confirmed
                return (state = TrendState.TrendLess);
            }
            ...
            // no context yet (startup/reload): infer direction from current structure
            bool canInferUp = (lastHHIndex >= 0) && keyHL.HasValue && ((lastLLIndex < 0) || (lastHHIndex > lastLLIndex)) && (currentClose >= keyHL.Value.Price);
            bool canInferDown = (lastLLIndex >= 0) && keyLH.HasValue && ((lastHHIndex < 0) || (lastLLIndex > lastHHIndex)) && (currentClose <= keyLH.Value.Price);

            if (canInferUp)
                return (state = TrendState.UpTrend);

            if (canInferDown)
                return (state = TrendState.DownTrend);

            return (state = TrendState.TrendLess);
        }

        private const string HLDOW_STATUS_NAME_H1 = CODE_NAME + ".HLDOW_STATUS_H1";
        private const string HLDOW_STATUS_NAME_M15 = CODE_NAME + ".HLDOW_STATUS_M15";
        private const string HLDOW_STATUS_NAME_M5 = CODE_NAME + ".HLDOW_STATUS_M5";
        private const string HLDOW_STATUS_NAME_M1 = CODE_NAME + ".HLDOW_STATUS_M1";

        private StackPanel _hlDowStatusPanel;
        private TextBlock _hlDowStatusTextM1;
        private TextBlock _hlDowStatusTextM5;
        private TextBlock _hlDowStatusTextM15;
        private TextBlock _hlDowStatusTextH1;

        private string _hlDowLastDrawnH1Text = "";
        private string _hlDowLastDrawnM15Text = "";
        private string _hlDowLastDrawnM5Text = "";
        private string _hlDowLastDrawnM1Text = "";

        // --- HL_DOW State Machine (Dow Theory, non-repaint pivots, RightBars=3) ---
        private enum TrendLessContext
        {
            None = 0,
            AfterDownEnd = 1, // Down終了後（TrendLess化）→ HL形成 → TriggerHighブレイクでUp確定待ち
            AfterUpEnd = 2    // Up終了後（TrendLess化）→ LH形成 → TriggerLowブレイクでDown確定待ち
        }

        private TrendState _hlDowStateM1 = TrendState.TrendLess;
        private TrendState _hlDowStateM5 = TrendState.TrendLess;
        private TrendState _hlDowStateM15 = TrendState.TrendLess;
        private TrendState _hlDowStateH1 = TrendState.TrendLess;

        private TrendLessContext _hlDowContextM1 = TrendLessContext.None;
        private TrendLessContext _hlDowContextM5 = TrendLessContext.None;
        private TrendLessContext _hlDowContextM15 = TrendLessContext.None;
        private TrendLessContext _hlDowContextH1 = TrendLessContext.None;

        private DateTime _hlDowContextStartTimeM1 = DateTime.MinValue;
        private DateTime _hlDowContextStartTimeM5 = DateTime.MinValue;
        private DateTime _hlDowContextStartTimeM15 = DateTime.MinValue;
        private DateTime _hlDowContextStartTimeH1 = DateTime.MinValue;

        private DateTime _hlDowLastProcessedBarTimeM1 = DateTime.MinValue;
        private DateTime _hlDowLastProcessedBarTimeM5 = DateTime.MinValue;
        private DateTime _hlDowLastProcessedBarTimeM15 = DateTime.MinValue;
        private DateTime _hlDowLastProcessedBarTimeH1 = DateTime.MinValue;

        private DateTime _hlDowLastLoggedBarTimeM1 = DateTime.MinValue;
        private DateTime _hlDowLastLoggedBarTimeM5 = DateTime.MinValue;
        private DateTime _hlDowLastLoggedBarTimeM15 = DateTime.MinValue;
...
private const string STOP_OVERLAY_NAME = CODE_NAME + ".EA_STOP_OVERLAY_CENTER";
...
private DateTime? ComputeNewsUnblockUtc(DateTime utcNow)
{
    ...
    return null;
        ...
        private const int HL_RightBars_M1 = 3;
        private const int HL_RightBars_M5 = 3;
        private const int HL_RightBars_M15 = 3;
        ...
        // 旧prefix削除対象（直近2世代分明示 / 運用指定）
        private const string HL_OldDrawObjectPrefix_010_033 = "EA_HL_DEV_010_033.";
        private const string HL_OldDrawObjectPrefix_010_032 = "EA_HL_DEV_010_032.";
        private static readonly string[] HL_OldDrawObjectPrefixes = new[] { HL_OldDrawObjectPrefix_010_033, HL_OldDrawObjectPrefix_010_032 };
        ...
        public struct HL_Pivot
        {
            ...
            public string Kind; // Display kind: H/L/HH/HL/LH/LL
        }

        private enum HL_FiboState
        {
            None = 0,
            Active = 1,
            Armed = 2
        ...
        private struct HL_FiboContext
        {
            public HL_FiboState State;
            public HL_PatternType Pattern;
            public HL_Direction Dir;
            public HL時間足 EnvTf;
            public HL時間足 EntryTf;

            public DateTime AnchorLowTime;
            public double AnchorLowPrice;
            public DateTime AnchorHighTime;
            public double AnchorHighPrice;

            public DateTime ArmedStartTime;
            public bool HasTriggerPivot;
            public DateTime TriggerPivotTime;
            public double TriggerPivotPrice;
            public bool BreakTriggeredThisBar;
            public bool BreakLatched;
        }

        private HL_FiboContext _hlFibo;

        private List<HL_Pivot> _m1Pivots = new List<HL_Pivot>(1024);
        private List<HL_Pivot> _m5Pivots = new List<HL_Pivot>(1024);
        private List<HL_Pivot> _m15Pivots = new List<HL_Pivot>(1024);
        private List<HL_Pivot> _h1Pivots = new List<HL_Pivot>(1024);
        private List<HL_Pivot> _h1Projected = new List<HL_Pivot>(1024);
        private readonly List<string> _hlDrawNames = new List<string>(8000);
        private Bars _barsM1;
        private Bars _barsM5;
        private Bars _barsM15Hl;
        private Bars _barsH1;
        private DateTime _lastPivotCalcBarTimeM1 = DateTime.MinValue;
        private DateTime _lastPivotCalcBarTimeM5 = DateTime.MinValue;
        private DateTime _lastPivotCalcBarTimeM15 = DateTime.MinValue;
        private DateTime _lastPivotCalcBarTimeH1 = DateTime.MinValue;
        private int _lastBarsM1Count = -1;
        private int _lastBarsM5Count = -1;
        private int _lastBarsM15Count = -1;
        private int _lastBarsH1Count = -1;
        private bool _hlRedrawPending = false;
        private int _hlClearCallCount = 0;

        public IReadOnlyList<HL_Pivot> HL_M1Pivots => _m1Pivots.AsReadOnly();
        public IReadOnlyList<HL_Pivot> HL_M5Pivots => _m5Pivots.AsReadOnly();
        public IReadOnlyList<HL_Pivot> HL_M15Pivots => _m15Pivots.AsReadOnly();
        public IReadOnlyList<HL_Pivot> HL_H1Pivots => _h1Pivots.AsReadOnly();
        public IReadOnlyList<HL_Pivot> HL_H1Projected => _h1Projected.AsReadOnly();
        ...
        protected override void OnStart()
        {
            ...
            bool isGold = sym.Contains("XAU") || sym.Contains("GOLD");
            _pipsScale = isGold ? 10 : 1; // Goldのみ036方式(×10)。XAG/SILVERは×1。
            ...
            _barsM1 = MarketData.GetBars(TimeFrame.Minute, SymbolName);
            _barsM5 = MarketData.GetBars(TimeFrame.Minute5, SymbolName);
            _barsM15Hl = MarketData.GetBars(TimeFrame.Minute15, SymbolName);
            _barsH1 = MarketData.GetBars(TimeFrame.Hour, SymbolName);
            _lastBarsM1Count = _barsM1 != null ? _barsM1.Count : -1;
            _lastBarsM5Count = _barsM5 != null ? _barsM5.Count : -1;
            _lastBarsM15Count = _barsM15Hl != null ? _barsM15Hl.Count : -1;
            _lastBarsH1Count = _barsH1 != null ? _barsH1.Count : -1;
            ResetHlDowStateMachine();
            int startupForceCleanFailed;
            int startupForceCleanRemoved = HL_ForceCleanDrawingsByPrefix(out startupForceCleanFailed);
            if (HL_LogOutput)
            {
                Print("HL_INIT | CodeName={0} | EnvTF={1} | EntryTF={2} | EqTolPips={3}",
                    CODE_NAME, EnvironmentTimeFrame, EntryTimeFrame, HL_EqualTolerancePips.ToString("F1", CultureInfo.InvariantCulture));
                Print("HL_FORCE_CLEAN_STARTUP | Removed={0} Failed={1}", startupForceCleanRemoved, startupForceCleanFailed);
            }
        ...
        protected override void OnTick()
        {
            ...
            // M1/M5/M15/H1 Barsがチャート足に依存しないよう、外部Barsの更新でHLDOWのみ評価
            TryEvaluateHlDowOnExternalBars(utcNow);
        ...
        private bool DirectionAllowsCOREModeEmaEntry(int signalIndex, double close, double ema, TradeType intended)
        {
            ...
            // 方向抑制は固定値で評価する（露出パラメータ依存を廃止）
            double enterPips = useSuppression ? 10.0 : 0.0;
            double ratio = useSuppression ? 0.6 : 0.0;
            int minHoldBars = useSuppression ? 2 : 0;
            // ------------------------------------------------------------
...
private bool PassesEma10Ema20DirectionDecision(TradeType type, int i1)
        ...
        private void ExecuteEntryWithPlannedType(TradeType plannedType, string reasonTag, double close1, double ema1, int i1, string positionComment = null)
        {
            ...
            // ============================================================
            // 最上位：方向ゲート（Envトレンド） ※全エントリー共通
            // Env=UP   : Buyのみ許可
            // Env=DOWN : Sellのみ許可
            // Env=TRENDLESS : パラメータ「環境足トレンドレス時：許可／禁止」に従う
            // EntryTf逆行フィルタ : パラメータ「環境足とエントリー足が反対なら禁止：はい／いいえ」に従う
            // ============================================================
            string denyReason;
            if (!CanEnterByEnvEntryTrend(type, out denyReason))
            {
                EmitExecuteSkipJsonl("TREND_GATE_" + denyReason, type, i1, close1, ema1, bid, ask);
                return;
            }
            ...
            PlaceTrade(type, entry, stop, tp, reasonTag, i1, positionComment);
        }
        ...
        private void PlaceTrade(TradeType type, double entry, double stop, double tpTargetPrice, string reasonTag, int decisionBarIndex, string positionComment = null)
        {
            ...
            string safePositionComment = string.IsNullOrWhiteSpace(positionComment) ? null : positionComment.Trim();
...
_entryInProgress = true;
                ...
                try
                {
                    ...
                    if (string.IsNullOrWhiteSpace(safePositionComment))
                        openResLocal = ExecuteMarketOrder(type, SymbolName, volumeInUnits, BOT_LABEL, slPipsToSend, tpPipsToSend);
                    else
                        openResLocal = ExecuteMarketOrder(type, SymbolName, volumeInUnits, BOT_LABEL, slPipsToSend, tpPipsToSend, safePositionComment);
                }
                catch (Exception)
                {
...
";
        ...
        private double PipsToInternalPips(double pips)
        {
            return pips * Math.Max(1.0, (double)_pipsScale);
        }

        private double PriceToPips(double priceDelta)
        {
            if (Symbol == null || Symbol.PipSize <= 0.0)
                return double.NaN;

            return priceDelta / Symbol.PipSize;
        }

        private double PriceToInternalPips(double priceDelta)
        {
            double pips = PriceToPips(priceDelta);
            if (double.IsNaN(pips))
                return double.NaN;

            return PipsToInternalPips(pips);
        }
        ...
        private void TryEvaluateHlDowOnExternalBars(DateTime utcNow)
        {
            bool updated = false;

            try
            {
                bool chartIsM1 = (Bars != null && Bars.TimeFrame == TimeFrame.Minute);
                bool chartIsM5 = (Bars != null && Bars.TimeFrame == TimeFrame.Minute5);
                bool chartIsM15 = (Bars != null && Bars.TimeFrame == TimeFrame.Minute15);
                bool chartIsH1 = (Bars != null && Bars.TimeFrame == TimeFrame.Hour);

                if (!chartIsM1 && _barsM1 != null && _barsM1.Count >= 2)
                {
                    int c = _barsM1.Count;
                    if (_lastBarsM1Count < 0)
                        _lastBarsM1Count = c;
                    else if (c != _lastBarsM1Count)
                    {
                        _lastBarsM1Count = c;
                        updated = true;
                    }
                ...
                if (!chartIsM5 && _barsM5 != null && _barsM5.Count >= 2)
                {
                    int c = _barsM5.Count;
                    if (_lastBarsM5Count < 0)
                        _lastBarsM5Count = c;
                    else if (c != _lastBarsM5Count)
                    {
                        _lastBarsM5Count = c;
                        updated = true;
                    }
                ...
                if (!chartIsM15 && _barsM15Hl != null && _barsM15Hl.Count >= 2)
                {
                    int c = _barsM15Hl.Count;
                    if (_lastBarsM15Count < 0)
                        _lastBarsM15Count = c;
                    else if (c != _lastBarsM15Count)
                    {
                        _lastBarsM15Count = c;
                        updated = true;
                    }
                ...
                if (!chartIsH1 && _barsH1 != null && _barsH1.Count >= 2)
                {
                    int c = _barsH1.Count;
                    if (_lastBarsH1Count < 0)
                        _lastBarsH1Count = c;
                    else if (c != _lastBarsH1Count)
                    {
                        _lastBarsH1Count = c;
                        updated = true;
                    }
        ...
        private void EvaluateSelectedJudgeAndUpdateUi(DateTime utcNow, bool initializeOnStart)
        {
            HL_RecalculateAndProject();
            EvaluateHlDowAndUpdateUi(utcNow, initializeOnStart);
        }
        ...
        private void EvaluateHlDowAndUpdateUi(DateTime utcNow, bool initializeOnStart)
        {
            ...
            bool isM5Chart = (Bars != null && Bars.TimeFrame == TimeFrame.Minute5);
            List<HL_Pivot> h1Input = isM5Chart ? (_h1Projected ?? new List<HL_Pivot>()) : (_h1Pivots ?? new List<HL_Pivot>());

            EvaluateOneTf("M1", _m1Pivots ?? new List<HL_Pivot>(), ResolveBarsForTf(HL時間足.M1), ref _hlDowStateM1, ref _hlDowContextM1, ref _hlDowContextStartTimeM1, ref _hlDowLastProcessedBarTimeM1, ref _hlDowLastLoggedBarTimeM1, utcNow);
            ...
            EvaluateOneTf("M15", _m15Pivots ?? new List<HL_Pivot>(), ResolveBarsForTf(HL時間足.M15), ref _hlDowStateM15, ref _hlDowContextM15, ref _hlDowContextStartTimeM15, ref _hlDowLastProcessedBarTimeM15, ref _hlDowLastLoggedBarTimeM15, utcNow);
            ...
            UpdateHlDowStatusDisplay(initializeOnStart, _hlDowStateM1, _hlDowStateM5, _hlDowStateM15, _hlDowStateH1);
        }
        ...
        private Bars ResolveBarsForTf(HL時間足 tf)
        {
            ...
            {
                case HL時間足.M1:
                    if ((_barsM1 == null || _barsM1.Count < 2) && Bars != null && Bars.TimeFrame == TimeFrame.Minute)
                        return Bars;
                    return _barsM1;
                case HL時間足.M5:
                    ...
                    return _barsM5;
                case HL時間足.M15:
                    if ((_barsM15Hl == null || _barsM15Hl.Count < 2) && Bars != null && Bars.TimeFrame == TimeFrame.Minute15)
                        return Bars;
                    return _barsM15Hl;
                default:
        ...
        private Bars GetBarsByHLTf(HL時間足 tf)
        {
            switch (tf)
            {
                case HL時間足.M1:
        ...
        private List<HL_Pivot> GetPivotsByHLTf(HL時間足 tf)
        {
            switch (tf)
            {
                case HL時間足.M1:
        ...
        private TrendState GetEnvTrendState(HL時間足 envTf)
        {
            switch (envTf)
            {
                case HL時間足.M1:
                    return _hlDowStateM1;
                case HL時間足.M5:
                    return _hlDowStateM5;
                case HL時間足.M15:
                    return _hlDowStateM15;
                default:
                    return _hlDowStateH1;
            }
        }

        private HL_Direction DecideDirection(TrendState trend)
        {
            if (trend == TrendState.UpTrend)
                return HL_Direction.Long;
            if (trend == TrendState.DownTrend)
                return HL_Direction.Short;
            return HL_Direction.None;
        }

        private bool CanEnterByEnvEntryTrend(TradeType plannedType, out string denyReason)
        {
            TrendState envTrend = GetEnvTrendState(EnvironmentTimeFrame);
            TrendState entryTrend = GetEnvTrendState(EntryTimeFrame);
            denyReason = "";

            // ------------------------------------------------------------
            // Env=TRENDLESS の扱い（基礎修正）
            // 1) パラメータが「禁止」なら全停止（最優先）
            // 2) 「許可」なら EntryTf の状態に従って方向を絞る
            //    Entry=UP   : Buyのみ
            //    Entry=DOWN : Sellのみ
            //    Entry=TRENDLESS : Buy/Sell両方OK
            // ------------------------------------------------------------
            if (envTrend == TrendState.TrendLess)
            {
                if (環境足トレンドレス時 == 環境足トレンドレス時_許可設定.禁止)
                {
                    denyReason = "ENV_TRENDLESS_BLOCK";
                    return false;
                }

                // 許可：EntryTfに従う
                if (entryTrend == TrendState.UpTrend)
                {
                    if (plannedType == TradeType.Sell)
                    {
                        denyReason = "ENV_TRENDLESS_ENTRY_UP_LONG_ONLY";
                        return false;
                    }
                    return true;
                }

                if (entryTrend == TrendState.DownTrend)
                {
                    if (plannedType == TradeType.Buy)
                    {
                        denyReason = "ENV_TRENDLESS_ENTRY_DOWN_SHORT_ONLY";
                        return false;
                    }
                    return true;
                }

                // EntryもTRENDLESSなら縛りなし
                return true;
            }

            if (envTrend == TrendState.UpTrend)
            {
                if (plannedType == TradeType.Sell)
                {
                    denyReason = "ENV_UP_LONG_ONLY";
                    return false;
                }
                ...
                if (entryTrend == TrendState.DownTrend)
                {
                    denyReason = "ENTRY_OPPOSITE_TO_ENV";
                    return false;
                }
            ...
            if (envTrend == TrendState.DownTrend)
            {
                if (plannedType == TradeType.Buy)
                {
                    denyReason = "ENV_DOWN_SHORT_ONLY";
                    return false;
                }
        ...
        private static bool FindLast(List<HL_Pivot> pivots, Func<HL_Pivot, bool> predicate, out HL_Pivot found)
        {
            found = default(HL_Pivot);
            if (pivots == null || predicate == null || pivots.Count == 0)
                return false;
        ...
        private static bool FindLastBefore(List<HL_Pivot> pivots, DateTime time, Func<HL_Pivot, bool> predicate, out HL_Pivot found)
        {
            found = default(HL_Pivot);
            if (pivots == null || predicate == null || pivots.Count == 0)
                return false;
        ...
        private static bool IsHigh(HL_Pivot p) => p.Side == 1;
        private static bool IsLow(HL_Pivot p) => p.Side == -1;

        private HL_FiboContext CreateEmptyFiboContext()
        {
            return new HL_FiboContext
            {
                State = HL_FiboState.None,
                Pattern = HL_PatternType.None,
                Dir = HL_Direction.None,
                EnvTf = EnvironmentTimeFrame,
                EntryTf = EntryTimeFrame,
                AnchorLowTime = DateTime.MinValue,
                AnchorLowPrice = 0.0,
                AnchorHighTime = DateTime.MinValue,
                AnchorHighPrice = 0.0,
                ArmedStartTime = DateTime.MinValue,
                HasTriggerPivot = false,
                TriggerPivotTime = DateTime.MinValue,
                TriggerPivotPrice = 0.0,
                BreakTriggeredThisBar = false,
                BreakLatched = false
            };
        }

        private void ResetFibo(string reason)
        {
            ResetFibo(reason, null, null);
        }

        private void ResetFibo(string reason, double? close, DateTime? closeBarTime)
        {
            string safeReason = string.IsNullOrWhiteSpace(reason) ? "NA" : reason.Trim();
            LogHlFiboEvent("HLFIBO|RESET|" + safeReason, close, closeBarTime, null, null, null);
            _hlFibo = CreateEmptyFiboContext();
        }

        private void SetAnchors(HL_Pivot lowPivot, HL_Pivot highPivot)
        {
            _hlFibo.AnchorLowTime = lowPivot.Time;
            _hlFibo.AnchorLowPrice = lowPivot.Price;
            _hlFibo.AnchorHighTime = highPivot.Time;
            _hlFibo.AnchorHighPrice = highPivot.Price;
        }

        private void HL_Fibo_OnBar_F2(DateTime utcNow)
        {
            Bars entryBars = GetBarsByHLTf(EntryTimeFrame);
            List<HL_Pivot> pivots = GetPivotsByHLTf(EntryTimeFrame);

            if (entryBars == null || entryBars.Count < 2 || pivots == null || pivots.Count == 0)
                return;

            int closedIndex = entryBars.Count - 2;
            if (closedIndex < 0 || closedIndex >= entryBars.Count)
                return;

            DateTime closeBarTime = entryBars.OpenTimes[closedIndex];
            double close = entryBars.ClosePrices[closedIndex];
            _hlFibo.BreakTriggeredThisBar = false;

            if (_hlFibo.State != HL_FiboState.None &&
                (_hlFibo.EnvTf != EnvironmentTimeFrame || _hlFibo.EntryTf != EntryTimeFrame))
            {
                ResetFibo("TF_CHANGED", close, closeBarTime);
            }
            ...
            _hlFibo.EnvTf = EnvironmentTimeFrame;
            _hlFibo.EntryTf = EntryTimeFrame;

            HL_Direction dir = DecideDirection(GetEnvTrendState(EnvironmentTimeFrame));
            if (dir == HL_Direction.None)
                return;

            if (_hlFibo.State != HL_FiboState.None && _hlFibo.Dir != dir)
            {
                ResetFibo("DIR_CHANGED", close, closeBarTime);
            }

            if (_hlFibo.State == HL_FiboState.None)
            {
                if (!TryCreateContinuationFibo(dir, pivots, close, closeBarTime))
                    return;
            }

            if (_hlFibo.State == HL_FiboState.Active)
                TryUpdateActiveEndpointF1(pivots, close, closeBarTime);

            double low = _hlFibo.AnchorLowPrice;
            double high = _hlFibo.AnchorHighPrice;
            double range = high - low;
            if (range <= 0.0)
            {
                ResetFibo("INVALID_RANGE", close, closeBarTime);
                return;
            }
            ...
            if (_hlFibo.Dir == HL_Direction.Long)
            {
                fib382 = high - (range * 0.382);
                fib618 = high - (range * 0.618);
                invalid618 = close < fib618;
                armReached = close <= fib382;
            }
            else
            {
            ...
            if (invalid618)
            {
                LogHlFiboEvent("HLFIBO|INVALID_618", close, closeBarTime, fib382, fib618, null);
                ResetFibo("INVALID_618", close, closeBarTime);
                return;
            }

            if (_hlFibo.State == HL_FiboState.Active && armReached)
            {
                _hlFibo.State = HL_FiboState.Armed;
                _hlFibo.ArmedStartTime = closeBarTime;
                _hlFibo.BreakLatched = false;
                LogHlFiboEvent("HLFIBO|ARMED", close, closeBarTime, fib382, fib618, null);
            }

            if (_hlFibo.State == HL_FiboState.Armed)
            {
                UpdateTriggerPivotAfterArmedF2(pivots, close, closeBarTime);
                bool breakTriggered = IsBreakoutFromTriggerPivotF2(close, closeBarTime);
                if (breakTriggered)
                    TryExecuteHlFiboBreakEntryF3(close, closeBarTime);

                HL_Pivot consumedPivot;
                if (TryConsumeArmedFiboF1(pivots, out consumedPivot))
                {
                    string note = string.Format(
                        CultureInfo.InvariantCulture,
                        "consumedPivot={0}@{1} | breakTriggeredThisBar={2}",
                        FormatFiboPriceForLog(consumedPivot.Price),
                        FormatFiboTimeForLog(consumedPivot.Time),
                        _hlFibo.BreakTriggeredThisBar ? "true" : "false");

                    LogHlFiboEvent("HLFIBO|CONSUMED", close, closeBarTime, fib382, fib618, note);
                    ResetFibo("CONSUMED", close, closeBarTime);
                }
            }
        }

        private bool TryCreateContinuationFibo(HL_Direction dir, List<HL_Pivot> pivots, double close, DateTime closeBarTime)
        {
            HL_Pivot low;
            HL_Pivot high;

            if (dir == HL_Direction.Long)
            {
                if (!FindLast(pivots, IsHigh, out high))
                    return false;
                if (!FindLastBefore(pivots, high.Time, IsLow, out low))
                    return false;
            }
            else if (dir == HL_Direction.Short)
            {
                if (!FindLast(pivots, IsLow, out low))
                    return false;
                if (!FindLastBefore(pivots, low.Time, IsHigh, out high))
                    return false;
            }
            else
            {
            ...
            _hlFibo = CreateEmptyFiboContext();
            _hlFibo.State = HL_FiboState.Active;
            _hlFibo.Pattern = HL_PatternType.Continuation;
            _hlFibo.Dir = dir;
            _hlFibo.EnvTf = EnvironmentTimeFrame;
            _hlFibo.EntryTf = EntryTimeFrame;
            SetAnchors(low, high);

            LogHlFiboEvent("HLFIBO|CONT|CREATE", close, closeBarTime, null, null, null);
            return true;
        }

        private void TryUpdateActiveEndpointF1(List<HL_Pivot> pivots, double close, DateTime closeBarTime)
        {
            if (_hlFibo.State != HL_FiboState.Active)
                return;
            ...
            {
                HL_Pivot newerHigh;
                bool hasUpdate = FindLast(
                    pivots,
                    p => IsHigh(p) && p.Time > _hlFibo.AnchorHighTime && p.Price > _hlFibo.AnchorHighPrice,
                    out newerHigh);

                if (!hasUpdate)
                    return;

                _hlFibo.AnchorHighTime = newerHigh.Time;
                _hlFibo.AnchorHighPrice = newerHigh.Price;
                LogHlFiboEvent("HLFIBO|END_UPDATE", close, closeBarTime, null, null, "updated=AnchorHigh");
                return;
            }

            if (_hlFibo.Dir == HL_Direction.Short)
            {
                HL_Pivot newerLow;
                bool hasUpdate = FindLast(
                    pivots,
                    p => IsLow(p) && p.Time > _hlFibo.AnchorLowTime && p.Price < _hlFibo.AnchorLowPrice,
                    out newerLow);

                if (!hasUpdate)
                    return;

                _hlFibo.AnchorLowTime = newerLow.Time;
                _hlFibo.AnchorLowPrice = newerLow.Price;
                LogHlFiboEvent("HLFIBO|END_UPDATE", close, closeBarTime, null, null, "updated=AnchorLow");
            }
        }

        private bool TryConsumeArmedFiboF1(List<HL_Pivot> pivots, out HL_Pivot consumedPivot)
        {
            consumedPivot = default(HL_Pivot);
            if (_hlFibo.State != HL_FiboState.Armed)
                return false;
            ...
            if (_hlFibo.Dir == HL_Direction.Long)
            {
                return FindLast(
                    pivots,
                    p => IsHigh(p) && p.Time >= _hlFibo.ArmedStartTime && p.Price > _hlFibo.AnchorHighPrice,
                    out consumedPivot);
            }
            ...
            if (_hlFibo.Dir == HL_Direction.Short)
            {
                return FindLast(
                    pivots,
                    p => IsLow(p) && p.Time >= _hlFibo.ArmedStartTime && p.Price < _hlFibo.AnchorLowPrice,
                    out consumedPivot);
            }
        ...
        private void UpdateTriggerPivotAfterArmedF2(List<HL_Pivot> pivots, double close, DateTime closeBarTime)
        {
            if (_hlFibo.State != HL_FiboState.Armed || pivots == null || pivots.Count == 0)
                return;
            ...
            if (_hlFibo.Dir == HL_Direction.Long)
            {
                hasCandidate = FindLast(
                    pivots,
                    p => IsHigh(p) && p.Time >= _hlFibo.ArmedStartTime && p.Price < _hlFibo.AnchorHighPrice,
                    out candidate);
                eventKey = "HLFIBO|TRIGGER_PIVOT_UPDATE|H1";
            }
            else if (_hlFibo.Dir == HL_Direction.Short)
            {
                hasCandidate = FindLast(
                    pivots,
                    p => IsLow(p) && p.Time >= _hlFibo.ArmedStartTime && p.Price > _hlFibo.AnchorLowPrice,
                    out candidate);
                eventKey = "HLFIBO|TRIGGER_PIVOT_UPDATE|L1";
            }
            else
            {
                return;
            ...
            bool changed = !_hlFibo.HasTriggerPivot ||
                           candidate.Time != _hlFibo.TriggerPivotTime ||
                           Math.Abs(candidate.Price - _hlFibo.TriggerPivotPrice) > double.Epsilon;
            if (!changed)
                return;

            _hlFibo.HasTriggerPivot = true;
            _hlFibo.TriggerPivotTime = candidate.Time;
            _hlFibo.TriggerPivotPrice = candidate.Price;

            string note = string.Format(
                CultureInfo.InvariantCulture,
                "armedStart={0} | triggerPivotPrice={1} | triggerPivotTime={2} | breakTriggeredThisBar={3}",
                FormatFiboTimeForLog(_hlFibo.ArmedStartTime),
                FormatFiboPriceForLog(_hlFibo.TriggerPivotPrice),
                FormatFiboTimeForLog(_hlFibo.TriggerPivotTime),
                _hlFibo.BreakTriggeredThisBar ? "true" : "false");

            LogHlFiboEvent(eventKey, close, closeBarTime, null, null, note);
        }

        private bool IsBreakoutFromTriggerPivotF2(double close, DateTime closeBarTime)
        {
            if (_hlFibo.State != HL_FiboState.Armed || !_hlFibo.HasTriggerPivot)
                return false;

            bool broken = false;
            if (_hlFibo.Dir == HL_Direction.Long)
                broken = close > _hlFibo.TriggerPivotPrice;
            else if (_hlFibo.Dir == HL_Direction.Short)
                broken = close < _hlFibo.TriggerPivotPrice;
            ...
            _hlFibo.BreakTriggeredThisBar = true;
            ...
            LogHlFiboEvent("HLFIBO|BREAK|TRIGGER", close, closeBarTime, null, null, note);
            return true;
        }

        private string BuildHlFiboEntryComment()
        {
            string patternText = FormatHlFiboPatternForComment(_hlFibo.Pattern);
            string dirText = (_hlFibo.Dir == HL_Direction.Long) ? "ロング" : (_hlFibo.Dir == HL_Direction.Short ? "ショート" : "NA");
            string envTfText = FormatHlTimeFrameForComment(_hlFibo.EnvTf);
            string entryTfText = FormatHlTimeFrameForComment(_hlFibo.EntryTf);
            return string.Format(CultureInfo.InvariantCulture, "{0}_{1}_{2}-{3}", patternText, dirText, envTfText, entryTfText);
        }

        private static string FormatHlFiboPatternForComment(HL_PatternType pattern)
        {
            if (pattern == HL_PatternType.Impulse)
                return "急変";
            if (pattern == HL_PatternType.Continuation)
                return "継続";
            return "NA";
        }

        private static string FormatHlTimeFrameForComment(HL時間足 tf)
        {
            return tf.ToString();
        }

        private void LogHlFiboEntryEvent(string status, string comment, double close, DateTime closeBarTime)
        {
            if (!HL_LogOutput)
                return;

            string safeStatus = string.IsNullOrWhiteSpace(status) ? "NA" : status.Trim();
            string safeComment = string.IsNullOrWhiteSpace(comment) ? "NA" : comment.Trim();
            string triggerText = _hlFibo.HasTriggerPivot
                ? (FormatFiboPriceForLog(_hlFibo.TriggerPivotPrice) + "@" + FormatFiboTimeForLog(_hlFibo.TriggerPivotTime))
                : "NA@NA";

            Print(
                "HLFIBO|ENTRY|{0}|{1} | envTf={2} | entryTf={3} | dir={4} | state={5} | close={6}@{7} | triggerPivot={8} | comment={9}",
                safeStatus,
                safeComment,
                _hlFibo.EnvTf,
                _hlFibo.EntryTf,
                FormatHlFiboDirectionForLog(_hlFibo.Dir),
                FormatHlFiboStateForLog(_hlFibo.State),
                FormatFiboPriceForLog(close),
                FormatFiboTimeForLog(closeBarTime),
                triggerText,
                safeComment);
        }

        private void TryExecuteHlFiboBreakEntryF3(double close, DateTime closeBarTime)
        {
            if (_hlFibo.State != HL_FiboState.Armed)
                return;

            if (_hlFibo.BreakLatched)
                return;

            string comment = BuildHlFiboEntryComment();
            bool canEnter = Positions.Count < Math.Max(1, MaxPositions);

            if (!canEnter)
            {
                LogHlFiboEntryEvent("SKIP_MAXPOS", comment, close, closeBarTime);
                _hlFibo.BreakLatched = true;
                return;
            }
            ...
            if (!CanEnterByEnvEntryTrend(type, out denyReason))
            {
                LogHlFiboEntryEvent("SKIP_TREND_GATE_" + denyReason, comment, close, closeBarTime);
                _hlFibo.BreakLatched = true;
                return;
            }

            int decisionBarIndex = (Bars != null && Bars.Count >= 2) ? (Bars.Count - 2) : 0;
            double close1 = (Bars != null && Bars.Count >= 2) ? Bars.ClosePrices[decisionBarIndex] : close;
            double ema1 = close1;
            if (_ema != null && _ema.Result != null && decisionBarIndex >= 0 && decisionBarIndex < _ema.Result.Count)
                ema1 = _ema.Result[decisionBarIndex];

            ExecuteEntryWithPlannedType(type, "HLFIBO_BREAK_TRIGGER", close1, ema1, decisionBarIndex, comment);
            _hlFibo.BreakLatched = true;
            LogHlFiboEntryEvent("EXECUTED", comment, close, closeBarTime);
        }

        private void LogHlFiboEvent(string eventKey, double? close, DateTime? closeBarTime, double? fib382, double? fib618, string note)
        {
            if (!HL_LogOutput)
                return;

            Print(
                "{0} | envTf={1} | entryTf={2} | dir={3} | state={4} | anchorLow={5}@{6} | anchorHigh={7}@{8} | close={9}@{10} | fib382={11} | fib618={12}{13}",
                string.IsNullOrWhiteSpace(eventKey) ? "HLFIBO|NA" : eventKey,
                _hlFibo.EnvTf,
                _hlFibo.EntryTf,
                FormatHlFiboDirectionForLog(_hlFibo.Dir),
                FormatHlFiboStateForLog(_hlFibo.State),
                FormatFiboPriceForLog(_hlFibo.AnchorLowPrice),
                FormatFiboTimeForLog(_hlFibo.AnchorLowTime),
                FormatFiboPriceForLog(_hlFibo.AnchorHighPrice),
                FormatFiboTimeForLog(_hlFibo.AnchorHighTime),
                close.HasValue ? FormatFiboPriceForLog(close.Value) : "NA",
                closeBarTime.HasValue ? FormatFiboTimeForLog(closeBarTime.Value) : "NA",
                fib382.HasValue ? FormatFiboPriceForLog(fib382.Value) : "NA",
                fib618.HasValue ? FormatFiboPriceForLog(fib618.Value) : "NA",
                string.IsNullOrWhiteSpace(note) ? "" : " | " + note);
        }

        private static string FormatFiboTimeForLog(DateTime time)
        {
            if (time == DateTime.MinValue)
                return "NA";

            return DateTime.SpecifyKind(time, DateTimeKind.Utc).ToString("yyyy-MM-dd HH:mm:ss", CultureInfo.InvariantCulture);
        }

        private static string FormatFiboPriceForLog(double price)
        {
            return price.ToString("F5", CultureInfo.InvariantCulture);
        }

        private static string FormatHlFiboStateForLog(HL_FiboState state)
        {
            if (state == HL_FiboState.Active)
                return "Active";
            if (state == HL_FiboState.Armed)
                return "Armed";
            return "None";
        }

        private static string FormatHlFiboDirectionForLog(HL_Direction dir)
        {
            if (dir == HL_Direction.Long)
                return "Long";
            if (dir == HL_Direction.Short)
                return "Short";
            return "None";
        }
            ...
            DateTime utcNow)
        {
            ...
            if (HL_LogOutput && barTime != lastLoggedBarTime)
            {
                ...
                PrintHlDowDecisionLog(tf, barTime, close, keyLH, keyHL, triggerHigh, triggerLow, lastHH, lastHL, lastLH, lastLL, state, utcNow);
            }
        }
        ...
        {
            Print(
                "[{0}][{1}] t={2} Close={3} KeyLH={4} KeyHL={5} TrigH={6} TrigL={7} HH={8} HL={9} LH={10} LL={11} => {12}{13}",
                CODE_NAME,
                tf,
                FormatStructureTimeForLog(barTime),
                FormatStructurePriceForLog(currentClose),
                FormatStructurePriceForLog(keyLH),
                FormatStructurePriceForLog(keyHL),
                FormatStructurePriceForLog(triggerHigh),
                FormatStructurePriceForLog(triggerLow),
                ...
                FormatHlDowTrendStateForLog(state),
                BuildTimeTag(utcNow));
        }
        ...
        private void EnsureHlDowStatusPanel()
        {
            ...
            _hlDowStatusTextM1 = new TextBlock
            {
                FontSize = 18,
            ...
            _hlDowStatusTextM15 = new TextBlock
            {
                FontSize = 18,
                Margin = new Thickness(0, 0, 0, 0)
            };
            ...
            _hlDowStatusPanel.AddChild(_hlDowStatusTextM1);
            _hlDowStatusPanel.AddChild(_hlDowStatusTextM5);
            _hlDowStatusPanel.AddChild(_hlDowStatusTextM15);
            _hlDowStatusPanel.AddChild(_hlDowStatusTextH1);

            Chart.AddControl(_hlDowStatusPanel);
        }

        private void UpdateHlDowStatusDisplay(bool force, TrendState m1State, TrendState m5State, TrendState m15State, TrendState h1State)
        {
            string m1Line = "M1 : " + FormatHlDowTrendState(m1State);
            string m5Line = "M5 : " + FormatHlDowTrendState(m5State);
            string m15Line = "M15 : " + FormatHlDowTrendState(m15State);
            string h1Line = "H1 : " + FormatHlDowTrendState(h1State);

            if (!force &&
                string.Equals(_hlDowLastDrawnM1Text, m1Line, StringComparison.Ordinal) &&
                string.Equals(_hlDowLastDrawnH1Text, h1Line, StringComparison.Ordinal) &&
                string.Equals(_hlDowLastDrawnM15Text, m15Line, StringComparison.Ordinal) &&
                string.Equals(_hlDowLastDrawnM5Text, m5Line, StringComparison.Ordinal))
            {
                return;
            }

            _hlDowLastDrawnM1Text = m1Line;
            _hlDowLastDrawnH1Text = h1Line;
            _hlDowLastDrawnM15Text = m15Line;
            ...
            BeginInvokeOnMainThread(() =>
            {
                try
                {
                    ...
                    Chart.RemoveObject(HLDOW_STATUS_NAME_M1);
                    Chart.RemoveObject(HLDOW_STATUS_NAME_H1);
                    Chart.RemoveObject(HLDOW_STATUS_NAME_M15);
                    Chart.RemoveObject(HLDOW_STATUS_NAME_M5);

                    if (_hlDowStatusTextM1 != null)
                    {
                        _hlDowStatusTextM1.Text = m1Line;
                        _hlDowStatusTextM1.ForegroundColor = ResolveHlDowStateColor(m1State);
                    }
                    ...
                    if (_hlDowStatusTextM15 != null)
                    {
                        _hlDowStatusTextM15.Text = m15Line;
                        _hlDowStatusTextM15.ForegroundColor = ResolveHlDowStateColor(m15State);
                    }
        ...
        {
            _hlDowLastDrawnM1Text = "";
            _hlDowLastDrawnH1Text = "";
            _hlDowLastDrawnM15Text = "";
            ...
            Action removeAction = () =>
            {
                try
                {
                    ...
                    _hlDowStatusTextM1 = null;
                    _hlDowStatusTextM5 = null;
                    _hlDowStatusTextM15 = null;
        ...
        private bool IsAthModeByDailyCloseBreakout(out double lastClosedDailyClose, out DateTime lastClosedDailyTimeUtc)
        {
            ...
            return lastClosedDailyClose > maxPrev;
        }
        ...
        {
            if ((_barsM1 == null || _barsM1.Count < 10) &&
                (_barsM5 == null || _barsM5.Count < 10) &&
                (_barsM15Hl == null || _barsM15Hl.Count < 10) &&
                (_barsH1 == null || _barsH1.Count < 10))
            {
                _m1Pivots = new List<HL_Pivot>();
                _m5Pivots = new List<HL_Pivot>();
                _m15Pivots = new List<HL_Pivot>();
            ...
            HL_RebuildPivotsIfNeeded(_barsM1, HL_RightBars_M1, maxBars, ref _lastPivotCalcBarTimeM1, ref _m1Pivots);
            HL_RebuildPivotsIfNeeded(_barsM5, HL_RightBars_M5, maxBars, ref _lastPivotCalcBarTimeM5, ref _m5Pivots);
            HL_RebuildPivotsIfNeeded(_barsM15Hl, HL_RightBars_M15, maxBars, ref _lastPivotCalcBarTimeM15, ref _m15Pivots);
        ...
        private void HL_LogPerBar()
        {
            string m1 = HL_FormatLastPivot(_m1Pivots);
            string m5 = HL_FormatLastPivot(_m5Pivots);
            string m15 = HL_FormatLastPivot(_m15Pivots);
            string h1 = HL_FormatLastPivot(_h1Pivots);
            string h1p = HL_FormatLastPivot(_h1Projected);
            Print("HL_BAR | M1={0} | M5={1} | M15={2} | H1={3} | H1Proj={4}", m1, m5, m15, h1, h1p);
        }
        ...
        private string HL_FormatLastPivot(List<HL_Pivot> pivots)
        {
            ...
            string type = HL_GetLastPivotTypeText(pivots);
        ...
        private string HL_GetLastPivotTypeText(List<HL_Pivot> pivots)
        {
            if (pivots == null || pivots.Count == 0)
                return "NA";

            return HL_GetPivotDisplayText(pivots[pivots.Count - 1]);
        }
        ...
        private bool HL_IsDrawObjectTargetName(string name)
        {
            ...
            // HLの描画だけを対象にする（右下ステータス等の ".REGIME_STATUS" / ".HLDOW_STATUS" は除外）
        ...
        private void HL_RedrawPivots()
        {
            ...
            try
            {
                BeginInvokeOnMainThread(() =>
                {
                    ...
                    try
                    {
                        ...
                        var selected = new HashSet<HL時間足>();
                        selected.Add(EnvironmentTimeFrame);
                        selected.Add(EntryTimeFrame);

                        if (selected.Contains(HL時間足.M1) && _m1Pivots != null && _m1Pivots.Count >= 2)
                            drawCount += HL_DrawSeries("M1", new List<HL_Pivot>(_m1Pivots), Color.Yellow);

                        if (selected.Contains(HL時間足.M5) && _m5Pivots != null && _m5Pivots.Count >= 2)
                            drawCount += HL_DrawSeries("M5", new List<HL_Pivot>(_m5Pivots), Color.Red);

                        if (selected.Contains(HL時間足.M15) && _m15Pivots != null && _m15Pivots.Count >= 2)
                            drawCount += HL_DrawSeries("M15", new List<HL_Pivot>(_m15Pivots), Color.DodgerBlue);

                        if (selected.Contains(HL時間足.H1) && _h1Pivots != null && _h1Pivots.Count >= 2)
                            drawCount += HL_DrawSeries("H1", new List<HL_Pivot>(_h1Pivots), Color.White);

                        if (HL_LogOutput)
                            Print("HL_REDRAW | DrawCount={0} | EnvTF={1} | EntryTF={2}", drawCount, EnvironmentTimeFrame, EntryTimeFrame);
                    }
                    finally
                    {
            ...
            catch
            {
                _hlRedrawPending = false;
                if (HL_LogOutput)
                    Print("HL_REDRAW_ENQUEUE_FAILED");
            }
        }
        ...
        private void HL_ClearDrawings(bool forceClean = false)
        {
            ...
            for (int i = 0; i < _hlDrawNames.Count; i++)
            {
                ...
                {
                    failedCount++;
                    if (HL_LogOutput && loggedFailedNameCount < HL_ForceCleanLogNameLimit)
                    {
        ...
        private int HL_ForceCleanDrawingsByPrefix(out int failedCount)
        {
            ...
            for (int i = 0; i < names.Count; i++)
            {
                ...
                try
                {
                    ...
                    if (HL_LogOutput && loggedRemovedNameCount < HL_ForceCleanLogNameLimit)
                    {
            ...
            if (HL_LogOutput)
                Print("HL_FORCE_CLEAN | Target={0} Removed={1} Failed={2}", names.Count, removedCount, failedCount);
