...
// ============================================================
// BASE: PIVOT_BOUNCE_M5_ALL_DAY_017_006
// THIS: PIVOT_BOUNCE_M5_ALL_DAY_017_007A
// ============================================================
...
namespace cAlgo
{
    ...
    public class HIVE_PivotBounce_v0_1_7 : Robot
    {
        ...
        // BASE: PIVOT_BOUNCE_M5_ALL_DAY_017_006
        // THIS: PIVOT_BOUNCE_M5_ALL_DAY_017_007A
        private const string CODE_NAME = "PIVOT_BOUNCE_M5_ALL_DAY_017_007A";
        private const string BOT_LABEL = "PIVOT_BOUNCE_v0_1_7A";
        // ============================================================
        ...
        [Parameter("東京セッション：新規エントリー（ON/OFF/HoldOnly）", Group = "JSTセッション別エントリー制御", DefaultValue = "ON")]
        public string TokyoSessionNewEntryMode { get; set; }

        [Parameter("欧州セッション：新規エントリー（ON/OFF/HoldOnly）", Group = "JSTセッション別エントリー制御", DefaultValue = "ON")]
        public string EuropeSessionNewEntryMode { get; set; }

        [Parameter("NYセッション：新規エントリー（ON/OFF/HoldOnly）", Group = "JSTセッション別エントリー制御", DefaultValue = "ON")]
        public string NySessionNewEntryMode { get; set; }
        ...
        // ===== JST Session (Tokyo/Europe/NY) =====
        private enum JstSession { Tokyo = 0, Europe = 1, Ny = 2 }

        private JstSession GetJstSession(DateTime jstNow)
        {
            int nowMin = jstNow.Hour * 60 + jstNow.Minute;

            // Tokyo: 09:00-15:00
            if (nowMin >= 9 * 60 && nowMin < 15 * 60)
                return JstSession.Tokyo;

            // Europe: 15:00-22:00
            if (nowMin >= 15 * 60 && nowMin < 22 * 60)
                return JstSession.Europe;

            // NY: 22:00-09:00 (wrap)
            return JstSession.Ny;
        }

        private string GetSessionNewEntryMode(JstSession session)
        {
            switch (session)
            {
                case JstSession.Tokyo: return TokyoSessionNewEntryMode;
                case JstSession.Europe: return EuropeSessionNewEntryMode;
                default: return NySessionNewEntryMode;
            }
        }

        private string NormalizeOnOffHold(string modeRaw)
        {
            if (string.IsNullOrWhiteSpace(modeRaw))
                return "ON";

            string s = modeRaw.Trim();
            s = s.ToUpperInvariant();

            if (s == "ON") return "ON";
            if (s == "OFF") return "OFF";
            if (s == "HOLDONLY" || s == "HOLD_ONLY" || s == "HOLD") return "HOLDONLY";

            // unknown -> treat as ON (minimum disruption)
            return "ON";
        }
...
";
            ...
            Print(
                ...
                "TouchExpireBars={23} | MaxTouchRange(PIPS)={24} | PivotRollover(NY)=17:00(FIXED) | StopOnInvalidPivot={25} | MaxPivotMissing={26} | MinRR={27}",
                CODE_NAME,
                BOT_LABEL,
                SymbolName,
                TradeStartTimeJst,
        ...
        protected override void OnBar()
        {
            ...
            // ===== Session-specific New Entry gate (JST) =====
            {
                DateTime jstNow = ToJst(utcNow);
                JstSession session = GetJstSession(jstNow);
                string mode = NormalizeOnOffHold(GetSessionNewEntryMode(session));
