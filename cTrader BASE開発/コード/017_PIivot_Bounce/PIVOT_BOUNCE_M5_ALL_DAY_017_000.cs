...
// ============================================================
// BASE: PIVOT_BOUNCE_M5_ALL_DAY_016_011
// THIS: PIVOT_BOUNCE_M5_ALL_DAY_017_000
// ============================================================
...
namespace cAlgo
{
    ...
    public class HIVE_PivotBounce_v0_1_7 : Robot
    {
        ...
        // BASE: PIVOT_BOUNCE_M5_ALL_DAY_016_011
        // THIS: PIVOT_BOUNCE_M5_ALL_DAY_017_000
        private const string CODE_NAME = "PIVOT_BOUNCE_M5_ALL_DAY_017_000";
        ...
        [Parameter("エントリー最大距離（PIPS）", Group = "エントリー距離・再接近制御", DefaultValue = 20.0, MinValue = 0.0)]
        public double EntryMaxDistancePips { get; set; }

        [Parameter("再接近監視バー数", Group = "エントリー距離・再接近制御", DefaultValue = 12, MinValue = 1)]
        public int ReapproachWindowBars { get; set; }

        [Parameter("再接近最大距離（PIPS）", Group = "エントリー距離・再接近制御", DefaultValue = 0.0, MinValue = 0.0)]
...
";
        ...
        // 017_000整理: 共通化（判定条件は変更しない）
        // Candle range [Low, High] must cross band [line-buffer, line+buffer]
        private bool IsCandleRangeCrossingBand(int signalIndex, double bufferPrice, PivotLine line)
        {
            ...
            return (low <= (linePrice + bufferPrice) && high >= (linePrice - bufferPrice));
        }

        // MODIFY-1 (016_010 base behavior): Touch is "proximity band cross"
        private void AddSupportTouchIf(int signalIndex, double bufferPrice, PivotLine line, ref List<PivotLine> list)
        {
            if (IsCandleRangeCrossingBand(signalIndex, bufferPrice, line))
                list.Add(line);
        }
        ...
        private void TryPivotBounceEntry_LatestTouchOnly()
        {
            ...
            if (IsSupportLine(_lastTouchedLine))
            {
                ...
                double rr;
                if (!PassesMinRrOrLog(_lastTouchedLine, TradeType.Buy, ask, _lastTouchedLinePrice - bufferPrice, tpTarget, out rr))
                    return;
            ...
            if (IsResistanceLine(_lastTouchedLine))
            {
                ...
                double rr;
                if (!PassesMinRrOrLog(_lastTouchedLine, TradeType.Sell, bid, _lastTouchedLinePrice + bufferPrice, tpTarget, out rr))
                    return;
        ...
        private bool PassesMinRrOrLog(PivotLine line, TradeType type, double entryPrice, double stopPrice, double tpTargetPrice, out double rr)
        {
            rr = 0.0;

            double minRr = Math.Max(0.0, MinRRRatio);
            if (minRr <= 0.0)
                return true;
            ...
            {
                Print(
                    "SKIP_MIN_RR | CodeName={0} | Symbol={1} | Line={2} | TradeType={3} | Entry={4} | Stop={5} | TpTarget={6} | RR={7} | MinRR={8}",
                    CODE_NAME,
                    SymbolName,
                    line.ToString(),
                    type.ToString(),
                    ...
                    rr.ToString("F3", CultureInfo.InvariantCulture),
                    minRr.ToString("F3", CultureInfo.InvariantCulture)
                );
                return false;
            }
        ...
        private bool ProcessPendingReapproachIfAny()
        {
            ...
            double rr;
            if (!PassesMinRrOrLog(_pendingLine, _pendingTradeType, currentPrice, _pendingStopPrice, _pendingTpTargetPrice, out rr))
            {
        ...
        // ============================================================
        // Min Hold gate helper (reserved)
        // ============================================================

        private bool IsMinHoldSatisfied(long posId, DateTime nowUtc)
        {
            int minHold = Math.Max(0, MinHoldMinutes);
            if (minHold <= 0)
                return true;

            if (!_metaByPosId.TryGetValue(posId, out EntryMeta meta))
                return true;

            DateTime entryUtc = meta.EntryTimeUtc;
            if (entryUtc == DateTime.MinValue)
                return true;

            double elapsedMin = (nowUtc - entryUtc).TotalMinutes;
            return elapsedMin >= minHold;
        }
