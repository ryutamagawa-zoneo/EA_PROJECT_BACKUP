...
// THIS: EMA_M5_ALL_DAY_020_074_XAUUSD_M5
...
namespace cAlgo
{
    ...
    public class EMA_M5_ALL_DAY_020_075_XAUUSD_M5 : Robot
    {
        // ============================================================
        // CODE NAME (Project Constitution compliant)
        // ============================================================
        // BASE: EMA_M5_ALL_DAY_020_047_XAUUSD_M5
        // THIS: EMA_M5_ALL_DAY_020_074_XAUUSD_M5
        private const string CODE_NAME = "EMA_M5_ALL_DAY_020_075_XAUUSD_M5";
...
private bool ShouldBlockNewEntry(TradeType type, int decisionBarIndex, string reasonTag)
{
    try
    {
        ...
        // 発注直後のクールダウン（Position反映遅延対策）
        if (_entryGuardUntilUtc != DateTime.MinValue && nowUtc < _entryGuardUntilUtc)
        {
            EmitExecuteSkipJsonl("ENTRY_COOLDOWN", type, decisionBarIndex, null, null, null, null);
            return true;
        }
    ...
    return null;
        ...
        private void TryEntryFramework(DateTime utcNow, bool useFixedExit)
        {
            ...
            if (useFixedExit)
            {
                ...
                if (ShouldBlockNewEntry(type, i1, "COREMODE_DIRECT"))
                    return;
                ...
                TradeResult resLocal = null;
                try
                {
                    EmitExecuteSendJsonl(type, i1, volumeInUnits, slInternalPips, tpInternalPips);
                    resLocal =  ExecuteMarketOrder(type, SymbolName, volumeInUnits, BOT_LABEL, slInternalPips, tpInternalPips);
                }
                catch (Exception)
                {
                    EmitExecuteSkipJsonl("EXECUTE_EXCEPTION", type, i1, null, null, null, null);
                }
                finally
                {
                    _entryInProgress = false;
                    _entryGuardUntilUtc = Server.Time.AddSeconds(3);
                }

                TradeResult res = resLocal;
