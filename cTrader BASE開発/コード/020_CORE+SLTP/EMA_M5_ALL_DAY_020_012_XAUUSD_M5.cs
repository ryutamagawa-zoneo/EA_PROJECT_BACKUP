...
// THIS: EMA_M5_ALL_DAY_020_012_XAUUSD_M5
...
namespace cAlgo
{
    ...
    public class EA_DEV_FIRST : Robot
    {
        ...
        // BASE: EMA_M5_ALL_DAY_019_048_XAUUSD_M5
        // THIS: EMA_M5_ALL_DAY_020_012_XAUUSD_M5
        private const string CODE_NAME = "EMA_M5_ALL_DAY_020_012_XAUUSD_M5";
        private const string BOT_LABEL = "EMA_M5_ALL_DAY_020_002_XAUUSD_M5";
                    ...
                    if (p.TakeProfit.HasValue && Math.Abs(p.TakeProfit.Value - tpAbs) <= (Symbol.PipSize * 0.1))
                    {
                        ...
                        if (EnableTpBoostBranchExit)
                        {
                            ...
                            if (!BranchExitOnlyOncePerPosition || !alreadyDetected)
                            {
                                ...
                                double baseTp = p.EntryPrice + (p.TradeType == TradeType.Buy ? 1.0 : -1.0) * FixedTpPips * Symbol.PipSize;
                                _tpBoostBaseTpPriceByPosId[p.Id] = baseTp;

                                _tpBoostOldTpByPosId[p.Id] = p.TakeProfit;
                                ...
                                if (EnableDebugLogJst)
                                {
                                    Print("TP_BOOST_DETECTED | CodeName={0} | PosId={1} | Stage=1 | BaseTpPrice={2} | OldTP={3} | NewTP={4}{5}",
                                        ...
                                        (p.TakeProfit.HasValue ? p.TakeProfit.Value.ToString(CultureInfo.InvariantCulture) : "null"),
...
";
        ...
        private bool ExportProReportHtml(out string savedPath)
        {
            ...
            html.AppendLine("<tr><th style=\"width:28%\">表示名</th><th style=\"width:20%\">型</th><th style=\"width:22%\">値</th><th style=\"width:30%\">内部名</th></tr>");

            if (_paramSnapshotEntries.Count == 0)
            {
                html.AppendLine("<tr><td colspan=\"4\">[ERR] パラメータースナップショットが取得できませんでした</td></tr>");
            }
            else
            {
                ...
                foreach (var p in _paramSnapshotEntries)
                {
                    ...
                    if (!string.Equals(g, lastGroup, StringComparison.Ordinal))
                    {
                        lastGroup = g;
                        html.AppendLine("<tr>"
                            + "<td colspan=\"4\" style=\"font-weight:bold;font-size:14px;background:#f0f0f0;\">"
                            + EscapeHtml(string.IsNullOrWhiteSpace(g) ? "[NO_GROUP]" : g)
                            + "</td>"
                            + "</tr>");
                    }

                    // parameter row (indented)
                    html.AppendLine("<tr>"
                        + "<td style=\"padding-left:16px;\">" + EscapeHtml(p.DisplayName) + "</td>"
            ...
            // ファイル名は「CODE_NAME + テスト期間 + パラメータスナップショット簡易ハッシュ」で一意化。
            var fromStamp = fromUtc.Value.ToString("yyyyMMdd_HHmm", CultureInfo.InvariantCulture);
            var toStamp = toUtc.Value.ToString("yyyyMMdd_HHmm", CultureInfo.InvariantCulture);

            // simple stable hash (within same run) from param snapshot json
            string ps = _paramSnapshotJson ?? "";
            int h = 17;
            unchecked
            {
            ...
            string hash8 = ((uint)h).ToString("X8", CultureInfo.InvariantCulture);

            var baseName = "PRO_RESULT_" + CODE_NAME + "_" + fromStamp + "_" + toStamp + "_" + hash8;
