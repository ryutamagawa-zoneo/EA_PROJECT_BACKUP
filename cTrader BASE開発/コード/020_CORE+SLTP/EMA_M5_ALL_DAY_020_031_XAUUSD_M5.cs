...
// THIS: EMA_M5_ALL_DAY_020_031_XAUUSD_M5
...
namespace cAlgo
{
    ...
    public class EA_DEV_FIRST : Robot
    {
        ...
        // BASE: EMA_M5_ALL_DAY_019_048_XAUUSD_M5
        // THIS: EMA_M5_ALL_DAY_020_031_XAUUSD_M5
        private const string CODE_NAME = "EMA_M5_ALL_DAY_020_031_XAUUSD_M5";
        private const string BOT_LABEL = "EMA_M5_ALL_DAY_020_031_XAUUSD_M5";
        ...
        [Parameter("建値移動トリガー（ドル）", Group = "SLイベント管理", DefaultValue = 0.0, MinValue = 0.0)]
        ...
        // - 建値移動トリガー（ドル）>0 のとき、含み益USD到達で SL を建値（＋オフセット）へ移動
        ...
        private bool RequestMoveStopLoss(Position p, double newStopLoss, string eventType, DateTime utcNow)
        {
            ...
            // 反映（SLのみ更新、TPは現状値を再送）
            // NOTE: slNorm is an absolute price level.
        ...
        private void ApplyBreakevenMoveIfNeeded()
        {
            ...
            for (int i = Positions.Count - 1; i >= 0; i--)
            {
                ...
                if (p.TradeType == TradeType.Buy)
                {
                    ...
                    double newSl = entry + (offsetPips * Symbol.PipSize);
...
";
        ...
        private double NormalizePrice(double price)
        {
            ...
            {
                // Fallback: snap to tick size
                double ts = (Symbol == null || Symbol.TickSize <= 0) ? 0.0 : Symbol.TickSize;
                if (ts <= 0.0)
                    return price;
