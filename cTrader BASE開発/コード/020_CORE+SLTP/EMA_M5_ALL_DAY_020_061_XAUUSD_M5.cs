...
// THIS: EMA_M5_ALL_DAY_020_061_XAUUSD_M5
...
namespace cAlgo
{
    ...
    public class EMA_M5_ALL_DAY_020_061_XAUUSD_M5 : Robot
    {
        // ============================================================
        // CODE NAME (Project Constitution compliant)
        // ============================================================
        // BASE: EMA_M5_ALL_DAY_020_047_XAUUSD_M5
        // THIS: EMA_M5_ALL_DAY_020_061_XAUUSD_M5
        private const string CODE_NAME = "EMA_M5_ALL_DAY_020_061_XAUUSD_M5";
        ...
        private string BuildEmaSnapshotLogPath()
        {
            ...
            // 出力先は指示により固定（普段の保管庫）
            const string fixedDir = @"D:\ChatGPT EA Development\ChatGPT EA Development\プロジェクト\保管庫";
        ...
        private static string NormalizeSignalReason(string reasonTag)
        {
            string tag = (reasonTag ?? string.Empty).Trim();
            if (string.IsNullOrEmpty(tag))
                return "NO_SIGNAL";

            string up = tag.ToUpperInvariant();
            // signal_reason はシグナル判定の理由のみ。実行段階の理由は混在させない。
            if (up.Contains("DRY_RUN") || up.Contains("TIME") || up.Contains("NEWS"))
                return "NO_SIGNAL";

            return tag;
        }

        private static string ResolveExecuteBlockReason(string reasonTag)
        {
            string tag = (reasonTag ?? string.Empty).Trim().ToUpperInvariant();
            if (string.IsNullOrEmpty(tag))
                return "OTHER";

            if (tag.Contains("DRY_RUN"))
                return "DRY_RUN";
            ...
            if (tag.Contains("DIR"))
                return "DIR_FILTER";
            if (tag.Contains("ATR"))
                return "ATR_ENV_BLOCK";
            if (tag.Contains("RISK"))
                return "RISK_BLOCK";
            if (tag.Contains("VOL"))
                return "VOLUME_BLOCK";
            ...
            if (tag.Contains("ORDER"))
                return "ORDER_SEND_FAIL";
            if (tag.Contains("SL") || tag.Contains("TP") || tag.Contains("DIST"))
                return "DIST_FILTER";
        ...
        private void InitializeEntryDecisionLogging()
        {
            ...
            try
            {
                if (!Directory.Exists(ENTRY_DECISION_FIXED_DIR))
                    Directory.CreateDirectory(ENTRY_DECISION_FIXED_DIR);
            }
            catch (Exception ex)
            {
            ...
            _entryDecisionJsonlPath = Path.Combine(ENTRY_DECISION_FIXED_DIR, "cTrader_ENTRY_DECISION_061_" + stamp + ".jsonl");
            _pendingStateJsonlPath = Path.Combine(ENTRY_DECISION_FIXED_DIR, "cTrader_PENDING_STATE_061_" + stamp + ".jsonl");
            ...
            bool reapproachConsumed)
        {
            ...
            string safeSignalReason = NormalizeSignalReason(signalReason);
            ...
            string entryJson =
                ...
                + "\"signal_allowed\":" + JsonBool(signalAllowed) + ","
                + "\"signal_reason\":\"" + JsonEsc(safeSignalReason) + "\","
        ...
        private void EmitExecuteDecision(TradeType? plannedType, bool execAllowed, string blockReason)
        {
            if (!_entryDecisionLogEnabled || _entryDecisionLogStopped || !EnableProReport)
                return;

            int i1 = (Bars != null && Bars.Count >= 2) ? (Bars.Count - 2) : -1;
            long barTimeUtcMs = SafeBarOpenTimeUtcMs(i1);
            string safeBlockReason = execAllowed ? "NONE" : ResolveExecuteBlockReason(blockReason);

            string execJson =
                "{"
                + "\"event\":\"" + (execAllowed ? "EXECUTE_SEND" : "EXECUTE_SKIP") + "\","
                + "\"bar_time_utc_ms\":" + barTimeUtcMs.ToString(CultureInfo.InvariantCulture) + ","
                + "\"plannedType\":\"" + PlannedTypeText(plannedType) + "\","
                + "\"exec_allowed\":" + JsonBool(execAllowed) + ","
                + "\"block_reason\":\"" + JsonEsc(safeBlockReason) + "\""
                + "}";

            AppendEntryDecisionJsonlLine(_entryDecisionJsonlPath, execJson, execAllowed ? "EXECUTE_SEND" : "EXECUTE_SKIP");
        }
...
[Parameter("出口：5分足クロスで終了", Group = "トレンドモード（出口）", DefaultValue = true)]
        ...
        private const string ENTRY_DECISION_FIXED_DIR = @"D:\ChatGPT EA Development\ChatGPT EA Development\プロジェクト\保管庫";
        ...
        protected override void OnBar()
        {
            ...
            void EmitGateBlockedDecision(string reasonTag)
            {
                ...
                TradeType? planned = cup ? (TradeType?)TradeType.Buy : (cdown ? (TradeType?)TradeType.Sell : null);
                bool signalAllowed = planned.HasValue;
                string signalReason = signalAllowed ? "EMA_CROSS" : "NO_SIGNAL";
                ...
                if (signalAllowed)
                    EmitExecuteDecision(planned, false, reasonTag);
            }
            ...
            {
                EmitGateBlockedDecision("MAX_POSITIONS");
                return;
            }
...
private void ApplyBreakevenMoveIfNeeded()
        ...
        private void TryEntryFramework(DateTime utcNow, bool useFixedExit)
        {
            ...
            TradeType? plannedTypeForLog = crossUp ? (TradeType?)TradeType.Buy : (crossDown ? (TradeType?)TradeType.Sell : null);
            bool signalAllowedForLog = plannedTypeForLog.HasValue;
            string signalReasonForLog = signalAllowedForLog ? "EMA_CROSS" : "NO_SIGNAL";
            EmitEntryDecisionAndPendingState(
                i1,
                i2,
                close1,
            ...
            if (useFixedExit)
            {
                ...
                if (maxSpreadInternalPips > 0.0 && spreadInternalPips > maxSpreadInternalPips)
                {
                    ...
                    EmitExecuteDecision(type, false, "SPREAD_BLOCK");
                    return;
                }
                ...
                if (_stopRequestedByRiskFailure)
                {
                    EmitExecuteDecision(type, false, "RISK_FATAL");
                    return;
                }
                ...
                if (riskAmount <= 0.0)
                {
                    EmitExecuteDecision(type, false, "RISK_INVALID");
                    return;
                }
                ...
                if (slDistancePrice <= 0.0)
                {
                    ...
                    EmitExecuteDecision(type, false, "SL_DISTANCE_INVALID");
                    return;
                }
                ...
                if (minTpInternalPips > 0.0 && tpInternalPips < minTpInternalPips)
                {
                    ...
                    EmitExecuteDecision(type, false, "TP_DISTANCE_INVALID");
                    return;
                }
                ...
                if (volumeInUnits < Symbol.VolumeInUnitsMin)
                {
                    ...
                    EmitExecuteDecision(type, false, "VOLUME_TOO_SMALL");
                    return;
                }
                ...
                if (res == null || !res.IsSuccessful || res.Position == null)
                {
                    ...
                    EmitExecuteDecision(type, false, "ORDER_SEND_FAIL");
                    return;
                }

                EmitExecuteDecision(type, true, "NONE");
        ...
        private void ExecuteEntryWithPlannedType(TradeType plannedType, string reasonTag, double close1, double ema1, int i1)
        {
            ...
            {
                EmitExecuteDecision(plannedType, false, "TICK_UNAVAILABLE");
                return;
            }
            ...
            if (!PassesSpreadFilter(bid, ask))
            {
                ...
                EmitExecuteDecision(plannedType, false, "SPREAD_BLOCK");
                return;
            }
            ...
            if (EnableEmaDirectionFilter)
            {
                ...
                {
                    EmitExecuteDecision(type, false, "DIR_FILTER");
                    return;
                }
            ...
            if (SlMode == SL方式.構造)
            {
                if (!TryGetStructureStop(type, entry, out stop, out intendedSlDistancePrice))
                {
                    ...
                    {
                        EmitExecuteDecision(type, false, "STRUCTURE_SL_BLOCK");
                        return;
                    }
                }
                else
                    ...
                    if (maxSlInternal > 0.0 && effSlPipsInternal > maxSlInternal)
                    {
                        if (EnableDebugLogJst)
                            Print("STRUCTURE_SL_BLOCK | CodeName={0} | Symbol={1} | Type={2} | Action=NO_ENTRY | Reason=MAX_SL_EXCEEDED | EffSLpips={3} MaxSLpips={4}{5}",
                                ...
                                BuildTimeTag(UtcNow()));
                        EmitExecuteDecision(type, false, "MAX_SL_EXCEEDED");
                        return;
                    }
                }
            }
            ...
            if (useTpEntry)
            {
                ...
                if (effectiveMinRr > 0.0 && expRr + 1e-12 < effectiveMinRr)
                {
                    ...
                    EmitExecuteDecision(type, false, "RR_FILTER_BLOCK");
                    return;
                }
            }
        ...
        private void TryEmaEntry()
        {
            ...
            void EmitAndReturn(string finalReason, bool signalAllowed)
            {
                EmitEntryDecisionAndPendingState(
                    i1,
                    i2,
        ...
        {
            void EmitExecSkip(string reason) => EmitExecuteDecision(type, false, reason);
            void EmitExecSend() => EmitExecuteDecision(type, true, "NONE");

            // Compliance gate：新規注文（保留注文含む発行直前）で必ず参照
            if (!TradePermission)
            {
                EmitExecSkip("TRADE_PERMISSION_BLOCK");
                return;
            }
            ...
            if (EnableAtrEnvGate)
            {
                ...
                if (AtrEnvMinPips > 0.0 && atrInputPips > 0.0 && atrInputPips < AtrEnvMinPips)
                {
                    ...
                    EmitExecSkip("ATR_ENV_GATE_BLOCK");
                    return;
                }
            ...
            if (!IsNewEntryAllowed(utcNow, out string newsReason))
            {
                ...
                EmitExecSkip("NEWS_BLOCK");
                return;
            }

            if (_stopRequestedByRiskFailure)
            {
                EmitExecSkip("RISK_FATAL");
                return;
            }
            ...
            {
                EmitExecSkip("SL_DISTANCE_INVALID");
                return;
            }
            ...
            if (useTp)
            {
                ...
                {
                    EmitExecSkip("TP_DISTANCE_INVALID");
                    return;
                }
            }
