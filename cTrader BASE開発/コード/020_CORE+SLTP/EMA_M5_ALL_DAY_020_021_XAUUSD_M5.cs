...
// THIS: EMA_M5_ALL_DAY_020_021_XAUUSD_M5
...
namespace cAlgo
{
    ...
    public class EA_DEV_FIRST : Robot
    {
        ...
        // BASE: EMA_M5_ALL_DAY_019_048_XAUUSD_M5
        // THIS: EMA_M5_ALL_DAY_020_021_XAUUSD_M5
        private const string CODE_NAME = "EMA_M5_ALL_DAY_020_021_XAUUSD_M5";
        private const string BOT_LABEL = "EMA_M5_ALL_DAY_020_021_XAUUSD_M5";
...
// ============================================================
// 方向制限（連敗時 BUY_ONLY / SELL_ONLY） 020_019
...
[Parameter("方向制限ON（買いのみ/売りのみ）", Group = "方向制限（連敗時）", DefaultValue = true)]
public bool EnableDirectionRestriction { get; set; }

[Parameter("方向制限 発動連敗数", Group = "方向制限（連敗時）", DefaultValue = 3, MinValue = 1)]
public int DirRestrictTriggerLossStreak { get; set; }

[Parameter("方向制限 保持トレード数", Group = "方向制限（連敗時）", DefaultValue = 3, MinValue = 1)]
public int DirRestrictHoldTrades { get; set; }

[Parameter("方向制限 Bias72閾値(±)", Group = "方向制限（連敗時）", DefaultValue = 0.6, MinValue = 0.0, MaxValue = 1.0)]
public double DirRestrictBiasThreshold { get; set; }

[Parameter("方向制限 EMA20傾き閾値(pips/6bars)", Group = "方向制限（連敗時）", DefaultValue = 0.2, MinValue = 0.0)]
public double DirRestrictEmaSlopeThresholdPips { get; set; }

[Parameter("Bias72 計算本数", Group = "方向制限（連敗時）", DefaultValue = 72, MinValue = 10, MaxValue = 500)]
public int DirRestrictBiasLookbackBars { get; set; }

[Parameter("EMA傾き 差分本数", Group = "方向制限（連敗時）", DefaultValue = 6, MinValue = 1, MaxValue = 50)]
public int DirRestrictEmaSlopeBars { get; set; }
        ...
        [Parameter("EMAモードでCORE入口再現を有効化", Group = "CORE制御・エントリー", DefaultValue = false)]
        public bool EnableEmaCOREEntryReplay { get; set; }
        ...
        private const double _CORE_EntryMaxDistancePips = 50.0;
        private const int _CORE_ReapproachWindowBars = 36;
        private const double _CORE_ReapproachMaxDistancePips = 40.0;
            ...
            for (int i = Positions.Count - 1; i >= 0; i--)
            {
                ...
                if (EnableDebugLogJst)
                {
                    TryPrintOncePerBar(p.Id, "BE_CHECK",
                        string.Format(CultureInfo.InvariantCulture,
                            "BE_CHECK | CodeName={0} | Symbol={1} | PosId={2} | Type={3} | UnrealizedUSD={4} | TriggerUSD={5} | Reached={6} | Entry={7} | SL={8} | TP={9}{10}",
                            CODE_NAME,
                            SymbolName,
                            p.Id,
                            p.TradeType,
                ...
                if (p.TradeType == TradeType.Buy)
                {
                    ...
                    bool modified = TryModifyPositionOncePerBar(p, entry, p.TakeProfit, "BREAKEVEN");
                    if (EnableDebugLogJst)
                    {
                        TryPrintOncePerBar(p.Id, modified ? "BE_APPLIED" : "BE_SKIPPED",
                            string.Format(CultureInfo.InvariantCulture,
                                modified
        ...
        private bool DirectionAllowsCOREModeEmaEntry(int signalIndex, double close, double ema, TradeType intended)
        {
            ...
            double enterPips = EnableCOREMode ? Math.Max(0.0, _CORE_DirectionDeadzonePips) : Math.Max(0.0, DirectionDeadzonePips);
            double ratio = EnableCOREMode ? Math.Max(0.0, _CORE_DirectionHysteresisExitEnterRatio) : Math.Max(0.0, DirectionHysteresisExitEnterRatio);
            int minHoldBars = EnableCOREMode ? Math.Max(0, _CORE_DirectionStateMinHoldBars) : Math.Max(0, DirectionStateMinHoldBars);
...
TradeResult res = ExecuteMarketOrder(type, SymbolName, volumeInUnits, BOT_LABEL, slInternalPips, tpInternalPips);
        ...
        private double GetFixedSlDistancePrice()
        {
            double pips = Math.Max(0.0, FixedSLPips);
            return UserPipsToPrice(pips);
        }

        private double GetAtrSlDistancePrice()
        {
            double atr = (_atrMinSl != null && _atrMinSl.Result != null && _atrMinSl.Result.Count > 0)
                ? _atrMinSl.Result.LastValue
                : 0.0;

            return Math.Max(0.0, MinSlAtrMult) * atr;
        }

        private double ClampSlDistancePrice(double distPrice)
        {
            double d = Math.Max(0.0, distPrice);

            // 下限（あなたの意図の「最小SL」）
            double floor = GetMinSlFloorPrice();
            if (floor > 0.0) d = Math.Max(d, floor);

            // 上限（最大SL）
            double cap = GetMaxSlCapPrice();
            if (cap > 0.0) d = Math.Min(d, cap);
        ...
        private void PlaceTrade(TradeType type, double entry, double stop, double tpTargetPrice, string reasonTag)
        {
            ...
            for (int attempt = 0; attempt < maxAttempts; attempt++)
            {
                ...
                if (EnableCOREMode)
                {
                    // COREMODE本体は不変。探索モード時のみSL/TPのON/OFFを反映。
...
private bool TryApplyDirectionRestriction(TradeType plannedType, DateTime utcNow, int barIndex, string contextTag, out string blockReason)
{
    ...
    if (_dirMode == DirectionRestrictionMode.Normal || _dirModeRemainingTrades <= 0)
    {
        ...
        if (bias >= biasThr && emaSlopePips >= slopeThr)
        {
            ...
            Print("DIRMODE_ON | Mode=BUY_ONLY | LossStreak={0} | Bias72={1:F3} NH={2} NL={3} | EmaSlope(pips/{4}bars)={5:F3} | Ctx={6}{7}",
                _lossStreak, bias, nh, nl, slopeBars, emaSlopePips, contextTag, BuildTimeTag(utcNow));
        }
        else if (bias <= -biasThr && emaSlopePips <= -slopeThr)
        {
            _dirMode = DirectionRestrictionMode.SellOnly;
            _dirModeRemainingTrades = DirRestrictHoldTrades;
            Print("DIRMODE_ON | Mode=SELL_ONLY | LossStreak={0} | Bias72={1:F3} NH={2} NL={3} | EmaSlope(pips/{4}bars)={5:F3} | Ctx={6}{7}",
                _lossStreak, bias, nh, nl, slopeBars, emaSlopePips, contextTag, BuildTimeTag(utcNow));
        }
        // else: remain Normal
    }
