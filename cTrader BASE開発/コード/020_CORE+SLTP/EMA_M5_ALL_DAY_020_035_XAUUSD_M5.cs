...
// THIS: EMA_M5_ALL_DAY_020_035_XAUUSD_M5
...
namespace cAlgo
{
    ...
    public class EA_DEV_FIRST : Robot
    {
        ...
        // BASE: EMA_M5_ALL_DAY_019_048_XAUUSD_M5
        // THIS: EMA_M5_ALL_DAY_020_035_XAUUSD_M5
        private const string CODE_NAME = "EMA_M5_ALL_DAY_020_035_XAUUSD_M5";
        private const string BOT_LABEL = "EMA_M5_ALL_DAY_020_035_XAUUSD_M5";
...
[Parameter("出口：5分足クロスで終了", Group = "トレンドモード（出口）", DefaultValue = true)]
        ...
        private void EvaluateTrendModeOnM5Bar(DateTime utcNow)
        {
            ...
            if (exit)
            {
                ...
                Print("TREND_OFF | CodeName={0} | Reason={1}{2}", CODE_NAME, reason, BuildTimeTag(utcNow));
            }
        }
        ...
        private bool CheckCloseTouchExit(List<TimeFrame> exitTfs, int period, int trendDir)
        {
            ...
            // LONG: prev Close > SMA && now Close <= SMA
            // SHORT: prev Close < SMA && now Close >= SMA
            foreach (var tf in exitTfs)
            {
                ...
                if (trendDir > 0)
                {
                    if (closePrev > smaPrev && closeNow <= smaNow)
                        return true;
                }
                else if (trendDir < 0)
                {
