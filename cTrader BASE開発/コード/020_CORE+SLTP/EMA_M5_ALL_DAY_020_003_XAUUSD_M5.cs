...
// THIS: EMA_M5_ALL_DAY_020_003_XAUUSD_M5
...
namespace cAlgo
{
    ...
    public class EA_DEV_FIRST : Robot
    {
        ...
        // BASE: EMA_M5_ALL_DAY_019_048_XAUUSD_M5
        // THIS: EMA_M5_ALL_DAY_020_003_XAUUSD_M5
        private const string CODE_NAME = "EMA_M5_ALL_DAY_020_003_XAUUSD_M5";
        private const string BOT_LABEL = "EMA_M5_ALL_DAY_020_003_XAUUSD_M5";
...
void ApplyEmergencyCloseIfNeeded()
        ...
        private void ApplyBreakevenMoveIfNeeded()
        {
            ...
            for (int i = Positions.Count - 1; i >= 0; i--)
            {
                ...
                bool reached = p.NetProfit >= trigger;

                // Debug: emit BE_CHECK only when reached (to keep logs readable)
                if (EnableDebugLogJst && reached)
                {
                    Print("BE_CHECK | CodeName={0} | Label={1} | Symbol={2} | Type={3} | PositionId={4} | NetProfitUSD={5} | TriggerUSD={6} | Entry={7} | SL={8} | TP={9}{10}",
                        CODE_NAME, BOT_LABEL, SymbolName, p.TradeType, p.Id,
                        p.NetProfit.ToString("F2", CultureInfo.InvariantCulture),
                        trigger.ToString("F2", CultureInfo.InvariantCulture),
                        p.EntryPrice.ToString("F3", CultureInfo.InvariantCulture),
                        p.StopLoss.HasValue ? p.StopLoss.Value.ToString("F3", CultureInfo.InvariantCulture) : "null",
                        p.TakeProfit.HasValue ? p.TakeProfit.Value.ToString("F3", CultureInfo.InvariantCulture) : "null",
                        BuildTimeTag(utcNow));
                }
                ...
                if (p.TradeType == TradeType.Buy)
                {
                    if (p.StopLoss.HasValue && p.StopLoss.Value >= entry)
                    {
                        ...
                        {
                            Print("BE_SKIPPED | CodeName={0} | Label={1} | Symbol={2} | Type={3} | PositionId={4} | Reason={5} | Entry={6} | SL={7}{8}",
                                CODE_NAME, BOT_LABEL, SymbolName, p.TradeType, p.Id,
                                "AlreadyBE",
                                entry.ToString("F3", CultureInfo.InvariantCulture),
                                p.StopLoss.Value.ToString("F3", CultureInfo.InvariantCulture),
                                BuildTimeTag(utcNow));
                        }
                        continue;
                    }
                    ...
                    {
                        Print(modified
                                ? "BE_APPLIED | CodeName={0} | Label={1} | Symbol={2} | Type={3} | PositionId={4} | NewSL={5} | NetProfitUSD={6} | TriggerUSD={7}{8}"
                                : "BE_SKIPPED | CodeName={0} | Label={1} | Symbol={2} | Type={3} | PositionId={4} | Reason={5} | NetProfitUSD={6} | TriggerUSD={7}{8}",
                            CODE_NAME, BOT_LABEL, SymbolName, p.TradeType, p.Id,
                            modified ? entry.ToString("F3", CultureInfo.InvariantCulture) : "ModifyRejectedOrGuard",
                            p.NetProfit.ToString("F2", CultureInfo.InvariantCulture),
                            trigger.ToString("F2", CultureInfo.InvariantCulture),
                            BuildTimeTag(utcNow));
                    }
