...
// ============================================================
// BASE: EMA_M5_ALL_DAY_019_048_XAUUSD_M5
// THIS: EMA_M5_ALL_DAY_020_034_XAUUSD_M5
...
namespace cAlgo
{
    ...
    public class EA_DEV_FIRST : Robot
    {
        ...
        // BASE: EMA_M5_ALL_DAY_019_048_XAUUSD_M5
        // THIS: EMA_M5_ALL_DAY_020_034_XAUUSD_M5
        private const string CODE_NAME = "EMA_M5_ALL_DAY_020_034_XAUUSD_M5";
        private const string BOT_LABEL = "EMA_M5_ALL_DAY_020_034_XAUUSD_M5";
        ...
        [Parameter("トレンド中：BEを使う", Group = "トレンドモード（BE）", DefaultValue = false)]
        public bool TrendEnableBreakeven { get; set; }

        [Parameter("BE発動：最短足序列 連続成立本数N", Group = "トレンドモード（BE）", DefaultValue = 2, MinValue = 1)]
        public int TrendBeConfirmBars { get; set; }
        ...
        private int _trendBeConsecutiveCount = 0;
        private bool _trendBeApplied = false;
        ...
        private void EvaluateTrendModeOnM5Bar(DateTime utcNow)
        {
            ...
            if (!_trendActive)
            {
                if (longOk)
                {
                    _trendActive = true;
                    _trendDir = +1;
                    _trendBeConsecutiveCount = 0;
                    _trendBeApplied = false;
                    _trendTpRemovedPosIds.Clear();
                    Print("TREND_ON | CodeName={0} | Dir=LONG{1}", CODE_NAME, BuildTimeTag(utcNow));
                }
                else if (shortOk)
                {
                    _trendActive = true;
                    ...
                    _trendBeApplied = false;
                    _trendTpRemovedPosIds.Clear();
                    Print("TREND_ON | CodeName={0} | Dir=SHORT{1}", CODE_NAME, BuildTimeTag(utcNow));
                }
                return;
            }

            // If trend is active, update BE consecutive confirmation based on entry ordering.
            bool stillOk = (_trendDir > 0) ? longOk : shortOk;
            if (stillOk)
                _trendBeConsecutiveCount++;
            else
                _trendBeConsecutiveCount = 0;

            // Exit evaluation (SMA cross) based on exit TF selection
            ...
            if (exitTfs.Count >= 2)
            {
                bool exit = CheckSmaCrossExit(exitTfs, period, _trendDir);
                if (exit)
                {
                    ...
                    _trendBeApplied = false;
                    _trendTpRemovedPosIds.Clear();
                    Print("TREND_OFF | CodeName={0} | Reason=EXIT_CROSS{1}", CODE_NAME, BuildTimeTag(utcNow));
                }
            }
        }
        ...
        private void ApplyTrendModeManagementOnTick(DateTime utcNow)
        {
            ...
            // Trend BE (one-time) - based on consecutive confirmations on shortest entry TF
            if (TrendEnableBreakeven && !_trendBeApplied)
            {
                int n = Math.Max(1, TrendBeConfirmBars);
                if (_trendBeConsecutiveCount >= n)
                {
                    ApplyTrendBreakevenToBotPositions(utcNow);
                    _trendBeApplied = true;
                    Print("TREND_BE_APPLIED | CodeName={0} | N={1}{2}", CODE_NAME, n, BuildTimeTag(utcNow));
                }
            }
        }
        ...
        private void RemoveTakeProfitForBotPositionsIfNeeded(DateTime utcNow)
        {
            for (int i = Positions.Count - 1; i >= 0; i--)
            {
                ...
                var res = ModifyPosition(p, p.StopLoss, null);
                if (res != null && res.IsSuccessful)
                {
                    _trendTpRemovedPosIds.Add(p.Id);
        ...
        private void ApplyTrendBreakevenToBotPositions(DateTime utcNow)
        {
            for (int i = Positions.Count - 1; i >= 0; i--)
            {
                Position p = Positions[i];
                ...
                // Move SL to entry price (break-even). Do not require TP.
                if (!p.StopLoss.HasValue)
                    continue;

                double newSl = NormalizePrice(p.EntryPrice);

                // If already at BE (or beyond), skip
                if (Math.Abs(p.StopLoss.Value - newSl) <= Symbol.TickSize * 0.5)
                    continue;

                var res = ModifyPosition(p, newSl, p.TakeProfit);
                if (res != null && res.IsSuccessful)
                {
                    Print("TREND_BE_MOVE | CodeName={0} | PosId={1} | NewSL={2}{3}",
                        CODE_NAME, p.Id, newSl.ToString("F5", CultureInfo.InvariantCulture), BuildTimeTag(utcNow));
                }
            }
        }
