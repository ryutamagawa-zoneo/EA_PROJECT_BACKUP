...
// ============================================================
// BASE: EMA_M5_ALL_DAY_018_029_XAUUSD_M5
// THIS: EMA_M5_ALL_DAY_018_030_XAUUSD_M5
// ARCHIVE_ID: 030 (file export marker; no runtime effect) (file export marker; no runtime effect)
// ============================================================
...
namespace cAlgo
{
    ...
    public class EA_DEV_FIRST : Robot
    {
        ...
        // BASE: EMA_M5_ALL_DAY_018_029_XAUUSD_M5
        // THIS: EMA_M5_ALL_DAY_018_030_XAUUSD_M5
        private const string CODE_NAME = "EMA_M5_ALL_DAY_018_030_XAUUSD_M5";
        private const string BOT_LABEL = "EMA_M5_ALL_DAY_018_030_XAUUSD_M5";
        ...
        // 最小保有時間（001MODE用）
        // ============================================================
        private class PendingProtection
        {
        ...
        private readonly Dictionary<long, PendingProtection> _pendingProtectionByPosId = new Dictionary<long, PendingProtection>();
        ...
        private bool IsMinHoldActive(Position p)
        {
            ...
            PendingProtection pp;
            if (_pendingProtectionByPosId.TryGetValue(p.Id, out pp))
            {
                return UtcNow() < pp.ReleaseUtc;
            }

            return false;
        }

        private void ApplyMinHoldReleaseIfNeeded()
        {
            if (!Enable001Mode)
                return;
            ...
            for (int i = Positions.Count - 1; i >= 0; i--)
            {
                ...
                if (now < pp.ReleaseUtc)
                    continue;

                // If protection already set, just clear pending state.
                bool slOk = p.StopLoss.HasValue && p.StopLoss.Value > 0.0;
                bool tpOk = p.TakeProfit.HasValue && p.TakeProfit.Value > 0.0;

                if (slOk && tpOk)
                {
                    _pendingProtectionByPosId.Remove(p.Id);
                    if (EnableDebugLogJst)
                        Print("MIN_HOLD_RELEASE | CodeName={0} | PosId={1} | AlreadySet=true{2}", CODE_NAME, p.Id, BuildTimeTag(now));
                    continue;
                }

                // Try to set SL/TP now.
                var modRes = ModifyPosition(p, pp.SlPrice, pp.TpPrice, ProtectionType.Absolute);
                if (modRes != null && modRes.IsSuccessful)
                {
                    _pendingProtectionByPosId.Remove(p.Id);
                    Print("MIN_HOLD_RELEASE | CodeName={0} | PosId={1} | SetSLTP=true{2}", CODE_NAME, p.Id, BuildTimeTag(now));
                }
                else
                {
                    // Fail-safe: if we cannot set protection after hold, request emergency close.
                    _emergencyCloseRequested.Add(p.Id);
                    _closeInitiatorByPosId[p.Id] = "MIN_HOLD_SET_FAILED";
                    Print("MIN_HOLD_SET_FAILED | CodeName={0} | PosId={1}{2}", CODE_NAME, p.Id, BuildTimeTag(now));
                }
            }
        }
        ...
        private void TryEmaEntry()
        {
            ...
            {
                // Regime entry: price above/below EMA on the last closed bar
                if (close1 > ema1)
            ...
            PlaceTrade(type, entry, stop, tp, useCrossEntry ? "EMA_CROSS" : "EMA_REGIME");
        }
        ...
        private void PlaceTrade(TradeType type, double entry, double stop, double tpTargetPrice, string reasonTag)
        {
            ...
            for (int attempt = 0; attempt < maxAttempts; attempt++)
            {
                ...
                if (Enable001Mode)
                {
                    ...
                    if (minHold > 0)
                    {
                        // 最小保有時間中はSL/TPを付けずに建て、経過後に付与する。
                        openRes = ExecuteMarketOrder(type, SymbolName, volumeInUnits, BOT_LABEL);
                    }
                    else
                    {
                        openRes = ExecuteMarketOrder(type, SymbolName, volumeInUnits, BOT_LABEL, effectiveSlPips, tpPipsFromTarget);
                    }
                }
                else
                    openRes = ExecuteMarketOrder(type, SymbolName, volumeInUnits, BOT_LABEL);
                ...
                if (Enable001Mode)
                {
                    ...
                    if (minHold > 0)
                    {
                        _pendingProtectionByPosId[pos.Id] = new PendingProtection
                        {
                            SlPrice = slPrice,
                            TpPrice = tpPrice,
                            ReleaseUtc = UtcNow().AddMinutes(minHold)
                        };

                        Print("MIN_HOLD_ACTIVE | CodeName={0} | PosId={1} | ReleaseInMin={2}{3}", CODE_NAME, pos.Id, minHold, BuildTimeTag(UtcNow()));
                    }
                }
                ...
                if (!Enable001Mode)
                {
                    modRes = ModifyPosition(pos, slPrice, tpPrice, ProtectionType.Absolute);
                }

                bool minHoldActive = Enable001Mode && Math.Max(0, MinHoldMinutes) > 0;
                bool slOk = (pos.StopLoss.HasValue && pos.StopLoss.Value > 0.0) || minHoldActive;
                if (!slOk)
                {
