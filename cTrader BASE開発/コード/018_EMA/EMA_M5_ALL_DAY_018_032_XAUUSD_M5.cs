...
// ============================================================
// BASE: EMA_M5_ALL_DAY_018_031_XAUUSD_M5
// THIS: EMA_M5_ALL_DAY_018_032_XAUUSD_M5
// ARCHIVE_ID: 032 (file export marker; no runtime effect) (file export marker; no runtime effect)
// ============================================================
...
namespace cAlgo
{
    ...
    public class EA_DEV_FIRST : Robot
    {
        ...
        // BASE: EMA_M5_ALL_DAY_018_031_XAUUSD_M5
        // THIS: EMA_M5_ALL_DAY_018_032_XAUUSD_M5
        private const string CODE_NAME = "EMA_M5_ALL_DAY_018_032_XAUUSD_M5";
        private const string BOT_LABEL = "EMA_M5_ALL_DAY_018_032_XAUUSD_M5";
...
{
    // 001 default deadzone/hysteresis (scaled for METAL by _pipsScale)
    double epsEnterPrice = InputPipsToInternalPips(Math.Max(0.0, _001_DirectionDeadzonePips)) * Symbol.PipSize;
    ...
    {
        // no deadzone -> simple side compare
        return intended == TradeType.Buy ? (close >= ema) : (close <= ema);
    }

    LineSideState prev = _ema20SideState;
    LineSideState next = UpdateLineSideState(_ema20SideState, close, ema, epsEnterPrice, epsExitPrice);

    if (next != prev)
        _ema20LastStateChangeIndex = signalIndex;

    _ema20SideState = next;

    int minHoldBars = Math.Max(0, _001_DirectionStateMinHoldBars);
    if (minHoldBars > 0 && _ema20LastStateChangeIndex >= 0)
    {
        int barsSince = signalIndex - _ema20LastStateChangeIndex;
        if (barsSince < minHoldBars)
            return false;
    }

    if (intended == TradeType.Buy)
        return _ema20SideState == LineSideState.Above;

    return _ema20SideState == LineSideState.Below;
}
...
private bool Is001ModeDistanceOkOrSetPending(int signalIndex, double close, double ema, TradeType intended, out string distanceReason)
{
    ...
    double maxDistPrice = InputPipsToInternalPips(Math.Max(0.0, _001_EntryMaxDistancePips)) * Symbol.PipSize;
    if (maxDistPrice <= 0.0)
        return true;
...
private bool TryConsume001ModeReapproachSignal(int signalIndex, double close, double ema, out TradeType intended, out string reasonTag)
{
    ...
    int window = Math.Max(1, _001_ReapproachWindowBars);
    if (signalIndex - _ema20PendingCreatedSignalIndex > window)
    {
        // expire
    ...
    double maxReapproachPrice = InputPipsToInternalPips(Math.Max(0.0, _001_ReapproachMaxDistancePips)) * Symbol.PipSize;
        ...
        private void TryEmaEntry()
{
    ...
    if (Enable001Mode)
    {
        ...
        if (crossUp || crossDown)
        {
            ...
            if (!DirectionAllows001ModeEmaEntry(signalIndex, close1, ema1, plannedType.Value))
            {
                if (EnableDebugLogJst)
                    Print("001_DIR_BLOCK | CodeName={0} | Symbol={1} | Type={2}{3}", CODE_NAME, SymbolName, plannedType.Value, BuildTimeTag(UtcNow()));
                return;
            }
            ...
            if (!Is001ModeDistanceOkOrSetPending(signalIndex, close1, ema1, plannedType.Value, out string distReason))
            {
                if (EnableDebugLogJst)
                {
                    double distPips = Math.Abs(close1 - ema1) / Symbol.PipSize;
                    Print("001_DIST_PENDING | CodeName={0} | Symbol={1} | Type={2} | DistPips={3} | Reason={4}{5}",
                        CODE_NAME, SymbolName, plannedType.Value,
        ...
        else
        {
            ...
            {
                if (EnableDebugLogJst)
                    Print("001_DIR_BLOCK | CodeName={0} | Symbol={1} | Type={2} | Reason=REAPPROACH{3}", CODE_NAME, SymbolName, plannedType.Value, BuildTimeTag(UtcNow()));
                return;
            }
        }
    }
    ...
    PlaceTrade(type, entry, stop, tp, reasonTag);
        ...
        private void PlaceTrade(TradeType type, double entry, double stop, double tpTargetPrice, string reasonTag)
        {
            ...
            for (int attempt = 0; attempt < maxAttempts; attempt++)
            {
                ...
                if (Enable001Mode)
                {
                    // 001MODEでもSL/TPは常に即時有効（最小保有時間は「裁量クローズ抑制」）
...
TradeResult modRes = null;
                ...
                // SL/TPは常に即時有効。エントリー後にAbsolute指定で再設定して、スリッページ後でも価格整合を取る。
