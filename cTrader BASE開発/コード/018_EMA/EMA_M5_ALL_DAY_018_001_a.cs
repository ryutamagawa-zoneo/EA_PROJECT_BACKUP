...
// ============================================================
// BASE: PIVOT_BOUNCE_M5_ALL_DAY_017_016
// THIS: EMA_M5_ALL_DAY_018_001_a
// ARCHIVE_ID: 001 (file export marker; no runtime effect)
// ============================================================
...
namespace cAlgo
{
    ...
    public class HIVE_PivotBounce_v0_1_7 : Robot
    {
        ...
        // BASE: PIVOT_BOUNCE_M5_ALL_DAY_017_016
        // THIS: EMA_M5_ALL_DAY_018_001_a
        private const string CODE_NAME = "EMA_M5_ALL_DAY_018_001_a";
        private const string BOT_LABEL = "EMA_M5_ALL_DAY_018_001_a";
        // ============================================================
        ...
        [Parameter("旧Entry（Pivot）を有効化（はい・いいえ）", Group = "旧Entry（無効化）", DefaultValue = false)]
        public bool EnableLegacyPivotEntry { get; set; }


        [Parameter("EMA20方向フィルター（はい・いいえ）", Group = "方向フィルタ", DefaultValue = false)]
        public bool EnableEma20DirectionFilter { get; set; }
        ...
        private bool _pivotContextActive;
        ...
        protected override void OnStart()
        {
            ...
            _ema20 = Indicators.ExponentialMovingAverage(Bars.ClosePrices, Math.Max(1, EmaPeriod));
...
";
        ...
        protected override void OnBar()
        {
            ...
            if (EnableTradingWindowFilter)
            {
                ...
                {
                    if (!EnableLegacyPivotEntry && EnableEmaDecisionLog)
                    {
            ...
            if (EnableLegacyPivotEntry)
            {
                _pivotContextActive = true;
                ...
                // (B) Legacy entry evaluation (may set pending)
                TryPivotBounceEntry_LatestTouchOnly();
                return;
            }

            _pivotContextActive = false;
            TryEmaEntry();
        }
        ...
        private void TryEmaEntry()
        {
            ...
            double ema1 = (_ema20 != null && _ema20.Result != null && _ema20.Result.Count > 0) ? _ema20.Result.Last(1) : 0.0;
            double ema2 = (_ema20 != null && _ema20.Result != null && _ema20.Result.Count > 0) ? _ema20.Result.Last(2) : 0.0;
            ...
            if (BuyOnly && type != TradeType.Buy)
                return;
            if (SellOnly && type != TradeType.Sell)
                return;
            ...
            {
                Print(
                    "EMA_SIGNAL | CodeName={0} | Symbol={1} | UtcNow={2:o} | Cross={3} | Type={4} | Close2={5} EMA2={6} | Close1={7} EMA1={8} | Entry={9} | Bid={10} Ask={11} | SpreadPips={12:F2} | BuyOnly={13} SellOnly={14}",
                    CODE_NAME,
                    SymbolName,
        ...
        private void PlaceTrade(TradeType type, double entry, double stop, double tpTargetPrice, string reasonTag)
        {
            ...
            {
                Print("SKIP_BUY_ONLY | CodeName={0} | Symbol={1} | Wanted=Sell", CODE_NAME, SymbolName);
                return;
            }



            // ===== EMA20 Direction Filter (017_016) =====
            if (EnableEma20DirectionFilter)
            {
                if (Bars == null || Bars.ClosePrices == null || Bars.ClosePrices.Count < 2 || _ema20 == null || _ema20.Result == null || _ema20.Result.Count < 2)
                {
                    // Safety: If EMA data is not ready, do not trade to avoid undefined behavior.
                    Print("SKIP_EMA20_FILTER | CodeName={0} | Symbol={1} | Reason=DataNotReady", CODE_NAME, SymbolName);
                    return;
                }

                double close1 = Bars.ClosePrices.Last(1);
                double ema1 = _ema20.Result.Last(1);

                if (BuyOnly)
                {
                    // BuyOnly=ON: Buy allowed only when Close(1) > EMA20(1); Sell always blocked above.
                    if (type == TradeType.Buy && close1 <= ema1)
                    {
                        Print("SKIP_EMA20_FILTER | CodeName={0} | Symbol={1} | Wanted=Buy | Close1={2} | EMA20_1={3} | BuyOnly=true",
                            CODE_NAME,
                            SymbolName,
                            close1.ToString("G17", CultureInfo.InvariantCulture),
                            ema1.ToString("G17", CultureInfo.InvariantCulture)
                ...
                {
                    // BuyOnly=OFF: Above EMA -> Buy only; Below EMA -> Sell only; Equal -> block both.
                    if (close1 > ema1)
                    {
                        if (type == TradeType.Sell)
                        {
                            Print("SKIP_EMA20_FILTER | CodeName={0} | Symbol={1} | Wanted=Sell | Close1={2} | EMA20_1={3} | Mode=AboveEMA",
                                CODE_NAME,
                                SymbolName,
                                close1.ToString("G17", CultureInfo.InvariantCulture),
                                ema1.ToString("G17", CultureInfo.InvariantCulture)
                    ...
                    else if (close1 < ema1)
                    {
                        if (type == TradeType.Buy)
                        {
                            Print("SKIP_EMA20_FILTER | CodeName={0} | Symbol={1} | Wanted=Buy | Close1={2} | EMA20_1={3} | Mode=BelowEMA",
                                CODE_NAME,
                                SymbolName,
                                close1.ToString("G17", CultureInfo.InvariantCulture),
                                ema1.ToString("G17", CultureInfo.InvariantCulture)
                    ...
                    {
                        // Equal: block both directions.
                        Print("SKIP_EMA20_FILTER | CodeName={0} | Symbol={1} | Wanted={2} | Close1={3} | EMA20_1={4} | Mode=Equal",
                            CODE_NAME,
                            SymbolName,
                            type.ToString(),
                            close1.ToString("G17", CultureInfo.InvariantCulture),
            ...
            if (EnablePivotHitLog && _pivotContextActive)
            {
