...
// ============================================================
// BASE: EMA_M5_ALL_DAY_018_026_XAUUSD_M5
// THIS: EMA_M5_ALL_DAY_018_028_XAUUSD_M5
// ARCHIVE_ID: 028 (file export marker; no runtime effect)
// ============================================================
...
namespace cAlgo
{
    ...
    public class EA_DEV_FIRST : Robot
    {
        ...
        // CODE NAME (Project Constitution compliant)
        // ============================================================
        // BASE: EMA_M5_ALL_DAY_018_023_XAUUSD_M5
        // THIS: EMA_M5_ALL_DAY_018_028_XAUUSD_M5
        private const string CODE_NAME = "EMA_M5_ALL_DAY_018_028_XAUUSD_M5";
        private const string BOT_LABEL = "EMA_M5_ALL_DAY_018_028_XAUUSD_M5";
        // ============================================================
        ...
        [Parameter("タッチ許容幅（PIPS）（貴金属=あなたPIPS/$相当）", Group = "エントリー関連", DefaultValue = 10.0, MinValue = 0.0)]
        public double TouchAllowancePips { get; set; }
        ...
        [Parameter("SL方式（固定/ATR/構造）", Group = "ストップロス関連", DefaultValue = SL方式.固定)]
        public SL方式 SlMode { get; set; }

        [Parameter("スイング判定 左右本数", Group = "ストップロス関連", DefaultValue = 2, MinValue = 1)]
        public int SwingLR { get; set; }

        [Parameter("スイング探索本数", Group = "ストップロス関連", DefaultValue = 80, MinValue = 10)]
        public int SwingLookback { get; set; }

        [Parameter("構造SLバッファ（PIPS）（貴金属=あなたPIPS/$相当）", Group = "ストップロス関連", DefaultValue = 100.0, MinValue = 0.0)]
        public double StructureSlBufferPips { get; set; }

        [Parameter("最大SL（PIPS）（貴金属=あなたPIPS/$相当）", Group = "ストップロス関連", DefaultValue = 100.0, MinValue = 0.0)]
        public double MaxSlPipsInput { get; set; }

        [Parameter("構造SLが見つからない場合はエントリーしない", Group = "ストップロス関連", DefaultValue = true)]
        public bool BlockEntryIfNoStructureSl { get; set; }

        [Parameter("構造SL最小距離（ドル）", Group = "ストップロス関連", DefaultValue = 2.0, MinValue = 0.0)]
        public double StructureSlMinDistanceDollars { get; set; }
        ...
        [Parameter("EMA反転で決済を行う", Group = "決済制御", DefaultValue = false)]
        public bool EnableEmaExit { get; set; }
        ...
        [Parameter("EMA方向フィルター（はい・いいえ）", Group = "方向フィルタ", DefaultValue = true)]
        public bool EnableEmaDirectionFilter { get; set; }


        [Parameter("方向判定時間足", Group = "方向フィルタ", DefaultValue = "Hour")]
        public TimeFrame DirectionTimeFrame { get; set; }

        [Parameter("方向判定EMA期間", Group = "方向フィルタ", DefaultValue = 200, MinValue = 1)]
        public int DirectionEmaPeriod { get; set; }

        [Parameter("傾き判定本数", Group = "方向フィルタ", DefaultValue = 5, MinValue = 1)]
        public int DirectionSlopeBars { get; set; }
        ...
        [Parameter("TP倍率（SL×倍率）", Group = "リスクリワード", DefaultValue = 1.0, MinValue = 0.0)]
        ...
        private ExponentialMovingAverage _ema240;

        private int _lastEntryBarIndex = -1;
        private double _prevTouchDist = double.NaN;
        private bool _volatilityLocked = false;
        private Bars _dirBars;
        private ExponentialMovingAverage _dirEma;
        ...
        protected override void OnStart()
        {
            ...
            _ema240 = Indicators.ExponentialMovingAverage(Bars.ClosePrices, 240);

            // 方向判定（028仕様：M5 240EMA傾き）
            _dirBars = Bars;
            _dirEma = _ema240;
            ...
            Print(
                ...
                "NewsMode={19} Before={20} After={21} | SpreadMax(PIPS)={22}{23} | Mode={24} PipsScale={25} SlipAllow={26} SlipAllowInt={27} | TPmult={28} | StructMin$={29}",
                CODE_NAME,
                BOT_LABEL,
                SymbolName,
                ...
                Math.Max(0.0, StructureSlMinDistanceDollars).ToString("F2", CultureInfo.InvariantCulture)
        ...
        {
            // 028仕様：M5 240EMA傾きで環境認識
            if (!EnableEmaDirectionFilter)
                return Regime.Neutral;

            if (Bars == null || _ema240 == null || _ema240.Result == null)
                return Regime.Neutral;

            int lastClosed = Bars.Count - 2;
            int n = Math.Max(1, DirectionSlopeBars);
            ...
            // 直近n本（確定足）の単調増加/減少で傾きを判定
            for (int i = 0; i < n; i++)
            {
                double cur = _ema240.Result[lastClosed - i];
                double prev = _ema240.Result[lastClosed - i - 1];
                if (cur <= prev) up = false;
                if (cur >= prev) down = false;
                if (!up && !down) break;
            }

            if (up) return Regime.Bull;
            if (down) return Regime.Bear;
            return Regime.Neutral;
        }

        private bool PassesDirectionFilter(TradeType type)
        {
            var r = GetRegime();

            if (r == Regime.Bear && type == TradeType.Buy)
            {
                if (EnableDebugLogJst)
                    Print("DIR_BLOCK_LONG | CodeName={0} | Regime=BEAR | HTF={1} | EMA={2} | SlopeBars={3}", CODE_NAME, DirectionTimeFrame, DirectionEmaPeriod, DirectionSlopeBars);
                return false;
            }

            if (r == Regime.Bull && type == TradeType.Sell)
            {
                if (EnableDebugLogJst)
                    Print("DIR_BLOCK_SHORT | CodeName={0} | Regime=BULL | HTF={1} | EMA={2} | SlopeBars={3}", CODE_NAME, DirectionTimeFrame, DirectionEmaPeriod, DirectionSlopeBars);
                return false;
            }

            return true; // Neutralは両方向許可
        }


        // ============================================================
        // 028: 240EMA×100EMA 押し目タッチ（足中）エントリー
        // ============================================================

        private void Try028TouchEntryOnTick()
        {
            if (Bars == null || Bars.Count < 260)
                return;

            // 取引時間帯（JST）＋ 経済指標フィルター（UTC）は、足中でも新規エントリーに適用する
            DateTime utcNow = UtcNow();
            ...
            News_InitOrRefresh(utcNow);
            if (!IsNewEntryAllowed(utcNow, out _))
                return;
            ...
            // 急変動フィルタ（形成中/確定足を両方監視）
            if (IsVolatilityBlockedNow())
                return;

            // 同一足での多重発注防止（形成中のM5足）
            int currentBar = Bars.Count - 1;
            if (_lastEntryBarIndex == currentBar)
                return;

            if (!TryGet028TouchSignal(bid, ask, out TradeType type))
                return;
            ...
            if (!PassesDirectionFilter(type))
                return;

            double entry = (type == TradeType.Buy) ? ask : bid;

            if (!TryComputeStopsAndTp(type, entry, out double stop, out double tp))
                return;

            PlaceTrade(type, entry, stop, tp, "EMA_PULLBACK_TOUCH");
            _lastEntryBarIndex = currentBar;
        }

        private bool IsVolatilityBlockedNow()
        {
            if (Bars == null || Bars.Count < 3 || _ema100 == null || _ema100.Result == null)
                return true;

            int forming = Bars.Count - 1;
            int lastClosed = Bars.Count - 2;

            // 10ドル以上（High-Low）で急変動とみなす（形成中・確定足の両方）
            double rangeForming = Bars.HighPrices[forming] - Bars.LowPrices[forming];
            double rangeClosed = Bars.HighPrices[lastClosed] - Bars.LowPrices[lastClosed];

            if (rangeForming >= 10.0 || rangeClosed >= 10.0)
                _volatilityLocked = true;

            if (!_volatilityLocked)
                return false;

            // 待機解除：直近確定足の終値が100EMAの上（ロング）/下（ショート）に戻ったら解除
            double close = Bars.ClosePrices[lastClosed];
            double ema100 = _ema100.Result[lastClosed];

            var r = GetRegime();

            if (r == Regime.Bull && close > ema100)
                _volatilityLocked = false;
            else if (r == Regime.Bear && close < ema100)
                _volatilityLocked = false;

            // 待機中にさらに急変動が出ても、継続（_volatilityLocked=true のまま）
            return _volatilityLocked;
        }

        private bool TryGet028TouchSignal(double bid, double ask, out TradeType type)
        {
            type = TradeType.Buy;

            if (_ema100 == null || _ema100.Result == null || _ema240 == null || _ema240.Result == null)
                return false;
            ...
            // 上から押し目（直前3本の終値が100EMAより上）/ショートは逆
            bool closesAbove = true;
            bool closesBelow = true;

            for (int k = 1; k <= 3; k++)
            {
                int idx = lastClosed - k;
                double c = Bars.ClosePrices[idx];
                double e = _ema100.Result[idx];

                if (c <= e) closesAbove = false;
                if (c >= e) closesBelow = false;
            }

            var r = GetRegime();

            // タッチ許容幅（入力PIPSは、貴金属=あなたPIPS/$相当）
            double tolInput = Math.Max(0.0, TouchAllowancePips);
            double tolInternalPips = InputPipsToInternalPips(tolInput);
            double tolPrice = tolInternalPips * Symbol.PipSize;

            double ema100Now = _ema100.Result[lastClosed];

            // タッチ判定はBid基準（発注はAsk/Bid）
            double dist = bid - ema100Now;

            if (double.IsNaN(_prevTouchDist))
            {
                _prevTouchDist = dist;
                return false;
            }

            bool inZone = Math.Abs(dist) <= tolPrice;
            bool touchedFromAbove = (_prevTouchDist > tolPrice) && inZone;
            bool touchedFromBelow = (_prevTouchDist < -tolPrice) && inZone;

            _prevTouchDist = dist;

            if (r == Regime.Bull && closesAbove && touchedFromAbove)
            {
                type = TradeType.Buy;
                return true;
            }

            if (r == Regime.Bear && closesBelow && touchedFromBelow)
            {
                type = TradeType.Sell;
                return true;
            }
        ...
        private bool TryComputeStopsAndTp(TradeType type, double entry, out double stop, out double tp)
        {
            tp = 0.0;
            stop = 0.0;

            // ===== 020 ADD: SL算出（構造=スイング）復帰（028でも踏襲） =====
            ...
            if (SlMode == SL方式.構造)
            {
                ...
                else
                {
                    ...
                    // --- Too-close guard (policy B): if structure SL is too close, fallback to MinSL/ATR (do not block entry) ---
                    double minStructDistPrice = GetStructureMinDistancePrice();
                    if (minStructDistPrice > 0.0 && intendedSlDistancePrice + 1e-12 < minStructDistPrice)
                    {
                        double minStructDistPips = minStructDistPrice / Symbol.PipSize;
                        if (EnableDebugLogJst)
                        {
                            Print("STRUCTURE_SL_FALLBACK | CodeName={0} | Symbol={1} | Type={2} | Action=FALLBACK_TO_MINSL | Reason=TOO_CLOSE | EffSLpips={3} < MinSLpips={4} | MinDist$={5}{6}",
                                CODE_NAME, SymbolName, type,
                                (intendedSlDistancePrice / Symbol.PipSize).ToString("F1", CultureInfo.InvariantCulture),
                                minStructDistPips.ToString("F1", CultureInfo.InvariantCulture),
                                Math.Max(0.0, StructureSlMinDistanceDollars).ToString("F2", CultureInfo.InvariantCulture),
                                BuildTimeTag(UtcNow()));
                        }

                        // Force fallback later
                        stop = 0.0;
                        intendedSlDistancePrice = 0.0;
                    }

                    if (EnableDebugLogJst && intendedSlDistancePrice > 0.0)
                    {
            ...
            double tpDistance = intendedSlDistancePrice * tpMult;
            ...
            double minRrFilter = Math.Max(0.0, MinRRRatio);
            if (minRrFilter > 0.0 && expRr + 1e-12 < minRrFilter)
            {
                if (EnableDebugLogJst)
                {
                    Print("RR_FILTER_BLOCK | CodeName={0} | Symbol={1} | Type={2} | Action=NO_ENTRY | ExpRR={3} | MinRR={4} | TPmult={5}{6}",
                        CODE_NAME,
                        SymbolName,
                        type,
                        expRr.ToString("F2", CultureInfo.InvariantCulture),
                        minRrFilter.ToString("F2", CultureInfo.InvariantCulture),
                        tpMult.ToString("F2", CultureInfo.InvariantCulture),
                        BuildTimeTag(UtcNow()));
                }
                return false;
            }

            tp = (type == TradeType.Buy) ? entry + tpDistance : entry - tpDistance;
            return true;
        }
        ...
        private double GetStructureMinDistancePrice()
        {
            double d = Math.Max(0.0, StructureSlMinDistanceDollars);
            if (d <= 0.0)
                return 0.0;

            // Metals: "10あなたPIPS = $1" convention. Convert dollars -> user pips -> internal pips -> price.
            if (_symbolCategory == SymbolCategory.Metal)
            {
                double userPips = d * 10.0;
                double internalPips = InputPipsToInternalPips(userPips);
                return internalPips * Symbol.PipSize;
            }

            // FX: interpret the value as pips (keeps behavior deterministic across symbols).
        ...
        private void PlaceTrade(TradeType type, double entry, double stop, double tpTargetPrice, string reasonTag)
        {
            ...
            for (int attempt = 0; attempt < maxAttempts; attempt++)
            {
                ...
                Print(
                    "ENTRY | CodeName={0} | Symbol={1} | Type={2} | VolUnits={3} | Entry={4} | IntendedSLpips={5} EffSLpips={6} | TPpips={7} | RiskMode={8} RiskInput={9}({10}) Balance={11} | ExpLoss={12} | Reason={13} | SL_Set=OK{14} | SlipAllow={15} SlipAllowInt={16} | ExpLossAtSL={17} | ExpRR={18} | MinRR={19} | TPmult={20}",
                    CODE_NAME,
                    SymbolName,
                    type,
                    volumeInUnits,
