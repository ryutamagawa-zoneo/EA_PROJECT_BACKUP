...
namespace cAlgo
{
    ...
    public class HIVE_PivotBounce_v0_1_0 : Robot
    {
        ...
        // FIX: Use fixed dollars instead of percent compounding.
        ...
        // ================= RISK_DIAG (NetProfit vs Expected) =================
        ...
        private long _diagRiskExpectedLossOverRisk;
        private long _diagRiskExpectedLeRiskButActualOver1200;
        ...
        private class RiskTrack
        {
            ...
            public double ExpectedLossAtSl;
        }

        // ================= STOP_DIAG (StopLoss Mult by NetProfit; closePrice不要) =================
        private long _diagStopClosed; // "StopLoss reason" where expected exists & mult computed
        ...
        // Spread/Slip pips are kept for compatibility, but may remain NaN if not computed.
        private double _diagSlipP50 = double.NaN, _diagSlipP90 = double.NaN, _diagSlipP95 = double.NaN, _diagSlipP99 = double.NaN, _diagSlipMax = double.NaN;
        private double _diagSpreadP50 = double.NaN, _diagSpreadP90 = double.NaN, _diagSpreadP95 = double.NaN, _diagSpreadP99 = double.NaN, _diagSpreadMax = double.NaN;
        ...
        protected override void OnStart()
        {
            ...
            Timer.Start(1);
            // [ADD] Position close event (for STOP_DIAG / CLOSE_REASON)
...
";
            ...
            Print(
                "Started. cBot={0} Symbol={1} | CodeName={2} | Window(JST) Start={3:D2}:{4:D2} End={5:D2}:{6:D2} ForceClose={7:D2}:{8:D2} | Pivot=Classic(Daily NY17:00) | Guards: MinSLPips={9} ATRPeriod={10} ATRMult={11} MaxLotsCap={12} | BETrigger$={13} | Risk$={14}",
        ...
        {
            _diagOnStop++;
            // [ADD] unsubscribe (safety)
            Positions.Closed -= OnPositionClosed;
            // ---- STOP_DIAG: compute Mult percentiles from samples (NetProfit based) ----
            ...
            Print(
                "DIAG_SUMMARY | CodeName={0} | Symbol={1} | OnStart={2} OnStop={3} OnTick={4} OnTimer={5} OnBar={6} | OnBarRet_BarsCount={7} OnBarRet_ForceFlat={8} OnBarRet_NotAllowNewEntries={9} OnBarRet_NewsWindow={10} OnBarRet_MaxPositions={11} OnBarRet_NoPivot={12} | PivotUpdateCalls={13} TryPivotBounceEntryCalled={14} | PlaceTradeCalled={15} PlaceTradeRet_Risk$<=0={16} PlaceTradeRet_Balance<=0={17} PlaceTradeRet_SLDist<=0={18} PlaceTradeRet_SLPips<=0={19} PlaceTradeRet_SLBelowMin={20} PlaceTradeRet_VolBelowMin={21} PlaceTradeRet_VolAboveCap={22} PlaceTradeRet_TPInvalidNoFallback(RR=0)={23} | TPInvalid_Buy={24} TPInvalid_Sell={25} | ExecuteCalled={26} Success={27} | TagCalled[S1]={28} TagTPInvalid[S1]={29} TagExec[S1]={30} TagSucc[S1]={31} | TagCalled[R1]={32} TagTPInvalid[R1]={33} TagExec[R1]={34} TagSucc[R1]={35} | R1Sell_TPInv_Samples={36} | R1Sell_TPInv_MinEntry={37} R1Sell_TPInv_MaxEntry={38} | R1Sell_TPInv_MinPP={39} R1Sell_TPInv_MaxPP={40} | R1Sell_TPInv_MinR1={41} R1Sell_TPInv_MaxR1={42} | R1Sell_TPInv_FirstSessionStartUTC={43:o} R1Sell_TPInv_LastSessionStartUTC={44:o} | S1Buy_TPInv_Samples={45} | S1Buy_TPInv_MinEntry={46} S1Buy_TPInv_MaxEntry={47} | S1Buy_TPInv_MinPP={48} S1Buy_TPInv_MaxPP={49} | S1Buy_TPInv_MinS1={50} S1Buy_TPInv_MaxS1={51} | S1Buy_TPInv_FirstSessionStartUTC={52:o} S1Buy_TPInv_LastSessionStartUTC={53:o} | RISK_DIAG Tracked={54} Closed={55} LossTrades={56} ActualLoss>1200={57} ExpectedLoss>Risk={58} Expected<=RiskButActual>1200={59} MaxExpectedLossAtSL$={60} MaxActualLoss$={61} MaxActualLossTimeUTC={62:o} | STOP_DIAG StopClosed={63} SlipPips_P50={64} P90={65} P95={66} P99={67} Max={68} | SpreadPips_P50={69} P90={70} P95={71} P99={72} Max={73} | Mult_P50={74} P90={75} P95={76} P99={77} Max={78}"
                + " | CLOSE_REASON StopLoss=" + _diagCloseReason_StopLoss
                + " TakeProfit=" + _diagCloseReason_TakeProfit
                + " Other=" + _diagCloseReason_ManualOrOther
                + " Unknown=" + _diagCloseReason_Unknown,
                CODE_NAME,
                SymbolName,
                _diagOnStart,
                _diagOnStop,
                ...
                _diagRiskExpectedLossOverRisk,
                _diagRiskExpectedLeRiskButActualOver1200,
                ...
                _diagStopClosed,
                _diagSlipP50, _diagSlipP90, _diagSlipP95, _diagSlipP99, _diagSlipMax,
                _diagSpreadP50, _diagSpreadP90, _diagSpreadP95, _diagSpreadP99, _diagSpreadMax,
...

        {
            ...
            if (_riskByPositionId.TryGetValue(posId, out RiskTrack tr))
            {
                ...
                if (isLoss)
                {
                    ...
                    if (tr.ExpectedLossAtSl > tr.RiskDollars)
                        _diagRiskExpectedLossOverRisk++;

                    if (tr.ExpectedLossAtSl <= tr.RiskDollars && actualLoss > 1200.0)
                        _diagRiskExpectedLeRiskButActualOver1200++;
                    ...
                    if (actualLoss > _diagRiskMaxActualLoss)
                    {
                        ...
                        DateTime ct = GetCloseTimeSafe(args, p);
                        _diagRiskMaxActualLossTimeUtc = ct;
                    }
                }
            ...
            // STOP_DIAG: StopLoss reason only, compute Mult using NetProfit (closePrice不要)
            if (reason == "StopLoss")
            {
                if (_expectedLossAtSlByPositionId.TryGetValue(posId, out double expectedLossAtSl) && expectedLossAtSl > 0.0)
                {
                    double actualLoss = Math.Max(0.0, -p.NetProfit);

                    if (actualLoss > 0.0)
                    {
                        double mult = actualLoss / expectedLossAtSl;
                        _stopLossMultSamples.Add(mult);
                        _diagStopClosed++;
                    }
                }

                _expectedLossAtSlByPositionId.Remove(posId);
            }
            else
            {
                // cleanup to avoid growth
                _expectedLossAtSlByPositionId.Remove(posId);
            }
        }
        ...
        private void PlaceTrade(TradeType type, double entry, double stop, double tpTargetPrice, string reasonTag)
        {
            ...
            // IMPORTANT CHANGE:
            // Previously: if above cap -> RETURN (no entry).
            // Now: cap volume to MaxLotsCap (risk decreases), and CONTINUE to execute.
            if (MaxLotsCap > 0.0)
            {
                ...
                if (maxUnits > 0 && volumeInUnits > maxUnits)
                {
                    ...
                    PrintGuardSkip(
                        ...
                        plannedLots,
                        "VOLUME_CAPPED_TO_MAX_LOTS_CAP (PlannedLots=" + plannedLots.ToString("F2", CultureInfo.InvariantCulture) + " -> CapLots=" + capLots.ToString("F2", CultureInfo.InvariantCulture) + ")"
                    );
            ...
            if (result != null && result.IsSuccessful)
            {
                ...
                // ---- RISK_DIAG: track this position (Risk$ / ExpectedLossAtSL$) ----
                if (result.Position != null)
                {
                    ...
                    var rt = new RiskTrack
                    {
                        RiskDollars = riskDollars,
                        ExpectedLossAtSl = expectedLossAtSl
                    };

                    _riskByPositionId[posId] = rt;

                    if (expectedLossAtSl > 0.0)
                        _expectedLossAtSlByPositionId[posId] = expectedLossAtSl;
            ...
            Print(
                "ENTRY | CodeName={0} | Reason={1} | Type={2} | Entry={3} | SL={4} | SLPips={5} | TPpips={6} | VolUnits={7} | Risk$={8}",
                CODE_NAME,
                reasonTag,
                type,
        ...
        private double CalcExpectedLossAtSlDollars(double slPips, long volumeInUnits)
        {
            ...
            double unitsPerLot = Symbol.QuantityToVolumeInUnits(1.0);
            if (unitsPerLot <= 0.0)
                return 0.0;

            double pipValuePerUnit = Symbol.PipValue / unitsPerLot;

            double expected = slPips * pipValuePerUnit * volumeInUnits;
        ...
        {
            // Prefer args time, else position time, else Server.Time
            try
            {
                if (args != null)
                {
                    ...
                    var pa = ta.GetProperty("ClosingTime");
                    if (pa != null)
                    {
                        object v = pa.GetValue(args, null);
                    ...
                    var pa2 = ta.GetProperty("Time");
                    if (pa2 != null)
                    {
                        object v = pa2.GetValue(args, null);
                        if (v is DateTime dt2) return DateTime.SpecifyKind(dt2, DateTimeKind.Utc);
                    }
                }
                ...
                if (p != null)
                {
                    ...
                    var pp = t.GetProperty("ClosingTime");
                    if (pp != null)
                    {
                        object v = pp.GetValue(p, null);
                        if (v is DateTime dt3) return DateTime.SpecifyKind(dt3, DateTimeKind.Utc);
                    }

                    var pp2 = t.GetProperty("CloseTime");
                    if (pp2 != null)
                    {
                        object v = pp2.GetValue(p, null);
                        if (v is DateTime dt4) return DateTime.SpecifyKind(dt4, DateTimeKind.Utc);
                    }
                }
            }
            catch
