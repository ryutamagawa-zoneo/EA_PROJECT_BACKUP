...
// ============================================================
// BASE: EMA_M5_ALL_DAY_019_015_XAUUSD_M5
// THIS: EMA_M5_ALL_DAY_019_019_XAUUSD_M5
// ARCHIVE_ID: 019_019 (file export marker; no runtime effect)
// ============================================================
...
namespace cAlgo
{
    ...
    public class EA_DEV_FIRST : Robot
    {
        ...
        // BASE: EMA_M5_ALL_DAY_019_015_XAUUSD_M5
        // THIS: EMA_M5_ALL_DAY_019_019_XAUUSD_M5
        private const string CODE_NAME = "EMA_M5_ALL_DAY_019_019_XAUUSD_M5";
        private const string BOT_LABEL = "EMA_M5_ALL_DAY_019_019_XAUUSD_M5";
        ...
        // ============================================================
        // PROレポート：1テスト=1ファイル保証（OnStop多重呼び出し耐性）
        // ============================================================
        ...
        [Parameter("構造TPブースト（条件付き）", Group = "TP関連 - 構造TPブースト", DefaultValue = false)]
        public bool EnableStructureTpBoost { get; set; }

        [Parameter("構造TPブースト開始R（第1段）", Group = "TP関連 - 構造TPブースト", DefaultValue = 0.9, MinValue = 0.0)]
        public double StructureTpBoostStartRStage1 { get; set; }

        [Parameter("構造TP第2段ブースト（有効）", Group = "TP関連 - 構造TPブースト", DefaultValue = true)]
        public bool EnableStructureTpBoostStage2 { get; set; }

        [Parameter("構造TPブースト開始R（第2段）", Group = "TP関連 - 構造TPブースト", DefaultValue = 1.8, MinValue = 0.0)]
        ...
        private readonly HashSet<long> _timeCutCloseRequested = new HashSet<long>();
        ...
        private readonly HashSet<long> _s2TimeCutBypassedLoggedPosIds = new HashSet<long>(); // S2_TIMECUT_BYPASSED (PosIdごと1回)
...
private void PrintS2TimeCutBypassedOnce(Position p, double heldMin)
{
    if (p == null) return;
    if (_s2TimeCutBypassedLoggedPosIds.Contains(p.Id))
        return;

    _s2TimeCutBypassedLoggedPosIds.Add(p.Id);

    Print("S2_TIMECUT_BYPASSED | CodeName={0} | PosId={1} | HeldMin={2:F1} | ThresholdMin={3}{4}",
        CODE_NAME, p.Id, heldMin, TIME_CUT_MINUTES_001, BuildTimeTag(UtcNow()));
}
// ============================================================
        // Stage2 Exit（状態変化）関連
        // ============================================================
        private Bars _barsM15;
        private ExponentialMovingAverage _emaM15_20;
        ...
        private const int TIME_CUT_MINUTES_001 = 30; // 001MODE=ON & 管理探索=OFF のみ
        ...
        protected override void OnStart()
        {
            ...
            _emaM15_20 = Indicators.ExponentialMovingAverage(_barsM15.ClosePrices, EMA_PERIOD_FIXED);
        ...
        protected override void OnStop()
        {
            ...
            // Stage2 / TimeCut bypass state cleanup (safety)
            _structureTpBoostedPosIdsStage2.Clear();
            _s2TimeCutBypassedLoggedPosIds.Clear();
        ...
        protected override void OnTimer()
        {
            ...
            // Phase Dʼ：Time Cut（001MODE=ON & 管理探索=OFF のみ）
            EnforceTimeCut001(utcNow);

            // 取引時間帯フィルタが無効なら、ここで終了（Time Cut は上で実行済み）
            if (!EnableTradingWindowFilter)
                return;
        ...
        private void ApplyStructureTpBoostIfNeeded()
        {
            ...
            // 001探索OFFのみ対象（探索ON=管理探索モードでは、探索側TP最適化に委ねる）
            ...
            foreach (var p in Positions)
            {
                ...
                else
                {
                    ...
                    // 2) M15 EMA20 タッチ（状態変化Exit）
                    if (IsM15Ema20Touch(p, out double ema20))
                    {
                        string logLine = string.Format(CultureInfo.InvariantCulture,
                            "STRUCT_S2_EXIT_M15_EMA20_TOUCH | CodeName={0} | Label={1} | Symbol={2} | Type={3} | PositionId={4} | EMA20={5} | Price={6}{7}",
                            CODE_NAME, BOT_LABEL, SymbolName, p.TradeType, p.Id,
                            ema20.ToString("F2", CultureInfo.InvariantCulture),
                            (p.TradeType == TradeType.Buy ? Symbol.Bid : Symbol.Ask).ToString("F2", CultureInfo.InvariantCulture),
                            BuildTimeTag(utcNow));

                        _closeInitiatorByPosId[p.Id] = "S2_EXIT_M15_EMA20_TOUCH";
                        TryClosePositionOncePerBar(p, "S2_EXIT_M15_EMA20_TOUCH", logLine);
                        continue;
                    }
...
";
        ...
        // ============================================================
        // Phase Dʼ：Time Cut（保有時間制限）
        //  - 001MODE=ON & 管理探索=OFF のみ有効
        //  - CLOSE_CORE は ForceFlat として記録（評価基盤仕様）
        //  - ClosePrice は OnPositionClosed の引数のみ使用（既存仕様）
        // ============================================================
        private void EnforceTimeCut001(DateTime utcNow)
        {
            if (!Enable001Mode)
                return;

            if (EnableMgmtExplore001)
                return;
            ...
            for (int i = Positions.Count - 1; i >= 0; i--)
            {
                ...
                DateTime entryUtc = DateTime.SpecifyKind(p.EntryTime, DateTimeKind.Utc);
                double heldMin = (utcNow - entryUtc).TotalMinutes;

                // Stage2中は TimeCut を常に不成立扱い（切り離し）
                if (IsStage2(p))
                {
                    // TimeCut閾値を超えた場合のみ、PosIdごと1回ログ（洪水防止）
                    if (heldMin > TIME_CUT_MINUTES_001)
                        PrintS2TimeCutBypassedOnce(p, heldMin);
                ...
                // Time Cut は PosId 単位で1回のみ（Close要求／ログの重複防止）
                if (_timeCutCloseRequested.Contains(p.Id))
                    continue;

                _timeCutCloseRequested.Add(p.Id);

                // 評価基盤：ForceFlat として統一（TimeCut 起因だが CloseReason は ForceFlat）
                _closeInitiatorByPosId[p.Id] = "FORCE_CLOSE:FORCE_FLAT_TIME_CUT";

                var res = ClosePosition(p);

                Print("TIME_CUT | CodeName={0} | PosId={1} | HeldMin={2:F1} | ThresholdMin={3}{4}",
                    CODE_NAME, p.Id, heldMin, TIME_CUT_MINUTES_001, BuildTimeTag(UtcNow()));

                // 失敗時のみ再試行を許可（成功時は OnPositionClosed で解除）
                if (res == null || !res.IsSuccessful)
                    _timeCutCloseRequested.Remove(p.Id);
                _structureTpBoostedPosIds.Remove(p.Id);
                _marketCloseCloseRequested.Remove(p.Id);
            }
        }
        ...
        private string NormalizeCloseReason(PositionCloseReason reason, string initiator)
        {
            ...
            if (!string.IsNullOrWhiteSpace(initiator) && initiator.StartsWith("FORCE_CLOSE:", StringComparison.Ordinal))
            {
                ...
                // TimeCut を含め、FORCE_FLAT で始まるものは ForceFlat に統一
        ...
        // ============================================================
        private bool IsM15Ema20Touch(Position p, out double emaValue)
        {
            emaValue = double.NaN;
            ...
            emaValue = _emaM15_20.Result.LastValue;
            if (double.IsNaN(emaValue) || emaValue <= 0)
                return false;

            // Touch判定：価格がEMAに到達したら成立（誤差・スプレッド考慮なし）
            if (p.TradeType == TradeType.Buy)
                return Symbol.Bid <= emaValue;
            else
                return Symbol.Ask >= emaValue;
        }
        ...
        private void OnPositionClosed(PositionClosedEventArgs args)
        {
            ...
            _s2TimeCutBypassedLoggedPosIds.Remove(p.Id);
