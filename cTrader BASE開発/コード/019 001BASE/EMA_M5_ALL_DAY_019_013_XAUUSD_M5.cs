...
// ============================================================
// BASE: EMA_M5_ALL_DAY_019_011_XAUUSD_M5
// THIS: EMA_M5_ALL_DAY_019_013_XAUUSD_M5
// ARCHIVE_ID: 019_013 (file export marker; no runtime effect)
// ============================================================
...
namespace cAlgo
{
    ...
    public class EA_DEV_FIRST : Robot
    {
        ...
        // BASE: EMA_M5_ALL_DAY_019_011_XAUUSD_M5
        // THIS: EMA_M5_ALL_DAY_019_013_XAUUSD_M5
        private const string CODE_NAME = "EMA_M5_ALL_DAY_019_013_XAUUSD_M5";
        private const string BOT_LABEL = "EMA_M5_ALL_DAY_019_013_XAUUSD_M5";
        ...
        [Parameter("構造TPブースト開始R", Group = "TP関連 - 構造TPブースト", DefaultValue = 1.0, MinValue = 0.0)]
        public double StructureTpBoostStartR { get; set; }
        ...
        private readonly HashSet<long> _structureTpBoostedPosIds = new HashSet<long>();
        ...
        // ============================================================
        // 構造TPブースト（条件付き） 固定パラメータ（Phase Hʼ 仕様）
        // ============================================================
        ...
        private void ApplyStructureTpBoostIfNeeded()
        {
            ...
            foreach (var p in Positions)
            {
                ...
                // 既にブースト済みならスキップ
                if (_structureTpBoostedPosIds.Contains(p.Id))
                    continue;

                if (!p.StopLoss.HasValue)
                    continue;

                double slDist = Math.Abs(p.EntryPrice - p.StopLoss.Value);
                if (slDist <= 0.0)
                    continue;

                double cur = (p.TradeType == TradeType.Buy) ? Symbol.Bid : Symbol.Ask;
                double profitDist = (p.TradeType == TradeType.Buy) ? (cur - p.EntryPrice) : (p.EntryPrice - cur);
                double unrealizedR = profitDist / slDist;

                if (unrealizedR + 1e-12 < Math.Max(0.0, StructureTpBoostStartR))
                    continue;

                // 構造TP算出（固定：2-200-50）
                if (!TryGetStructureTakeProfitBoost(p.TradeType, p.EntryPrice, out double tpAbs))
                    continue;

                // 既に同等のTPが入っていればスキップ（微小差は許容）
                ...
                bool modified = TryModifyPositionOncePerBar(p, p.StopLoss, tpAbs, "STRUCTURE_TP_BOOST");
                if (modified)
                {
                    _structureTpBoostedPosIds.Add(p.Id);

                    Print("STRUCTURE_TP_BOOST_ACTIVATED | CodeName={0} | Label={1} | Symbol={2} | Type={3} | Trigger=UnrealizedR>={4} | UnrealizedR={5} | TpAbs={6}{7}",
                        CODE_NAME,
                        BOT_LABEL,
                        SymbolName,
                        p.TradeType,
                        Math.Max(0.0, StructureTpBoostStartR).ToString("F2", CultureInfo.InvariantCulture),
                        unrealizedR.ToString("F2", CultureInfo.InvariantCulture),
                        tpAbs.ToString("F5", CultureInfo.InvariantCulture),
                        BuildTimeTag(UtcNow()));
                }
            }
        }

        private bool TryGetStructureTakeProfitBoost(TradeType type, double entry, out double tpAbs)
        {
            tpAbs = 0.0;

            int lr = Math.Max(1, SET_BoostStructureTpLR);
            int lookback = Math.Max(10, SET_BoostStructureTpLookbackH1);

            if (_barsH1 == null || _barsH1.Count < lookback + (lr * 2) + 5)
                return false;

            double bufferInput = Math.Max(0.0, SET_BoostStructureTpBufferPips);
