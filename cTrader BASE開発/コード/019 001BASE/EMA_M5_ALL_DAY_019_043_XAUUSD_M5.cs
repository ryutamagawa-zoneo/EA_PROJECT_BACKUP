...
// THIS: EMA_M5_ALL_DAY_019_043_XAUUSD_M5
...
namespace cAlgo
{
    ...
    public class EA_DEV_FIRST : Robot
    {
        ...
        // BASE: EMA_M5_ALL_DAY_019_029_XAUUSD_M5
        // THIS: EMA_M5_ALL_DAY_019_043_XAUUSD_M5
        private const string CODE_NAME = "EMA_M5_ALL_DAY_019_043_XAUUSD_M5";
        private const string BOT_LABEL = "EMA_M5_ALL_DAY_019_043_XAUUSD_M5";
        ...
        protected override void OnBar()
        {
            ...
            if (IsCOREModeEnabled() && EnableCOREMode)
            {
            ...
            // COREモードOFF + エントリーモード=CORE は、Framework（入口）を使用し、出口（SLTP）はパラメーター基準にする
            if (IsCOREModeEnabled() && IsEntryModeCORE())
            {
                LogEntryRouteOnce("FrameworkParam");
                TryEntryFrameworkSignalWithParamExit(utcNow);
                return;
            }
        ...
        {
            // COREMODEは「明示的にModeCOREを選択し、かつEnableCOREModeがON」のときのみ有効
            if (EntryMode != エントリーモード.ModeCORE)
                return false;
        ...
        // ============================================================
        private bool isCOREEnabled => IsCOREModeEnabled();
        private bool isCOREModeEnabled => IsCOREModeEnabled();
        ...
        // ============================================================
        // Framework（基準入口）シグナルのみ（SL/TPはパラメーター世界）
        //  - COREMODE=OFF かつ エントリーモード=CORE のとき使用
        //  - 入口は TryEntryFramework と完全同一（EMAクロス確定足2本）
        //  - 出口（SL/TP）は ExecuteEntryWithPlannedType に委譲（パラメーター基準）
        // ============================================================
        private void TryEntryFrameworkSignalWithParamExit(DateTime utcNow)
        {
            if (_emaCORE == null || _emaCORE.Result == null || _emaCORE.Result.Count < 3)
                return;
            ...
            // 入口の判定ログ（比較用）
            Print("CORE_TEST_ENTRY_SIGNAL | Mode=FrameworkParam | Type={0} | Reason={1} | Close2={2} EMA2={3} | Close1={4} EMA1={5}{6}",
                type,
            ...
            // 出口（SL/TP）はパラメーター世界へ
            ExecuteEntryWithPlannedType(type, "FRAMEWORK_EMA_CROSS", ema1, close1, Bars.Count - 2
            );
        }
        ...
        private void PlaceTrade(TradeType type, double entry, double stop, double tpTargetPrice, string reasonTag)
        {
            ...
            for (int attempt = 0; attempt < maxAttempts; attempt++)
            {
                ...
                Print(
                    "SLTP_MODE | CodeName={0} | Symbol={1} | COREMODE={2} | Path={3}{4}",
                    CODE_NAME,
                    SymbolName,
                    (UseCOREFixedModeEquivalent() ? "ON" : "OFF"),
                    (UseCOREFixedModeEquivalent() ? "CORE" : "022"),
