...
// THIS: EMA_M5_ALL_DAY_019_030_XAUUSD_M5
// ARCHIVE_ID: 019_030 (file export marker; no runtime effect)
// ============================================================
...
namespace cAlgo
{
    ...
    public class EA_DEV_FIRST : Robot
    {
        ...
        // BASE: EMA_M5_ALL_DAY_019_029_XAUUSD_M5
        // THIS: EMA_M5_ALL_DAY_019_030_XAUUSD_M5
        private const string CODE_NAME = "EMA_M5_ALL_DAY_019_030_XAUUSD_M5";
        private const string BOT_LABEL = "EMA_M5_ALL_DAY_019_030_XAUUSD_M5";
        const int EMA_PERIOD_FIXED = 20;

        // ============================================================
        // 001 固定値（001.zip/001.txt の Started / PARAM_JSON と整合）
        //  - 001MODE=ON 時はパラメータUIの数値を一切参照しない（資金管理/経済指標/取引時間帯は除外）
        // ============================================================
        private const double FIX001_MaxSpreadPips = 0.0;
        private const double FIX001_MinSLPips = 30.0;
        private const int FIX001_SlAtrPeriod = 14;
        private const double FIX001_SlAtrMult = 0.5;
        private const double FIX001_MinRRRatio = 1.0;
        private const double FIX001_MinTpDistancePips = 0.0;
...
// ============================================================
        ...
        protected override void OnStart()
        {
            ...
            _atrMinSl001 = Indicators.AverageTrueRange(FIX001_SlAtrPeriod, MovingAverageType.Simple);
        ...
        protected override void OnTick()
        {
            ...
            // 001MODE=ON: keep SL/TP fixed (no breakeven / no structure TP boosts)
            if (!Enable001Mode)
            {
            ...
            // Risk layer (independent): always allowed
                ...
                private void TryEntryFramework001(DateTime utcNow)
        {
            ...
            // ------------------------------------------------------------
            // 001MODE=ON: Distance/Reapproach consumption (if pending)
            // ------------------------------------------------------------
            TradeType type;
            bool isReapproachTriggered = false;
            string reapproachReasonTag = "NA";

            if (_ema20ReapproachPending)
            {
                TradeType intended;
                string reasonTag;
                if (TryConsume001ModeReapproachSignal(i1, close1, ema1, out intended, out reasonTag))
                {
                    type = intended;
                    isReapproachTriggered = true;
                    reapproachReasonTag = reasonTag;

                    if (EnableDebugLogJst)
                    {
                        Print("REAPPROACH_TRIGGERED_ENTRY | CodeName={0} | Symbol={1} | Type={2} | Reason={3}{4}",
                            CODE_NAME, SymbolName, type, reapproachReasonTag, BuildTimeTag(utcNow));
                    }
                }
                else
                {
                    // still pending but not triggered; do not require cross here
                    // continue to normal cross detection below
                    type = TradeType.Buy; // temp init; will be overwritten if cross occurs
                }
            }
            else
            {
            ...
            // ------------------------------------------------------------
            // Cross detection (normal path). If reapproach already triggered,
            // we skip cross requirement and proceed to sizing/entry.
            // ------------------------------------------------------------
            if (!isReapproachTriggered)
            {
                ...
                type = crossUp ? TradeType.Buy : TradeType.Sell;

                // Distance gate (Too far -> set pending and exit)
                string distanceReason;
                bool distanceOk = Is001ModeDistanceOkOrSetPending(i1, close1, ema1, type, out distanceReason);
                if (!distanceOk)
                {
                    if (EnableDebugLogJst)
                    {
                        Print("REAPPROACH_PENDING_SET | CodeName={0} | Symbol={1} | Type={2} | Reason={3}{4}",
                            CODE_NAME, SymbolName, type, distanceReason, BuildTimeTag(utcNow));
                    }
                    return;
                }
            }

            // Spread gate (fixed value; do NOT use parameter UI)
            double bid = Symbol.Bid;
            double ask = Symbol.Ask;
            double spreadPrice = ask - bid;
            double spreadInternalPips = spreadPrice / Symbol.PipSize;

            double maxSpreadInternalPips = InputPipsToInternalPips(FIX001_MaxSpreadPips);
            ...
            {
                Print(
                    "EMA_SIGNAL | CodeName={0} | Symbol={1} | UtcNow={2:o} | Type={3} | Close2={4} EMA2={5} | Close1={6} EMA1={7} | Bid={8} Ask={9} | SpreadPips={10:F2} | Reapproach={11}({12})",
                    CODE_NAME,
                    SymbolName,
            ...
            // SL distance: max(MinSL from pips, MinSL from ATR) (fixed values)
            double minSlInternalPips = InputPipsToInternalPips(FIX001_MinSLPips);
            ...
            double minSlPriceFromAtr = Math.Max(0.0, FIX001_SlAtrMult) * atrValue;
            ...
            // TP target by RR (fixed)
            double rr = Math.Max(1.0, FIX001_MinRRRatio);
            ...
            // Min TP distance filter (fixed; default=0 -> disabled)
            double minTpInternalPips = InputPipsToInternalPips(FIX001_MinTpDistancePips);
            ...
            bool includeSlipInSizing = IncludeSlipAllowanceInSizing; // risk layer; allowed
...
private void TryEntrySignal001_WithParamExit()
{
    ...
    TradeType plannedType = crossUp ? TradeType.Buy : TradeType.Sell;

    // Execute with parameter-based SL/TP
    ExecuteEntryWithPlannedType(plannedType, "001_EMA_CROSS", close1, ema1, i1);
}
