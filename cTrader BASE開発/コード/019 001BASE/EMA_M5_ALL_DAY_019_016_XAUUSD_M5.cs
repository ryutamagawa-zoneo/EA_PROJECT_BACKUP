...
// THIS: EMA_M5_ALL_DAY_019_016_XAUUSD_M5
// ARCHIVE_ID: 019_016 (file export marker; no runtime effect)
// ============================================================
...
namespace cAlgo
{
    ...
    public class EA_DEV_FIRST : Robot
    {
        ...
        // BASE: EMA_M5_ALL_DAY_019_015_XAUUSD_M5
        // THIS: EMA_M5_ALL_DAY_019_016_XAUUSD_M5
        private const string CODE_NAME = "EMA_M5_ALL_DAY_019_016_XAUUSD_M5";
        private const string BOT_LABEL = "EMA_M5_ALL_DAY_019_016_XAUUSD_M5";
        ...
        protected override void OnTick()
        {
            ...
            // NOTE: For analysis, we still emit a throttled SKIP log when positions exist and management is gated.
        ...
        private void ApplyStructureTpBoostIfNeeded()
        {
            ...
            foreach (var p in Positions)
            {
                ...
                if (!stage2Active)
                {
                    ...
                    // REACHED (always)
                    TryPrintOncePerBar(p.Id, "S2_REACHED",
                        string.Format(CultureInfo.InvariantCulture,
                            "STRUCT_TP_S2_REACHED | CodeName={0} | Label={1} | Symbol={2} | Type={3} | PositionId={4} | TriggerR>={5} | UnrealizedR={6}{7}",
                            CODE_NAME, BOT_LABEL, SymbolName, p.TradeType, p.Id,
                            r2.ToString("F2", CultureInfo.InvariantCulture),
                            unrealizedR.ToString("F2", CultureInfo.InvariantCulture),
                            BuildTimeTag(utcNow)));

                    // Stage2: 構造TP算出（固定：2-300-50）
                    if (!TryGetStructureTakeProfitBoostStage(p.TradeType, p.EntryPrice, SET_BoostStructureTpLookbackH1_Stage2, SET_BoostStructureTpBufferPips_Stage2, out double tpAbs2))
                        continue;

                    // ALREADY_SET (same TP)
                    if (p.TakeProfit.HasValue && Math.Abs(p.TakeProfit.Value - tpAbs2) <= (Symbol.PipSize * 0.1))
                    {
                        _structureTpBoostedPosIdsStage2.Add(p.Id);

                        TryPrintOncePerBar(p.Id, "S2_ALREADY_SET",
                            string.Format(CultureInfo.InvariantCulture,
                                "STRUCT_TP_S2_ALREADY_SET | CodeName={0} | Label={1} | Symbol={2} | Type={3} | PositionId={4} | UnrealizedR={5}{6}{7}",
                                CODE_NAME, BOT_LABEL, SymbolName, p.TradeType, p.Id,
                                unrealizedR.ToString("F2", CultureInfo.InvariantCulture),
                                BuildTpDiag(p, tpAbs2),
                                BuildTimeTag(utcNow)));

                        continue;
                    }

                    bool modified2 = TryModifyPositionOncePerBar(p, p.StopLoss, tpAbs2, "STRUCTURE_TP_STAGE2");

                    if (modified2)
                    {
                        _structureTpBoostedPosIdsStage2.Add(p.Id);

                        TryPrintOncePerBar(p.Id, "S2_MODIFY_OK",
                            string.Format(CultureInfo.InvariantCulture,
                                "STRUCT_TP_S2_MODIFY_OK | CodeName={0} | Label={1} | Symbol={2} | Type={3} | PositionId={4} | UnrealizedR={5}{6} | LookbackH1={7} | BufferPips={8}{9}",
                                CODE_NAME, BOT_LABEL, SymbolName, p.TradeType, p.Id,
                                unrealizedR.ToString("F2", CultureInfo.InvariantCulture),
                                BuildTpDiag(p, tpAbs2),
                                SET_BoostStructureTpLookbackH1_Stage2,
                                SET_BoostStructureTpBufferPips_Stage2.ToString("F1", CultureInfo.InvariantCulture),
                                BuildTimeTag(utcNow)));
                    }
                    else
                    {
                        TryPrintOncePerBar(p.Id, "S2_MODIFY_FAIL",
                            string.Format(CultureInfo.InvariantCulture,
                                "STRUCT_TP_S2_MODIFY_FAIL | CodeName={0} | Label={1} | Symbol={2} | Type={3} | PositionId={4} | UnrealizedR={5}{6} | LookbackH1={7} | BufferPips={8} | Reason=ModifySkippedOrFailed{9}",
                                CODE_NAME, BOT_LABEL, SymbolName, p.TradeType, p.Id,
                                unrealizedR.ToString("F2", CultureInfo.InvariantCulture),
                                BuildTpDiag(p, tpAbs2),
                                SET_BoostStructureTpLookbackH1_Stage2,
