...
// ============================================================
// BASE: EMA_M5_ALL_DAY_018_040_XAUUSD_M5
// THIS: EMA_M5_ALL_DAY_019_001_XAUUSD_M5
// ARCHIVE_ID: 019_001 (file export marker; no runtime effect)
// ============================================================
...
namespace cAlgo
{
    ...
    public class EA_DEV_FIRST : Robot
    {
        ...
        private const string CODE_NAME = "EMA_M5_ALL_DAY_019_001_XAUUSD_M5";
        private const string BOT_LABEL = "EMA_M5_ALL_DAY_019_001_XAUUSD_M5";
...
[Parameter("緊急クローズ倍率", Group = "資金管理・ロット制御", DefaultValue = 1.2, MinValue = 1.0)]
        ...
        [Parameter("建値移動トリガー", Group = "利確（TP）関連", DefaultValue = 1000.0, MinValue = 0.0)]
...
[Parameter("SLを使用（はい・いいえ）", Group = "ストップロス関連", DefaultValue = true)]
...
[Parameter("TPを使用（はい・いいえ）", Group = "利確（TP）関連", DefaultValue = true)]
...
[Parameter("TP方式（SL倍率/固定/ATR/構造）", Group = "利確（TP）関連", DefaultValue = TP方式.SL倍率)]
...
[Parameter("固定TP（PIPS）", Group = "利確（TP）関連", DefaultValue = 0.0, MinValue = 0.0)]
public double FixedTpPips { get; set; }

[Parameter("TP用ATR期間", Group = "利確（TP）関連", DefaultValue = 14, MinValue = 1)]
public int TpAtrPeriod { get; set; }

[Parameter("TP用ATR倍率", Group = "利確（TP）関連", DefaultValue = 2.0, MinValue = 0.0)]
public double TpAtrMult { get; set; }

[Parameter("TP構造スイング判定 左右本数（H1）", Group = "利確（TP）関連", DefaultValue = 2, MinValue = 1)]
public int TpSwingLR { get; set; }

[Parameter("TP構造スイング探索本数（H1）", Group = "利確（TP）関連", DefaultValue = 200, MinValue = 10)]
public int TpSwingLookback { get; set; }

[Parameter("構造TPバッファ（PIPS）（H1スイング手前に置く）", Group = "利確（TP）関連", DefaultValue = 50.0, MinValue = 0.0)]
...
// ============================================================
// 019_001 ADD: 001管理探索モード（001エントリー固定 / 管理のみ探索）
...
// ============================================================
// 019_001 ADD: 構造TP（H1スイング） + フォールバック（ATR）
// ============================================================
