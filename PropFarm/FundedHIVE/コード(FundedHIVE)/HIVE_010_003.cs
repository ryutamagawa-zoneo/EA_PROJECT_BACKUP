...
// ============================================================
// BASE: EMA_M5_ALL_DAY_020_033_XAUUSD_M5
// THIS: HIVE_010_003
// ARCHIVE_ID: 019_048 (file export marker; no runtime effect)
// ============================================================
...
namespace cAlgo
{
    ...
    public class HIVE_010_003 : Robot
    {
        // ============================================================
        // CODE NAME (Project Constitution compliant)
        // ============================================================
        // BASE: EMA_M5_ALL_DAY_020_033_XAUUSD_M5
        // THIS: HIVE_010_003
        private const string CODE_NAME = "HIVE_010_003";
        private const string BOT_LABEL = "HIVE_010_003";
        private const string BASE_CODE_NAME = "EMA_M5_ALL_DAY_020_033_XAUUSD_M5";
        ...
        [Parameter("PROレポート・OHLC出力", Group = "資金管理・ロット制御", DefaultValue = true)]
        public bool EnableProReport { get; set; }

        [Parameter("PROレポート保存先フォルダ", Group = "資金管理・ロット制御", DefaultValue = "D:\\ChatGPT EA Development\\ChatGPT EA Development\\バックテスト\\保管庫")]
        ...
        [Parameter("SLイベント管理を有効化", Group = "SLイベント管理", DefaultValue = false)]
        ...
        [Parameter("建値移動トリガー", Group = "SLイベント管理", DefaultValue = 0.0, MinValue = 0.0)]
        public double BreakevenTriggerDollars { get; set; }

        [Parameter("建値移動SLオフセット（PIPS）", Group = "SLイベント管理", DefaultValue = 0.0, MinValue = 0.0)]
        public double BreakevenSLOffsetPips { get; set; }

        // ============================================================
        // 以降は将来拡張用（現段階では未実装：ロジック参照なし）
        // パラメーター枠のみ追加し、挙動は032と完全一致させる。
        // ============================================================

        [Parameter("部分利確後SL移動を有効化", Group = "SLイベント管理", DefaultValue = false)]
        public bool EnablePartialCloseSlMove { get; set; }

        [Parameter("部分利確トリガー", Group = "SLイベント管理", DefaultValue = 0.0, MinValue = 0.0)]
        public double PartialCloseTriggerDollars { get; set; }

        public enum 部分利確後SL移動方式
        {
            現在SLを維持 = 0,
            建値へ移動 = 1,
            建値オフセットへ移動 = 2,
        ...
        [Parameter("部分利確後SL移動方式", Group = "SLイベント管理", DefaultValue = 部分利確後SL移動方式.現在SLを維持)]
        public 部分利確後SL移動方式 PartialCloseSlMoveMode { get; set; }

        [Parameter("段階的SLを有効化", Group = "SLイベント管理", DefaultValue = false)]
        public bool EnableStepSlMove { get; set; }

        [Parameter("段階SL利益間隔", Group = "SLイベント管理", DefaultValue = 0.0, MinValue = 0.0)]
        public double StepSlProfitIntervalDollars { get; set; }

        [Parameter("段階SL移動量（PIPS）", Group = "SLイベント管理", DefaultValue = 0.0, MinValue = 0.0)]
        public double StepSlMovePips { get; set; }
        ...
        private DateTime? _stopUntilUtc = null;

        private DateTime _dayAnchorJst = DateTime.MinValue;   // 直近の JST 09:00 アンカー
        private double _dayStartEquity = 0.0;

        private enum StopLiquidationState
{
    Idle = 0,
    InProgress = 1,
    Completed = 2,
...
private StopLiquidationState _stopLiquidationState = StopLiquidationState.Idle;
private int _stopLiquidationMode = 0;
private int _stopLiquidationAttempt = 0;
private DateTime _stopLiquidationNextAttemptUtc = DateTime.MinValue;
private bool _stopLiquidationLoggedDone = false;
private bool _stopLiquidationLoggedFail = false;

private const int STOP_LIQUIDATION_MAX_ATTEMPTS = 3;
private const int STOP_LIQUIDATION_RETRY_SECONDS = 2;

// 永久停止（合格）到達後：全決済完了したら cBot 自体も停止（バックテストでも停止して合格日が分かるようにする）
private bool _passStopBotStopIssued = false;
...
public bool TradePermission { get; private set; } = true;
        ...
        // ============================================================
        // FundedHIVE Compliance：停止判定（OnTick / OnTimer 2秒）
        // ============================================================

        private void EvaluateComplianceStop(DateTime utcNow, string source)
        {
            // JST 09:00 アンカー更新（= 日次切替）
            DateTime nowJst = ToJst(utcNow);
            DateTime anchorJst = GetDailyAnchorJst(nowJst);

            if (_dayAnchorJst == DateTime.MinValue)
            {
                _dayAnchorJst = anchorJst;
                _dayStartEquity = Account.Equity;

                Print("COMPLIANCE_INIT | CodeName={0} | Label={1} | DayAnchorJst={2} | DayStartEquity={3}",
                    CODE_NAME, BOT_LABEL, _dayAnchorJst.ToString("yyyy-MM-dd HH:mm:ss", CultureInfo.InvariantCulture),
                    _dayStartEquity.ToString("F2", CultureInfo.InvariantCulture));
            }
            else if (anchorJst != _dayAnchorJst)
            {
                _dayAnchorJst = anchorJst;
                _dayStartEquity = Account.Equity;

                // 日次切替時：日次停止は解除（永久停止は解除しない）
                if (_stopMode == 1)
                {
                    ClearStopMode("DAILY_RESET@09:00");
                }

                Print("COMPLIANCE_DAY_RESET | CodeName={0} | Label={1} | DayAnchorJst={2} | DayStartEquity={3}",
                    CODE_NAME, BOT_LABEL, _dayAnchorJst.ToString("yyyy-MM-dd HH:mm:ss", CultureInfo.InvariantCulture),
                    _dayStartEquity.ToString("F2", CultureInfo.InvariantCulture));
            }

            // 日次停止期限での解除（通常は 09:00 リセットで解除されるが、念のため保持）
            if (_stopMode == 1 && _stopUntilUtc.HasValue && utcNow >= _stopUntilUtc.Value)
            {
                ClearStopMode("DAILY_UNTIL_REACHED");
            }

            // 優先順位：永久停止（合格）＞日次停止（損失）
            if (_stopMode != 2)
            {
                if (InitialBalanceForPass > 0.0 && PassStopProfitPercent > 0.0)
                {
                    double passPct = (Account.Equity - InitialBalanceForPass) / InitialBalanceForPass * 100.0;

                    if (passPct >= PassStopProfitPercent)
                    {
                        ActivateStopMode(
                            2,
                            string.Format(CultureInfo.InvariantCulture, "PASS_STOP | Equity={0} Init={1} Pct={2:F3}% Target={3:F3}%",
                                Account.Equity, InitialBalanceForPass, passPct, PassStopProfitPercent),
                            null,
                            utcNow,
                            source);
                    }
            ...
            if (_stopMode == 0)
            {
                if (DailyStopLossPercent > 0.0 && _dayStartEquity > 0.0)
                {
                    double dailyPct = (Account.Equity - _dayStartEquity) / _dayStartEquity * 100.0;

                    if (dailyPct <= -DailyStopLossPercent)
                    {
                        DateTime nextResetJst = GetNextDailyResetJst(nowJst);
                        DateTime untilUtc = JstToUtc(nextResetJst);

                        ActivateStopMode(
                            1,
                            string.Format(CultureInfo.InvariantCulture, "DAILY_STOP | Equity={0} DayStart={1} Pct={2:F3}% Limit=-{3:F3}%",
                                Account.Equity, _dayStartEquity, dailyPct, DailyStopLossPercent),
                            untilUtc,
                            utcNow,
                            source);
                    }
            ...
            // 停止中：全決済（保留注文キャンセル→全ポジションクローズ）を完遂する
            if (_stopMode != 0)
            {
                EnsureStopLiquidation(utcNow);

                // 合格停止（永久停止）の場合は、全決済が完了したタイミングで cBot も停止する
                StopRobotIfPassStopAndLiquidated(utcNow, source);
            }
        }

        private void StopRobotIfPassStopAndLiquidated(DateTime utcNow, string source)
        {
            if (_stopMode != 2)
                return;

            if (_stopLiquidationState != StopLiquidationState.Completed)
                return;

            if (_passStopBotStopIssued)
                return;

            _passStopBotStopIssued = true;

            Print("COMPLIANCE_BOT_STOP | CodeName={0} | Label={1} | Source={2} | UtcNow={3} | Reason={4}",
                CODE_NAME,
                BOT_LABEL,
                source,
                utcNow.ToString("yyyy-MM-dd HH:mm:ss", CultureInfo.InvariantCulture),
                string.IsNullOrWhiteSpace(_stopReason) ? "NA" : _stopReason);

            Stop();
        }

        private DateTime GetDailyAnchorJst(DateTime nowJst)
        {
            DateTime today0900 = new DateTime(nowJst.Year, nowJst.Month, nowJst.Day, 9, 0, 0, DateTimeKind.Unspecified);
            if (nowJst.TimeOfDay >= TimeSpan.FromHours(9))
                return today0900;
            return today0900.AddDays(-1);
        }

        private DateTime GetNextDailyResetJst(DateTime nowJst)
        {
            DateTime today0900 = new DateTime(nowJst.Year, nowJst.Month, nowJst.Day, 9, 0, 0, DateTimeKind.Unspecified);
            if (nowJst.TimeOfDay < TimeSpan.FromHours(9))
                return today0900;
        ...
        private DateTime JstToUtc(DateTime jst)
        {
            if (_jstTz == null)
                return DateTime.SpecifyKind(jst, DateTimeKind.Utc);

            DateTime jstUnspecified = DateTime.SpecifyKind(jst, DateTimeKind.Unspecified);
            return TimeZoneInfo.ConvertTimeToUtc(jstUnspecified, _jstTz);
        }

        private void ActivateStopMode(int mode, string reason, DateTime? untilUtc, DateTime utcNow, string source)
        {
            // G1：同一停止モードへの再突入は抑制（ただし 1→2 の昇格は許可）
            if (_stopMode == mode)
                return;

            if (mode == 1 && _stopMode != 0)
                return;

            _stopMode = mode;
            _stopReason = reason ?? "";
            _stopUntilUtc = untilUtc;

            // 公開ゲート同期
            ApplyTradePermission(false, _stopReason, _stopUntilUtc);

            string untilStr = _stopUntilUtc.HasValue ? _stopUntilUtc.Value.ToString("yyyy-MM-dd HH:mm:ss", CultureInfo.InvariantCulture) : "null";

            Print("COMPLIANCE_STOP | CodeName={0} | Label={1} | Mode={2} | Source={3} | UntilUtc={4} | Reason={5}",
                CODE_NAME, BOT_LABEL, _stopMode, source, untilStr, _stopReason);

            // 全決済（C）開始
            StartStopLiquidation(utcNow);
        }

        private void ClearStopMode(string source)
        {
            if (_stopMode == 2)
                return;
            ...
            _stopLiquidationState = StopLiquidationState.Idle;
            _stopLiquidationMode = 0;
            _stopLiquidationAttempt = 0;
            _stopLiquidationNextAttemptUtc = DateTime.MinValue;
            _stopLiquidationLoggedDone = false;
            _stopLiquidationLoggedFail = false;

            ApplyTradePermission(true, "", null);

            Print("COMPLIANCE_RESUME | CodeName={0} | Label={1} | Source={2}",
                CODE_NAME, BOT_LABEL, source);
        }

        private void StartStopLiquidation(DateTime utcNow)
{
    // 停止モード突入時に呼ばれる。状態機械を初期化し、必要なら次の Ensure で実行される。
    _stopLiquidationState = StopLiquidationState.Idle;
    _stopLiquidationMode = _stopMode;
    _stopLiquidationAttempt = 0;
    _stopLiquidationNextAttemptUtc = utcNow;
    _stopLiquidationLoggedDone = false;
    _stopLiquidationLoggedFail = false;
}
private void EnsureStopLiquidation(DateTime utcNow)
{
    // 停止モードが変わったら初期化（例：1→2）
    if (_stopLiquidationMode != _stopMode)
    {
        StartStopLiquidation(utcNow);
    }

    int pendingNow = CountMyPendingOrders();
    int posNow = CountMyPositions();

    // 既にフラットなら何もしない（ログも出さない）
    if (pendingNow == 0 && posNow == 0)
    {
        if (_stopLiquidationState != StopLiquidationState.Completed)
        {
            _stopLiquidationState = StopLiquidationState.Completed;

            if (!_stopLiquidationLoggedDone)
            {
                _stopLiquidationLoggedDone = true;
                Print("COMPLIANCE_LIQUIDATE_DONE | CodeName={0} | Label={1} | Mode={2}",
                    CODE_NAME, BOT_LABEL, _stopMode);
            }
        }
        return;
    }

    // フラットでなくなった（外部要因など）場合は再実行可能に戻す
    if (_stopLiquidationState == StopLiquidationState.Completed)
    {
        _stopLiquidationState = StopLiquidationState.Idle;
        _stopLiquidationLoggedDone = false;
    ...
    // 既に打ち切り済みなら何もしない（停止モードは維持）
    if (_stopLiquidationState == StopLiquidationState.Failed)
        return;

    // リトライ待ち
    if (utcNow < _stopLiquidationNextAttemptUtc)
        return;

    if (_stopLiquidationAttempt >= STOP_LIQUIDATION_MAX_ATTEMPTS)
    {
        _stopLiquidationState = StopLiquidationState.Failed;

        if (!_stopLiquidationLoggedFail)
        {
            _stopLiquidationLoggedFail = true;
            Print("COMPLIANCE_LIQUIDATE_FAIL | CodeName={0} | Label={1} | Mode={2} | Attempts={3}/{4} | Pending={5} | Pos={6}",
                CODE_NAME, BOT_LABEL, _stopMode, _stopLiquidationAttempt, STOP_LIQUIDATION_MAX_ATTEMPTS, pendingNow, posNow);
        }
        return;
    }

    _stopLiquidationState = StopLiquidationState.InProgress;
    _stopLiquidationAttempt++;

    int pendingBefore = pendingNow;
    int posBefore = posNow;

    // 1) 保留注文全キャンセル
    CancelAllMyPendingOrders();

    // 2) 全ポジションクローズ
    CloseAllMyPositions("COMPLIANCE_STOP", _stopReason);

    int pendingAfter = CountMyPendingOrders();
    int posAfter = CountMyPositions();

    Print("COMPLIANCE_LIQUIDATE | CodeName={0} | Label={1} | Mode={2} | Attempt={3}/{4} | Pending {5}->{6} | Pos {7}->{8}",
        CODE_NAME, BOT_LABEL, _stopMode, _stopLiquidationAttempt, STOP_LIQUIDATION_MAX_ATTEMPTS,
        pendingBefore, pendingAfter, posBefore, posAfter);

    if (pendingAfter == 0 && posAfter == 0)
    {
        _stopLiquidationState = StopLiquidationState.Completed;
    ...
    // 取り残しがあればリトライ（2秒）
    _stopLiquidationNextAttemptUtc = utcNow.AddSeconds(STOP_LIQUIDATION_RETRY_SECONDS);
}

private int CountMyPositions(
)
        {
            int c = 0;
        ...
        private int CountMyPendingOrders()
        {
            int c = 0;
            for (int i = 0; i < PendingOrders.Count; i++)
            {
                PendingOrder o = PendingOrders[i];
                if (o == null) continue;
                if (o.SymbolName != SymbolName) continue;
                if (o.Label != BOT_LABEL) continue;
                c++;
            }
            return c;
        }

        private void CancelAllMyPendingOrders()
        {
        ...
        private void CloseAllMyPositions(string trigger, string reason)
        {
            for (int i = Positions.Count - 1; i >= 0; i--)
            {
                ...
                try
                {
                    _closeInitiatorByPosId[p.Id] = "FORCE_CLOSE:COMPLIANCE";
                }
                catch { }
        ...
        protected override void OnStart()
        {
            ...
            EvaluateComplianceStop(UtcNow(), "OnStart");
            // ============================
        ...
        protected override void OnTick()
        {
            ...
            EvaluateComplianceStop(utcNow, "OnTick");
            if (_stopMode != 0)
                return;
        ...
        protected override void OnTimer()
        {
            ...
            EvaluateComplianceStop(utcNow, "OnTimer");
            if (_stopMode != 0)
                return;
        ...
        private void ApplyBreakevenMoveIfNeeded()
        {
            ...
            for (int i = Positions.Count - 1; i >= 0; i--)
            {
                ...
                bool reached = (p.NetProfit + 1e-12 >= trigger);

                if (EnableDebugLogJst)
                {
                    TryPrintOncePerBar(p.Id, "BE_CHECK",
                        string.Format(CultureInfo.InvariantCulture,
                            "BE_CHECK | CodeName={0} | Symbol={1} | PosId={2} | Type={3} | UnrealizedUSD={4} | TriggerUSD={5} | Reached={6} | Entry={7} | SL={8} | TP={9} | OffsetPips={10}{11}",
                            CODE_NAME,
                            SymbolName,
                            p.Id,
                            p.TradeType,
                            p.NetProfit.ToString("F2", CultureInfo.InvariantCulture),
                            trigger.ToString("F2", CultureInfo.InvariantCulture),
                            reached ? "true" : "false",
                ...
                if (p.TradeType == TradeType.Buy)
                {
                    if (p.StopLoss.HasValue && p.StopLoss.Value + Symbol.PipSize * 0.01 >= entry)
                    {
                        ...
                        if (EnableDebugLogJst)
                        {
                            TryPrintOncePerBar(p.Id, "BE_SKIPPED",
                                string.Format(CultureInfo.InvariantCulture,
                                    "BE_SKIPPED | CodeName={0} | Symbol={1} | PosId={2} | Reason=AlreadyBE{3}",
                    ...
                    if (EnableDebugLogJst)
                    {
                        TryPrintOncePerBar(p.Id, ok ? "BE_APPLIED" : "BE_SKIPPED",
                            string.Format(CultureInfo.InvariantCulture,
                                ok
                                    ? "BE_APPLIED | CodeName={0} | Symbol={1} | PosId={2} | NewSL={3} | TP={4} | UnrealizedUSD={5} | TriggerUSD={6}{7}"
                                    : "BE_SKIPPED | CodeName={0} | Symbol={1} | PosId={2} | Reason=ModifyFailed | NewSL={3} | TP={4} | UnrealizedUSD={5} | TriggerUSD={6}{7}",
                                CODE_NAME,
                                SymbolName,
                                p.Id,
                                NormalizePrice(newSl).ToString("F5", CultureInfo.InvariantCulture),
                                (p.TakeProfit.HasValue ? p.TakeProfit.Value.ToString("F5", CultureInfo.InvariantCulture) : "null"),
                                p.NetProfit.ToString("F2", CultureInfo.InvariantCulture),
                                trigger.ToString("F2", CultureInfo.InvariantCulture),
                                BuildTimeTag(utcNow)));
        ...
        private void TryEmaEntry()
        {
            ...
            if (UseCOREEntrySuppressionStructure())
            {
                ...
                if (crossUp || crossDown)
                {
                    ...
                    if (!DirectionAllowsCOREModeEmaEntry(signalIndex, close1, ema1, plannedType.Value))
                    {
                        return;
                    }

                    // Distance gate (set pending if too far)
                    if (!IsCOREModeDistanceOkOrSetPending(signalIndex, close1, ema1, plannedType.Value, out string distReason))
                    {
                        return;
                    }
                }
                else
                {
                    ...
                    if (!TryConsumeCOREModeReapproachSignal(signalIndex, close1, ema1, out TradeType intended, out string reapReason))
                        return;
        ...
        private void PlaceTrade(TradeType type, double entry, double stop, double tpTargetPrice, string reasonTag)
        {
            ...
            if (EnableAtrEnvGate)
            {
                ...
                double atrPips = (Symbol != null && Symbol.PipSize > 0) ? atr / Symbol.PipSize : 0.0;

                if (AtrEnvMinPips > 0.0 && atrPips > 0.0 && atrPips < AtrEnvMinPips)
                {
                    if (EnableDebugLogJst)
                        Print("ATR_ENV_GATE_BLOCK | ATR(pips)={0:F2} < Min={1:F2}{2}", atrPips, AtrEnvMinPips, BuildTimeTag(UtcNow()));
                    return;
                }

                if (AtrEnvMaxPips > 0.0 && atrPips > 0.0 && atrPips > AtrEnvMaxPips)
                {
                    if (EnableDebugLogJst)
                        Print("ATR_ENV_GATE_BLOCK | ATR(pips)={0:F2} > Max={1:F2}{2}", atrPips, AtrEnvMaxPips, BuildTimeTag(UtcNow()));
                    return;
                }
            }
...
";
        ...
        private bool TryApplyDirectionRestriction(TradeType plannedType, DateTime utcNow, int barIndex, string contextTag, out string blockReason)
        {
            ...
            if (_dirMode == DirectionRestrictionMode.Normal || _dirModeRemainingTrades <= 0)
            {
                ...
                if (bias >= biasThr && emaSlopePips >= slopeThr)
                {
                    ...
                    Print("DIRMODE_ON | Mode=BUY_ONLY | LossStreak={0}/{1} | Bias72={2:F3} NH={3} NL={4} | EmaSlope(pips/{5}bars)={6:F3} | Ctx={7}{8}",
                        _lossStreak, DirRestrictTriggerLossStreak, bias, nh, nl, slopeBars, emaSlopePips, contextTag, BuildTimeTag(utcNow));
                }
                else if (bias <= -biasThr && emaSlopePips <= -slopeThr)
                {
        ...
        {
            // PROレポートの保存先と同一系統に合わせる（既存仕様の踏襲）
            string dir = ProReportOutputFolder;
            if (string.IsNullOrWhiteSpace(dir))
                dir = @"D:\ChatGPT EA Development\ChatGPT EA Development\バックテスト\保管庫";
        ...
        private void OnPositionClosed(PositionClosedEventArgs args)
        {
            ...
            try
            {
                ...
                {
                    if (pp == null) continue;
                    if (pp.SymbolName != SymbolName) continue;
        ...
        private bool ExportProReportHtml(out string savedPath)
        {
            ...
            // 出力
            // NOTE: "1回のテスト=1ファイル" を保証するため、同一テスト内で複数回保存されても同名にして上書きする。
            // ファイル名は「CODE_NAME + runId + テスト期間」で固定する。
            string runId = _proRunId;
            if (string.IsNullOrWhiteSpace(runId))
            {
                // fallback（通常はOnStartで設定済み）
                runId = ToJst(DateTime.UtcNow).ToString("yyyyMMdd_HHmmss_fff", CultureInfo.InvariantCulture);
                _proRunId = runId;
            }

            var fromStamp = fromUtc.HasValue ? fromUtc.Value.ToString("yyyyMMdd_HHmm", CultureInfo.InvariantCulture) : "NA";
            var toStamp = toUtc.HasValue ? toUtc.Value.ToString("yyyyMMdd_HHmm", CultureInfo.InvariantCulture) : "NA";

            var baseName = "PRO_RESULT_" + CODE_NAME + "_" + runId + "_" + fromStamp + "_" + toStamp;
            ...
            try
            {
                string ohlcCsvPath = Path.ChangeExtension(fullPath, ".csv");
