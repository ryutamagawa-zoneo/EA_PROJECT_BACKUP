...
// ============================================================
// BASE: EMA_M5_ALL_DAY_020_052_XAUUSD_M5
// THIS: HIVE_010_004
// ARCHIVE_ID: 010_004 (file export marker; no runtime effect)
// ============================================================
...
namespace cAlgo
{
    ...
    public class HIVE_010_004 : Robot
    {
        // ============================================================
        // CODE NAME (Project Constitution compliant)
        // ============================================================
        // BASE: EMA_M5_ALL_DAY_020_052_XAUUSD_M5
        // THIS: HIVE_010_004
        private const string CODE_NAME = "HIVE_010_004";
        private const string BOT_LABEL = CODE_NAME;
        private const string BASE_CODE_NAME = "EMA_M5_ALL_DAY_020_052_XAUUSD_M5";
...
public double DailyStopLossPercent { get; set; }
        ...
        [Parameter("PROレポート保存先フォルダ", Group = "資金管理・ロット制御", DefaultValue = "D:\\ChatGPT EA Development\\ChatGPT EA Development\\プロジェクト\\保管庫")]
...
[Parameter("出口：5分足クロスで終了", Group = "トレンドモード（出口）", DefaultValue = true)]
        ...
        [Parameter("トレンド出口判定ログを出力", Group = "トレンドモード（出口）", DefaultValue = true)]
        public bool EnableTrendExitDecisionLog { get; set; }
        ...
        [Parameter("SLイベント判定ログを出力", Group = "SLイベント管理", DefaultValue = false)]
        public bool EnableSLEventCheckLog { get; set; }
        ...
        protected override void OnStart()
        {
            ...
            // FundedHIVE Compliance (prop) evaluation
        ...
        protected override void OnBar()
        {
            ...
            // FundedHIVE Compliance (prop) evaluation (timer-based)
            if (_hiveCompliance != null)
...
private string FormatUserPipsWithDollar(double userPips)
        ...
        private string ResolveProOutputDirectory()
        {
            ...
            if (string.IsNullOrWhiteSpace(dir))
                dir = @"D:\ChatGPT EA Development\ChatGPT EA Development\プロジェクト\保管庫";
...
// ============================================================
// FundedHIVE Compliance Controller (Prop Layer)
//  - No-touch to CORE logic: only updates trade gate + performs liquidation when stopped
//  - Daily reset anchor: JST 09:00
//  - Daily loss basis: Equity (as per 003 implementation)
// ============================================================
private sealed class HiveComplianceController
{
    private readonly HIVE_010_004 _bot;

    private int _stopMode = 0; // 0=run, 1=daily stop, 2=pass stop(permanent)
    private string _stopReason = "";
    private DateTime? _stopUntilUtc = null;

    private DateTime _dayAnchorJst = DateTime.MinValue;
    ...
    // Timer throttling (052 Timer may be 1s; compliance requires 2s)
    private DateTime _lastTimerEvalUtc = DateTime.MinValue;

    public HiveComplianceController(HIVE_010_004 bot)
    {
        _bot = bot;
    }
    ...
    public void Update(DateTime utcNow, string source)
    {
        ...
        if (string.Equals(source, "OnTimer", StringComparison.Ordinal))
        {
            if (_lastTimerEvalUtc != DateTime.MinValue && (utcNow - _lastTimerEvalUtc).TotalSeconds < 2.0)
                return;
            _lastTimerEvalUtc = utcNow;
        }

        EvaluateComplianceStop(utcNow, source);
    }

    private void EvaluateComplianceStop(DateTime utcNow, string source)
    {
        DateTime nowJst = _bot.ToJst(utcNow);
        DateTime anchorJst = GetDailyAnchorJst(nowJst);

        if (_dayAnchorJst == DateTime.MinValue)
        {
            _dayAnchorJst = anchorJst;
            _dayStartEquity = _bot.Account.Equity;

            _bot.Print("COMPLIANCE_INIT | CodeName={0} | Label={1} | DayAnchorJst={2} | DayStartEquity={3}",
        ...
        else if (anchorJst != _dayAnchorJst)
        {
            ...
            _bot.Print("COMPLIANCE_DAY_RESET | CodeName={0} | Label={1} | DayAnchorJst={2} | DayStartEquity={3}",
                CODE_NAME, BOT_LABEL, _dayAnchorJst.ToString("yyyy-MM-dd HH:mm:ss", CultureInfo.InvariantCulture),
                _dayStartEquity.ToString("F2", CultureInfo.InvariantCulture));
        }
        ...
        // Priority: pass stop (2) > daily stop (1)
        if (_stopMode != 2)
        {
            if (_bot.InitialBalanceForPass > 0.0 && _bot.PassStopProfitPercent > 0.0)
            {
                double passPct = (_bot.Account.Equity - _bot.InitialBalanceForPass) / _bot.InitialBalanceForPass * 100.0;
                if (passPct >= _bot.PassStopProfitPercent)
                {
                    ActivateStopMode(
                        2,
                        string.Format(CultureInfo.InvariantCulture, "PASS_STOP | Equity={0} Init={1} Pct={2:F3}% Target={3:F3}%",
                            _bot.Account.Equity, _bot.InitialBalanceForPass, passPct, _bot.PassStopProfitPercent),
                        null,
                        utcNow,
                        source);
                }
        ...
        if (_stopMode == 0)
        {
            if (_bot.DailyStopLossPercent > 0.0 && _dayStartEquity > 0.0)
            {
                double dailyPct = (_bot.Account.Equity - _dayStartEquity) / _dayStartEquity * 100.0;
                if (dailyPct <= -_bot.DailyStopLossPercent)
                {
                    ...
                    ActivateStopMode(
                        ...
                        string.Format(CultureInfo.InvariantCulture, "DAILY_STOP | Equity={0} DayStart={1} Pct={2:F3}% Limit=-{3:F3}%",
                            _bot.Account.Equity, _dayStartEquity, dailyPct, _bot.DailyStopLossPercent),
                        untilUtc,
                        utcNow,
                        source);
                }
    ...
    private void StopRobotIfPassStopAndLiquidated(DateTime utcNow, string source)
    {
        ...
        _bot.Print("COMPLIANCE_BOT_STOP | CodeName={0} | Label={1} | Source={2} | UtcNow={3} | Reason={4}",
            CODE_NAME,
            BOT_LABEL,
            source,
    ...
    private DateTime JstToUtc(DateTime jst)
    {
        ...
        return TimeZoneInfo.ConvertTimeToUtc(jstUnspecified, _bot._jstTz);
    }

    private void ActivateStopMode(int mode, string reason, DateTime? untilUtc, DateTime utcNow, string source)
    {
        // G1: suppress re-entry to same stop mode (allow 1->2 escalation)
        if (_stopMode == mode)
            return;
        ...
        _bot.SetTradePermission(false, _stopReason, _stopUntilUtc);
        ...
        _bot.Print("COMPLIANCE_STOP | CodeName={0} | Label={1} | Mode={2} | Source={3} | UntilUtc={4} | Reason={5}",
    ...
    private void ClearStopMode(string source)
    {
        ...
        _bot.SetTradePermission(true, "", null);

        _bot.Print("COMPLIANCE_RESUME | CodeName={0} | Label={1} | Source={2}",
    ...
    private void EnsureStopLiquidation(DateTime utcNow)
    {
        ...
        if (pendingNow == 0 && posNow == 0)
        {
            if (_stopLiquidationState != StopLiquidationState.Completed)
            {
                ...
                if (!_stopLiquidationLoggedDone)
                {
                    ...
                    _bot.Print("COMPLIANCE_LIQUIDATE_DONE | CodeName={0} | Label={1} | Mode={2}",
        ...
        if (_stopLiquidationAttempt >= STOP_LIQUIDATION_MAX_ATTEMPTS)
        {
            ...
            if (!_stopLiquidationLoggedFail)
            {
                ...
                _bot.Print("COMPLIANCE_LIQUIDATE_FAIL | CodeName={0} | Label={1} | Mode={2} | Attempts={3}/{4} | Pending={5} | Pos={6}",
        ...
        _bot.Print("COMPLIANCE_LIQUIDATE | CodeName={0} | Label={1} | Mode={2} | Attempt={3}/{4} | Pending {5}->{6} | Pos {7}->{8}",
    ...
    {
        int c = 0;
        for (int i = 0; i < _bot.Positions.Count; i++)
        {
            var p = _bot.Positions[i];
            if (p == null) continue;
    ...
    {
        int c = 0;
        for (int i = 0; i < _bot.PendingOrders.Count; i++)
        {
            var o = _bot.PendingOrders[i];
            if (o == null) continue;
            if (o.SymbolName != _bot.SymbolName) continue;
    ...
    {
        for (int i = _bot.PendingOrders.Count - 1; i >= 0; i--)
        {
            var o = _bot.PendingOrders[i];
            if (o == null) continue;
            if (o.SymbolName != _bot.SymbolName) continue;
            ...
            try { _bot.CancelPendingOrder(o); }
            catch { }
        }
    }

    private void CloseAllMyPositions()
    {
        for (int i = _bot.Positions.Count - 1; i >= 0; i--)
        {
            var p = _bot.Positions[i];
            if (p == null) continue;
            if (p.SymbolName != _bot.SymbolName) continue;
            ...
            try
            {
                if (_bot._closeInitiatorByPosId != null)
                    _bot._closeInitiatorByPosId[p.Id] = "FORCE_CLOSE:COMPLIANCE";
            }
            catch { }

            try { _bot.ClosePosition(p); }
            catch { }
        }
    }
}
